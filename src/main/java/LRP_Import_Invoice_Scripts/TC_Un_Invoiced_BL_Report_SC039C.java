...
public class TC_Un_Invoiced_BL_Report_SC039C extends Keywords {

	public void Un_Invoiced_BL_Report_SC039C(WebDriver driver, ExtentTest test, ExtentTest test1, String selected_dataset,Map<String, String> Excel_data,String url)
			throws Exception {

		String testcase_Name = "TC_Un_Invoiced_BL_Report_SC039C";
		...
		waitForElement(driver, total_Revenue_GridCell_UIB);
		String actual_TotalRevenue = getText(driver, total_Revenue_GridCell_UIB);
		System.out.println("actual_TotalRevenue : " + actual_TotalRevenue);
		String act_TotalRevenue = actual_TotalRevenue.replace(",", "");
		double actual_TotalRevenue_UIB = getNumberValue(act_TotalRevenue);

		String actual_AgencyRevenue="";
		if(isdisplayed(driver, agencyRevenue_GridCell_UIB)) {
			waitForElement(driver, agencyRevenue_GridCell_UIB);
			actual_AgencyRevenue = getText(driver, agencyRevenue_GridCell_UIB);
		}
		...
		if (!actual_BL_MCN_Number.contains("<-->")) { // AE00216200<-->AE00216200CV3
			...
			List<String> amount_In_USD = new ArrayList<String>();

			Step_Start(21,	"Shipment type has Export Invoice Find the Payment Mode Values as Prepaid GO to Amount in USD.\r\n"
							+ "Sum all the Amount and compare both Uninvoiced  report and BL.",
					test, test1);

			Extent_call(test, test1, "Compare Values For Total Revenue .");
			...
			if (blType.equals("Export") || actualInvoiceType.equals("Export Invoice")) {
				...
				amount_In_USD = getValuesByFirstColumnAndHeader(billOfLading_Charges, "Payment Mode", "Prepaid",
						"Amount In USD");

				if (amount_In_USD.size() == 0) {
					amount_In_USD.add("0.00");
				}

				System.out.println("amount_In_USD : " + amount_In_USD);
			}

			Step_End(21,
					"Shipment type has Export Invoice Find the Payment Mode Values as Prepaid GO to Amount in USD.\r\n"
							+ "Sum all the Amount and compare both Uninvoiced  report and BL.",
					test, test1);
			Step_Start(22,
					"Shipment type has Import Invoice Find the Payment Mode Values as Collect GO to Amount in USD.\r\n"
							+ "Sum all the Amount and compare both Uninvoiced  report and BL.",
					test, test1);
			double agencyAmount=0;
			if (blType.equals("Import") || actualInvoiceType.equals("Import Invoice")) {

				
				if(!actual_AgencyRevenue.trim().equals("")) {
					agencyAmount=getNumberValue(actual_AgencyRevenue);
				}
				Extent_pass_New(driver, "Shipment type is Import Invoice", test, test1);

				amount_In_USD = getValuesByFirstColumnAndHeader(billOfLading_Charges, "Payment Mode", "Collect",
						"Amount In USD");
			...
			Step_End(22,
					...
					test, test1);
			Step_Start(23,
					"Shipment type has Cross Invoice Find the Payment Mode Values as Payat GO to Amount in USD.\r\n"
							+ "Sum all the Amount and compare both Uninvoiced  report and BL.",
					test, test1);
			...
			if (actualInvoiceType.equals("Cross Invoice")) {
				...
				List<String> amount_In_USD1 = getValuesByFirstColumnAndHeader(billOfLading_Charges, "Payment Mode", "PayAt-Prepaid",
						"Amount In USD");

				List<String> amount_In_USD2 = getValuesByFirstColumnAndHeader(billOfLading_Charges, "Payment Mode", "PayAt-Collect",
						"Amount In USD");
				
				List<String> amount_In_USD3 = getValuesByFirstColumnAndHeader(billOfLading_Charges, "Payment Mode", "PayAt",
						"Amount In USD");
				
				amount_In_USD.addAll(amount_In_USD1);
				amount_In_USD.addAll(amount_In_USD2);
				amount_In_USD.addAll(amount_In_USD3);
			...
			double totalRevenue_BOL = 0;
			for (int i = 0; i < amount_In_USD.size(); i++) {

				String actValue = (amount_In_USD.get(i)).replace(",", "");
				totalRevenue_BOL = totalRevenue_BOL + getNumberValue(actValue);
			}

			if (blType.equals("Import") || actualInvoiceType.equals("Import Invoice")) {
				totalRevenue_BOL=totalRevenue_BOL+agencyAmount;
			}
			...
			if (actual_TotalRevenue_UIB == totalRevenue_BOL) {
				System.out.println("Matched || Total Revenue Bill of lading  : " + totalRevenue_BOL
						+ " || Total Revevenue in Uninvoiced BL Report : " + actual_TotalRevenue_UIB);
				Extent_pass_New(driver,
						"Matched || Total Revenue Bill of lading  : " + totalRevenue_BOL
								+ " || Total Revevenue in Uninvoiced BL Report : " + actual_TotalRevenue_UIB,
						test, test1);
			} else {
				System.out.println("Not Matched || Total Revenue Bill of lading : " + totalRevenue_BOL
						+ " || Total Revevenue in Uninvoiced BL Report : " + actual_TotalRevenue_UIB);
				Extent_fail(driver,
						"Not Matched || Total Revenue Bill of lading : " + totalRevenue_BOL
								+ " || Total Revevenue in Uninvoiced BL Report : " + actual_TotalRevenue_UIB,
						test, test1);
			}
		

			Step_End(23,	"Shipment type has Cross Invoice Find the Payment Mode Values as Payat GO to Amount in USD.\r\n"
							+ "Sum all the Amount and compare both Uninvoiced  report and BL.",
					test, test1);
...
//		Total Revenue
			...
			List<String> amount_In_USD = getValuesByHeader(MCN_Charges, "Amount In USD");

			if (blType.equals("Export") || actualInvoiceType.equals("Export Invoice")) {

				Extent_pass_New(driver, "Shipment type is Export Invoice", test, test1);

				amount_In_USD = getValuesByFirstColumnAndHeader(MCN_Charges, "Payment Mode", "Prepaid",
						"Amount in USD");
			...
			if (blType.equals("Import") || actualInvoiceType.equals("Import Invoice")) {
				...
				amount_In_USD = getValuesByFirstColumnAndHeader(MCN_Charges, "Payment Mode", "Collect",
						"Amount in USD");
			...
			if (actualInvoiceType.equals("Cross Invoice")) {
				...
				List<String> amount_In_USD1 = getValuesByFirstColumnAndHeader(MCN_Charges, "Payment Mode", "PayAt-Prepaid", "Amount in USD");
				List<String> amount_In_USD2 = getValuesByFirstColumnAndHeader(MCN_Charges, "Payment Mode", "PayAt-Collect", "Amount in USD");
				List<String> amount_In_USD3 = getValuesByFirstColumnAndHeader(MCN_Charges, "Payment Mode", "PayAt", "Amount in USD");
			...
			double totalRevenue_MCN = 0;
			for (int i = 0; i < amount_In_USD.size(); i++) {

				String actValue = (amount_In_USD.get(i)).replace(",", "");
				totalRevenue_MCN = totalRevenue_MCN + Double.parseDouble(actValue);
			}

			Extent_pass_New(driver, " MCN Charges Table : "+MCN_Charges_Tabledata, test, test1);
			Extent_pass_New(driver,"Un Invoiced BL Report Table"+UnInvoiced_Table_Data,test,test1);
			
			if (actual_TotalRevenue_UIB == totalRevenue_MCN) {
				System.out.println("Matched || Total Revenue MCN  : " + totalRevenue_MCN
						+ " || Total Revevenue in Uninvoiced BL Report : " + actual_TotalRevenue_UIB);
				Extent_pass_New(driver,
						"Matched || Total Revenue MCN  : " + totalRevenue_MCN
								+ " || Total Revevenue in Uninvoiced BL Report : " + actual_TotalRevenue_UIB,
						test, test1);
			} else {
				System.out.println("Not Matched || Total Revenue MCN : " + totalRevenue_MCN
						+ " || Total Revevenue in Uninvoiced BL Report : " + actual_TotalRevenue_UIB);
				Extent_fail(driver,
						"Not Matched || Total Revenue MCN : " + totalRevenue_MCN
								+ " || Total Revevenue in Uninvoiced BL Report : " + actual_TotalRevenue_UIB,
						test, test1);
			}
