...
public class TC_Journal_Upload_JU06 extends Keywords{
	public void Journal_Upload_JU06(WebDriver driver, ExtentTest test, ExtentTest test1,String selected_dataset,Map<String, String> Excel_data,String url) throws EncryptedDocumentException, InvalidFormatException, IOException{
		String testcase_Name="TC_Journal_Upload_JU06";
		...
		String Exist_Remarks = Excel_data.get("Exist_Remarks"); 
		...
		Step_Start(8, "Journal entries not verified it showing the validation  in Remarks column - \"Reference Number (Eg:233032) Already Exists\"", test, test1);
		...
		for(int i=0;i<remarks_value.size();i++) {
			if (Exist_Remarks.contains(remarks_value.get(i))) {
				Extent_pass_New(driver, "Matched || Expected Remark is : " + Exist_Remarks + " || Actual Remarks in Journal Module : " + remarks_value.get(i), test,test1);
				System.out.println("Matched || Expected Remark is : " + Exist_Remarks + " || Actual Remarks in Journal Module : " + remarks_value.get(i));
			} else {
				System.out.println("Not Matched || Expected Remark is : " + Exist_Remarks + " || Actual Remarks in Journal Module : " + remarks_value.get(i));
				Extent_fail(driver,"Not Matched || Expected Remark is : " + Exist_Remarks + " || Actual Remarks in Journal Module : " + remarks_value.get(i), test,test1);
			}
		}
		Step_End(8, "Journal entries not verified it showing the validation  in Remarks column - \"Reference Number (Eg:233032) Already Exists\"", test, test1);
