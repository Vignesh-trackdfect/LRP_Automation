...
public class TC_Contract_Management_System_SC025 extends Keywords {
	
	public void Contract_Management_System_SC25(WebDriver driver, ExtentTest test, ExtentTest test1,
			String selected_dataset,Map<String, String> Excel_data,String url) {

		String testCaseName = "TC_Contract_Management_System_SC025";
	...
	waitForElement(driver, NewButton_ToolBar);
	 click(driver, NewButton_ToolBar);
		...
		if (expectedDate.equals(ValidTo_date)) {
			Extent_pass(driver,
					"Matched || Expected value is :  The efective  date from the CMS module should be  : "
							+ expectedDate
							+ " || the valid TO  date from the RateView module should be less than  one day is  :"
							+ ValidTo_date,
					test, test1);
			System.out.println("Matched || Expected value is :  The efective  date from the CMS module should  be  : "
					+ expectedDate + "|| the valid TO  date from the RateView module should be less than  one day is :"
					+ ValidTo_date);
		} else {
			System.out
					.println("Not Matched || Expected value is :   The efective  date from the CMS module should be : "
							+ expectedDate
							+ " || the valid TO  date from the RateView module should be less than  one day is : "
							+ ValidTo_date);
			Extent_fail(driver,
					"Not Matched || Expected value is :   The efective  date from the CMS module should be : "
							+ expectedDate
							+ " || Actual value is : the valid TO  date from the RateView module should be less than  one day is: "
							+ ValidTo_date,
					test, test1);
