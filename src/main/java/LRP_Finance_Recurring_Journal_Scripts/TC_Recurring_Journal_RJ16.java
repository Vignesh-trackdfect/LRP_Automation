...
public class TC_Recurring_Journal_RJ16 extends Keywords{
	public void Recurring_Journal_RJ16(WebDriver driver, ExtentTest test, ExtentTest test1,String selected_dataset,Map<String, String> Excel_data,String url) throws EncryptedDocumentException, InvalidFormatException, IOException{
		String testcase_Name="TC_Recurring_Journal_RJ16";
		...
		waitForElement(driver, Posting_details_Page_RJ);
		...
		waitForElement(driver, scroll_RJ);
//		horizontalscroll(driver, scroll_RJ, 1000);
		List<Map<String, String>> table_Data = getTableData(driver, posting_Details_Grid_Header, posting_Details_Grid_Rows);
		List<String> Activity_dates = getValuesByHeader(table_Data, "Activity Date");
		System.out.println("Activity_Date :"+ Activity_dates);
		DateTimeFormatter formatter = DateTimeFormatter.ofPattern("dd/MM/yyyy");
		LocalDate fromDate = LocalDate.parse(From_date, formatter);
		LocalDate toDate = LocalDate.parse(To_date, formatter);
		for (String Activity_Date : Activity_dates) {
			LocalDate currentDate = LocalDate.parse(Activity_Date, formatter); // Parse the date
			...
			if ((currentDate.isEqual(fromDate) || currentDate.isAfter(fromDate)) &&
				...
				System.out.println("Expected Result is : The Current date should be with in the range of from and to Date || The Actual Result is : Date " + currentDate + " is within the range of "+From_date+" and "+To_date);
				Extent_pass_New(driver,"Expected Result is : The Current date should be with in the range of from and to Date || The Actual Result is : Date " + currentDate + " is within the range of "+From_date+" and "+To_date, test, test1);
			} else {
				System.out.println("Expected Result is : The Current date should be with in the range of from and to Date || The Actual Result is : Date " + currentDate + " is within the range of "+From_date+" and "+To_date);
				Extent_fail(driver,"Expected Result is : The Current date should be with in the range of from and to Date || The Actual Result is : Date " + currentDate + " is within the range of "+From_date+" and "+To_date, test, test1);
			}
		}
