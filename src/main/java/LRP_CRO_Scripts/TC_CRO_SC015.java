...
public class TC_CRO_SC015 extends Keywords {
	
	public void CRO_SC015(WebDriver driver, ExtentTest test, ExtentTest test1, String selected_dataset,Map<String, String> Excel_data,String url) throws Exception {

		String testCaseName="TC_CRO_SC015";
		...
		String ETA_Date_ValidationPopup = Excel_data.get("ETA_Date_ValidationPopup");
		String Add_ETA_Date = Excel_data.get("Add_ETA_Date");
		...
		waitForElement(driver, Terminal_ETA_Date_CRO);
		String ETA_Date_value=getText(driver, Terminal_ETA_Date_CRO);
		String Actual_ETA_Date=extractDate(ETA_Date_value);
		
		String uptoDate=addDaysToDate(Actual_ETA_Date,Add_ETA_Date);
		...
		waitForPopup(driver, popup_Message, ETA_Date_ValidationPopup);
		String Act_ETA_Popup=getText(driver, popup_Message);
		if(ETA_Date_ValidationPopup.equals(Act_ETA_Popup)) {
			System.out.println("Popup Matched || Expected : "+ETA_Date_ValidationPopup+" || Actual : "+Act_ETA_Popup);
			Extent_pass_New(driver, "Popup Matched || Expected : "+ETA_Date_ValidationPopup+" || Actual : "+Act_ETA_Popup, test, test1);
		}else {
			System.out.println("Popup Not Matched || Expected : "+ETA_Date_ValidationPopup+" || Actual : "+Act_ETA_Popup);
			Extent_fail(driver, "Popup Not Matched || Expected : "+ETA_Date_ValidationPopup+" || Actual : "+Act_ETA_Popup, test, test1);
		}
