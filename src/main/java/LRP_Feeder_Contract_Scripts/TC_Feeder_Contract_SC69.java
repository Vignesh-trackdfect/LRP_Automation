...
public class TC_Feeder_Contract_SC69 extends Keywords {

	public void Feeder_Contract_SC69(WebDriver driver, ExtentTest test, ExtentTest test1, String selected_dataset,
			Map<String, String> Excel_data, String url) {

		String testcase_Name = "TC_Feeder_Contract_SC69";
		...
		Step_Start(2, "Check the Reject Button present in the Feeder contract.", test, test1);

		waitForElement(driver, FC_Reject_Btn);

		if (isElementEnabled(driver, FC_Reject_Btn)) {
			System.out.println(
					"Expected result : Reject button is present in feeder contract || Actual result : Reject button is present in feeder contract ");
			Extent_pass_New(driver,
					"Expected result : Reject button is present in feeder contract  || Actual result : Reject button is present in feeder contract ",
					test, test1);
		} else {
		...
		Step_End(2, "2.Check the Reject Button present in the Feeder contract.", test, test1);
		...
		if (!isdisplayed(driver, FC_Reject_Btn)) {
			System.out.println(
					"Expected result : Reject button should not present in feeder contract || Actual result : Reject  button is not present in feeder contract ");
			Extent_pass_New(driver,
					"Expected result : Reject button should not present in feeder contract  || Actual result : Reject button is not present in feeder contract ",
					test, test1);
		} else {
			...
			System.out.println(
					"Expected result : Reject button should not present in feeder contract  || Actual result : Reject button is present in feeder contract ");
			Extent_fail(driver,
					"Expected result : Reject button should not present in feeder contract  || Actual result : Reject button is present in feeder contract ",
					test, test1);
		}
