...
public class TC_Haulage_Contract_SC23 extends Keywords{
	public void Haulage_Contract_SC23(WebDriver driver, ExtentTest test, ExtentTest test1, String selected_dataset,Map<String, String> Excel_data,String url) {
		String testcase_Name="TC_Haulage_Contract_SC23";
		...
		String AmountType = Excel_data.get("AmountType");
		...
		click(driver, HC_LDN_MTY_Select);
	      ...
	      formatLocatorClick(driver, DropDown_Select, HC_LDN_MTY_Value);
		      ...
		      waitForElement(driver, HC_PickUp_Term);
		            String pick_Up_Term_Act = getText(driver, HC_PickUp_Term);
		            
		        if(Pick_Up_Term_Value.equals(pick_Up_Term_Act)) {
		   			System.out.println("Matched || Expected Result is : "+ Pick_Up_Term_Value + " || Actual Result is : "+ pick_Up_Term_Act);
		   			Extent_pass_New(driver, "Matched || Expected Result is : "+ Pick_Up_Term_Value + " || Actual Result is : "+ pick_Up_Term_Act, test, test1);
		   		}else {
		   			System.out.println("Not Matched || Expected Result is : "+ Pick_Up_Term_Value + " || Actual Result is : "+ pick_Up_Term_Act);
		   			Extent_fail(driver, "Not Matched || Expected Result is : "+ Pick_Up_Term_Value + " || Actual Result is : "+ pick_Up_Term_Act, test, test1);
		   		}
		            

		waitForElement(driver, currency_SearchButton_HC);
		click(driver, currency_SearchButton_HC);
		
		twoColumnMultipleSearchWindow(driver, CurrencyCode_Header, condition, CurrencyCode);


		waitForElement(driver, amount_Type_Dropdown_HC);
		click(driver, amount_Type_Dropdown_HC);
		
		formatLocatorClick(driver, DropDown_Select, AmountType);
		
		 Step_Start(5, "Enter the value in the amount field .", test, test1);

		waitForElement(driver, amount_Textfield_HC);
		Newclear(driver, amount_Textfield_HC);
		waitForElement(driver, amount_Textfield_HC);
		sendKeys(driver, amount_Textfield_HC, Amount);
		
		waitForElement(driver, currency_Textfield_HC);
		click(driver, currency_Textfield_HC);
		
		waitForElement(driver, amount_Textfield_HC);
         String actualAmount = getAttribute(driver, amount_Textfield_HC,"value");	
         double actAmount=Double.parseDouble(actualAmount);
         double expAmount=Double.parseDouble(Amount);
         
         if(actAmount==expAmount) {
        	 System.out.println("Expected Amount :"+expAmount + "||  Actual Amount : "+actAmount);
        	 Extent_pass_New(driver, "Expected Amount :"+expAmount + " || Actual Amount : "+actAmount, test, test1);
         }else {
        	 System.out.println("Expected Amount :"+expAmount + " || Actual Amount : "+actAmount);
             Extent_fail(driver, "Expected Amount :"+expAmount + "||  Actual Amount : "+actAmount, test, test1);
         }
		
         Step_End(5, "Enter the value in the amount field .", test, test1);
