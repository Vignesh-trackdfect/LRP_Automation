...
public class TC_Vendor_Invoice_Registration_SC21 extends Keywords{
	
	public void Vendor_Invoice_Registration_SC21(WebDriver driver, ExtentTest test,ExtentTest test1,String selected_Excel_dataset,Map<String, String> Excel_data,String url) {
		
		String testCaseName="TC_Vendor_Invoice_Registration_SC21";
		//  Get data from excel sheet	
			...
			String status_Of_STC_RadioButton = Excel_data.get("Status_Of_STC_RadioButton");
...
//			Vendor Invoice Registration
		...
		// Vendor invoice number		
			...
			Step_End(6, "Enter the value up to 25 characters.", test, test1);
				...
				Step_Start(12, "In the Activity details, Click Activity type dropdown and select the activity type as a \"STC\"", test, test1);
			...
			Step_End(12, "In the Activity details, Click Activity type dropdown and select the activity type as a \"STC\"", test, test1);
			Step_Start(13, "Default system select the STC sum no.", test, test1);
			...
			if(actualStatus.contains(status_Of_STC_RadioButton)) {
				
				System.out.println("Matched || Expected : "+RadioButtonDescription+" Radio Button should be selected || Actual : "+RadioButtonDescription+" Radio Button is selected");
				Extent_pass_New(driver, "Matched || Expected : "+RadioButtonDescription+" Radio Button should be selected || Actual : "+RadioButtonDescription+" Radio Button is selected", test, test1);
			}
			else {
				System.out.println("Not Matched || Expected : "+RadioButtonDescription+" Radio Button should be selected || Actual : "+RadioButtonDescription+" Radio Button is not selected");
				Extent_fail(driver, "Not Matched || Expected : "+RadioButtonDescription+" Radio Button should be selected || Actual : "+RadioButtonDescription+" Radio Button is not selected", test, test1);
			}			
			...
			waitForElement(driver, Service_Search_Button_VIR);
			click(driver, Service_Search_Button_VIR);
			...
			waitForElement(driver, Vessel_Search_Button_VIR);
			click(driver, Vessel_Search_Button_VIR);
			...
			Step_Start(17, "Click the Voyage search and select the Required Voyage and bound", test, test1);

			waitForElement(driver, Voyage_Search_Button_VIR);
			click(driver, Voyage_Search_Button_VIR);
			
			twoColumnMultipleSearchWindow(driver, Voyage_Bound_Header, condition, Voyage_Bound_Input);

			Step_End(17, "Click the Voyage search and select the Required Voyage and bound", test, test1);
			Step_Start(18, "Click the Port search and select the required Port", test, test1);

			waitForElement(driver, Port_Search_Button_VIR);
			click(driver, Port_Search_Button_VIR);
			...
			Step_Start(19, "Click the Terminal and Select the Required terminal ", test, test1);

			waitForElement(driver, Terminal_Search_Button_VIR);
			click(driver, Terminal_Search_Button_VIR);
			
			twoColumnSearchWindow(driver, Terminal_Header, condition, Terminal_Code);

			Step_End(19, "Click the Terminal and Select the Required terminal ", test, test1);
