...
public class TC_Import_Documentation_SC061 extends Keywords{
	public void Import_Documentation_SC061(WebDriver driver, ExtentTest test, ExtentTest test1, String selected_dataset,Map<String, String> Excel_data,String url) {
		String testcase_Name="TC_Import_Documentation_SC061";
		...
		String Invoice_Saved_Popup = Excel_data.get("Invoice_Saved_Popup");
		String Invoice_Created_Popup = Excel_data.get("Invoice_Created_Popup");
		...
		String Do_You_Want_Delete_Import_Invoice = Excel_data.get("Do_You_Want_Delete_Import_Invoice");
		...
		String Charege_Code_Grid=String.format(ImportDocumentation_Locators.Charege_Code_Grid, Charge_Code);
		...
		String imp_Inv_Status=getAttribute(driver, import_Invoice_Checkbox_IMP_DOC, "class");
		if(imp_Inv_Status.contains(checkbox_Status)) {
			...
			scrollTop(driver);
			Step_Start(7, "click the Import invoice button in Import documenation", test, test1);
			waitForElement(driver, Invoice_Button);
			click(driver, Invoice_Button);
			Step_End(7, "click the Import invoice button in Import documenation", test, test1);
			Step_Start(8, "once clicked the import  invoice button ,system has open the import invoice module", test, test1);
			Step_Start(9, "Import invoice has saved ", test, test1);
			scrollTop(driver);
			waitForElement(driver,SaveButton_ToolBar);
			jsClick(driver, SaveButton_ToolBar);
			waitForPopup(driver, popup_Message, Invoice_Saved_Popup);
			String InvoiceSaved=getText(driver, popup_Message);
			if (Invoice_Saved_Popup.equals(InvoiceSaved)) {
				System.out.println("Matched || Expected Popup was : " + Invoice_Saved_Popup + " || Actual Popup was : " + InvoiceSaved);
				Extent_pass_New(driver, "Matched || Expected Popup was : " + Invoice_Saved_Popup + " || Actual Popup was : " + InvoiceSaved, test,test1);
				waitForElement(driver, popup_Message_Ok_Button);
				click(driver, popup_Message_Ok_Button);
			} else {
				System.out.println("NotMatched || Expected Popup was : " + Invoice_Saved_Popup + " || Actual Popup was : " + InvoiceSaved);
				Extent_fail(driver, "NotMatched || Expected Popup was : " + Invoice_Saved_Popup + " || Actual Popup was : " + InvoiceSaved,test, test1);
			}
			Step_End(9, "Import invoice has saved ", test, test1);
			Step_Start(10, "after invoice saved then retrive the bl in import documentation", test, test1);
			...
			Step_End(10, "after invoice saved then retrive the bl in import documentation", test, test1);
		}
		Step_Start(11, "click the edit button system will populate the  below validation 'Invoice has been created'", test, test1);
		scrollTop(driver);
		waitForElement(driver,Edit_Button_toolBar);
		click(driver, Edit_Button_toolBar);
		waitForPopup(driver, popup_Message, Invoice_Created_Popup);
		String InvoiceCreated=getText(driver, popup_Message);
		if (Invoice_Created_Popup.equals(InvoiceCreated)) {
			System.out.println("Matched || Expected Popup was : " + Invoice_Created_Popup + " || Actual Popup was : " + InvoiceCreated);
			Extent_pass_New(driver, "Matched || Expected Popup was : " + Invoice_Created_Popup + " || Actual Popup was : " + InvoiceCreated, test,test1);
			Step_Start(12, "Click the OK button", test, test1);
			waitForElement(driver, popup_Message_Ok_Button);
			click(driver, popup_Message_Ok_Button);
			Step_End(12, "Click the OK button", test, test1);
		} else {
			System.out.println("NotMatched || Expected Popup was : " + Invoice_Created_Popup + " || Actual Popup was : " + InvoiceCreated);
			Extent_fail(driver, "NotMatched || Expected Popup was : " + Invoice_Created_Popup + " || Actual Popup was : " + InvoiceCreated,test, test1);
		}
		Step_End(11, "click the edit button system will populate the  below validation 'Invoice has been created'", test, test1);
		Step_Start(13, "Go to the collect charges tab ", test, test1);
		waitForElement(driver,Collect_Charges_Tab);
		click(driver, Collect_Charges_Tab);
		Step_End(13, "Go to the collect charges tab ", test, test1);
		Step_Start(14, "all fields are disable  in import documentation collect charges tab.(Try Click charge code search Button if allows it is fail Case if not then PASS)", test, test1);
		waitForElement(driver,Charege_Code_Grid);
		doubleClick(driver, Charege_Code_Grid);
		
		if (!isClickable(driver, Charge_Code_Search_Button_IMP_DOC)) {
			System.out.println("Expected : While try to Click the Charge Code Search Field it Should not allows to click It Should Disabled || Actual :While try to Click the Charge Code Search Field it not allows to click It was Disabled");
			Extent_pass_New(driver, "Expected : While try to Click the Charge Code Search Field it Should not allows to click It Should Disabled || Actual :While try to Click the Charge Code Search Field it not allows to click It was Disabled", test,test1);
		} else {
			System.out.println("Expected : While try to Click the Charge Code Search Field it Should not allows to click It Should Disabled || Actual :While try to Click the Charge Code Search Field it allows to click It was not Disabled");
			Extent_fail(driver,"Expected : While try to Click the Charge Code Search Field it Should not allows to click It Should Disabled || Actual :While try to Click the Charge Code Search Field it allows to click It was not Disabled",test, test1);
		}
		Step_End(14, "all fields are disable  in import documentation collect charges tab.(Try Click charge code search Button if allows it is fail Case if not then PASS)", test, test1);
		Step_Start(15, "click the save in import documentation module", test, test1);
		scrollTop(driver);
		waitForElement(driver, SaveButton_ToolBar);
		jsClick(driver, SaveButton_ToolBar);
		Step_End(15, "click the save in import documentation module", test, test1);
		waitForElement(driver, popup_Message_Ok_Button);
		click(driver, popup_Message_Ok_Button);
		if(Do_You_Want_Delete_Import_Invoice.equalsIgnoreCase("Yes")) {
			if(imp_Inv_Status.contains(checkbox_Status)) {
				waitForElement(driver, Invoice_Button);
				click(driver, Invoice_Button);
			}else {
				waitForElement(driver,Import_Invoice_Module);
				click(driver, Import_Invoice_Module);
			}
		...
		if(Do_You_Want_Delete_Import_Doc.equalsIgnoreCase("Yes")) {
			Extent_cal(test, test1, "switch to the import documentation");
			waitForElement(driver,Import_Documentaion_Module_IMP_DOC);
			click(driver, Import_Documentaion_Module_IMP_DOC);
