...
public class TC_Discharge_Reconciliation_MD30A extends Keywords {
	public void Discharge_Reconciliation_MD30A(WebDriver driver, ExtentTest test, ExtentTest test1, String selected_dataset,Map<String, String> Excel_data,String url) {

		String testcase_Name = "TC_Discharge_Reconciliation_MD30A";
	...
	click(driver, Close_Current_tab);
		...
		Step_Start(15, "Filter Only RowS  Accourdingly POL", test, test1);
		waitForDisplay(driver, OLV_Grid_Filter_Btn);
		waitForElement(driver, OLV_Grid_Filter_Btn);
		jsClick(driver, OLV_Grid_Filter_Btn);
//		Jclick(driver, OLV_Grid_Filter_Btn);
	
		
		reArrangeAG_GridColumnsNew(driver, OLV_cocloum_click, Column_Header);
		
		sendKeys(driver, Re_arange_Bl_filter, Bl_Number);
		Step_End(15, "Filter Only RowS  Accourdingly POL", test, test1);
		
		Step_Start(16, "Verify that Make sure BLs matched these fields- POL", test, test1);
		
		List<WebElement> tableCells = driver.findElements(By.xpath("//div[@id='OPS-OPS_OpsListGrid_tbl']//div[@col-id='blpolL' and @role='gridcell']"));

		for (WebElement cell : tableCells) {
			String cellText = cell.getText().trim(); // Get text and trim whitespace
			if (Pol_Code.equals(cellText)) {

				System.out.println("Pass : The expected Pol Detials from Discharge Reconciliation is : "
						+ Pol_Code + "|| The actual Pol Detials from Operetion List View is : " + cellText);
				Extent_pass(
						driver, "Pass : The expected Pol Detials from Discharge Reconciliation is : "
								+ Pol_Code + " || The actual Pol Detials from Operetion List View is : " + cellText,
						test, test1);
			} else {
				System.out.println(" Fail : The expected Pol Detials from Discharge Reconciliation is : "
						+ Pol_Code + " || The actual Pol Detials from Operetion List View is : " + cellText);
				Extent_fail(
						driver, "Fail : The expected Pol Detials from Discharge Reconciliation is : "
								+ Pol_Code + " || The actual Pol Detials from Operetion List View is : " + cellText,
						test, test1);
		...
		Step_End(16, "Verify that Make sure BLs matched these fields- POL", test, test1);
