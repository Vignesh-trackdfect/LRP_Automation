...
public class TC_Booking_SC093 extends Keywords{
	public void Booking_SC093(WebDriver driver, ExtentTest test,ExtentTest test1,String selected_dataset,Map<String, String> Excel_data,String url) {

		//		Get data from excel sheet

		String test_Case_Name = "TC_Booking_SC093";
...
//		
	...
	tab(driver);
		...
		Step_Start(7, "In Booking charge tab while select base unit as Tues for 40s container then system calculated the base rate Quantity (2)correctly", test, test1);
		...
		safeclick(driver, rateref_toolbtn);
		waitForElement(driver, BKGcondition_Filter_Local);
		safeclick(driver, BKGcondition_Filter_Local);
		...
		Step_End(7, "In Booking charge tab while select base unit as Tues for 40s container then system calculated the base rate Quantity (2) correctly", test, test1);
