...
public class TC_Haulage_Contract_SC11 extends Keywords{
	public void Haulage_Contract_SC11(WebDriver driver, ExtentTest test, ExtentTest test1, String selected_dataset,Map<String, String> Excel_data,String url) {
		String testcase_Name="TC_Haulage_Contract_SC11";
		...
		Step_End(4, "It show the Haulage Contract input details page", test, test1);
	 	...
	 	Step_Start(5, "Click the Pickup type drop down and select the value.", test, test1);
		...
		click(driver, HC_drop_Type_Dropdown);
		  ...
		  formatLocatorClick(driver, DropDown_Select, Drop_Type_Value);
       	Step_End(5, "Click the Pickup type drop down and select the value.", test, test1);
     	Step_Start(6, "Based on the Pickuptype ,we select the Pickup terminal.", test, test1);
     	waitForElement(driver, HC_Pickup_Type_Dropdown);
		
         String pickup_Type_Value_Actual = getText(driver, HC_drop_Type_Dropdown);	
         
         if(pickup_Type_Value_Actual.contains(Drop_Type_Value)) {
        	 System.out.println("Expected Value taken in test data for selecting Droup type :"+Drop_Type_Value + "||  Actual Value taken in  Droup Terminal: "+pickup_Type_Value_Actual);
        	 Extent_pass_New(driver, "Expected Value taken in test data for selecting Droup type:"+Drop_Type_Value + "||  Actual Value taken in  Droup Terminal  : "+pickup_Type_Value_Actual, test, test1);
         }else {
        	 System.out.println("Expected Value taken in test data for selecting pickup Droup:"+Drop_Type_Value + "||  Actual Value taken in  Droup Terminal  : "+pickup_Type_Value_Actual);
             Extent_fail(driver, "Expected Value taken in test data for selecting pickup Droup :"+Drop_Type_Value + " || Actual Value taken in  Droup Terminal: "+pickup_Type_Value_Actual, test, test1);
         }
		
         Step_End(6, ".Based on the Pickuptype ,we select the Pickup terminal..", test, test1);
