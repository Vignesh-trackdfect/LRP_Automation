...
public class TC_Cost_Activity_Report_TS027 extends Keywords {
	...
	public void Cost_Activity_Report_TS027(WebDriver driver, ExtentTest test, ExtentTest test1, String selected_dataset,Map<String, String> Excel_data,String url)
			throws Exception {
		...
		if(isdisplayed(driver, App_Expense_Table_Rows)) {
			...
			List<WebElement> Invoice_Column = listOfElements(driver, InvoiceNumber);
			...
			for (int i = 0; i < Invoice_Column.size(); i++) {
				String act_InvNumber = Invoice_Column.get(i).getText();
				if(!act_InvNumber.equals("")) {
					String Act_Quantity=inv_Numbers.get(i).getText();;
					Actual_Quantity_Numbers.add(Act_Quantity);
					String act_RefNumber = ref_Numbers.get(i).getText();
					Actual_Ref_Numbers.add(act_RefNumber);
					Act_Quantity_.put(act_RefNumber, Act_Quantity);
