...
public class TC_Transportation_Dispatch_Notice_SC_TDN_26 extends Keywords{
	...
	public void Transportation_Dispatch_Notice_SC_TDN_26(WebDriver driver,ExtentTest test,ExtentTest test1,String selected_dataset,Map<String, String> Excel_data,String url) {
		...
		String Mode_Of_Transport =Excel_data.get("Mode_Of_Transport");
		...
		String warning_Message_Expected =Excel_data.get("Warning_Message");
		...
		String container_Value =Excel_data.get("container_Value");
		...
		waitForPopup(driver, popup_Message, warning_Message_Expected);   
		String actual_Warn_Message = getText(driver, popup_Message);
		...
		if(warning_Message_Expected.equalsIgnoreCase(actual_Warn_Message)) {
			...
			Extent_pass_New(driver,"Matched ||Expected Popup Message is : "+warning_Message_Expected+ " || Actual  Popup Message is : "+ actual_Warn_Message, test, test1);
			System.out.println("clicking the yes button"+"****Draft TDN Container leg is removed.****.");
			Extent_pass_New(driver, "clicking the yes button"+"****Draft TDN Container leg is removed.****.", test, test1);
		...
		waitForPopup(driver, popup_Message, TDN_Updated_popup);   
		String actual_TDN_Updated_Popup=getText(driver, popup_Message);
		if(actual_TDN_Updated_Popup.equals(TDN_Updated_popup)) {
			Extent_pass_New(driver, "Matched || Expected Pop up : "+TDN_Updated_popup +" || Actual Pop up : "+actual_TDN_Updated_Popup, test, test1);
