...
public class TC_Haulage_Contract_SC18 extends Keywords{
	public void Haulage_Contract_SC18(WebDriver driver, ExtentTest test, ExtentTest test1, String selected_dataset,Map<String, String> Excel_data,String url) {
		String testcase_Name="TC_Haulage_Contract_SC18";
		...
		Step_End(4, "It show the Haulage Contract input details page", test, test1);
	 	...
	 	Step_Start(5, ".Click the Trip types and select the value from the Drop down..", test, test1);
		...
		waitForElement(driver, HC_drop_Type_Dropdown);
         ...
         String HC_H_Mode_Actual = getText(driver, HC_Trip_Types);	
         
         if(HC_H_Mode_Actual.contains(Trip_Type_Value)) {
        	 System.out.println("Matched ||Expected Value taken in test data for selecting Trip Types :"+ Trip_Type_Value + "|| Actual Value taken in  Trip Types in Dropdown: "+ HC_H_Mode_Actual);
        	 Extent_pass_New(driver, "Matched ||Expected Value taken in test data for selecting Trip Types:"+ Trip_Type_Value + " ||Actual Value taken in Trip Types in Dropdown  : "+ HC_H_Mode_Actual, test, test1);
         }else {
        	 System.out.println("NotMatched ||Expected Value taken in test data for selecting Trip Types:"+ Trip_Type_Value + " ||Actual Value taken in  Trip Types in Dropdown : "+ HC_H_Mode_Actual);
             Extent_fail(driver, "NotMatched ||Expected Value taken in test data for selecting Trip Types :"+ Trip_Type_Value + " || Actual Value taken in  Trip Types in Dropdown: "+ HC_H_Mode_Actual, test, test1);
         }
		
     	
 	 	Step_End(5, "..Click the Trip types and select the value from the Drop down..", test, test1);
