...
public class TC_Vendor_Invoice_Registration_SC38  extends Keywords{
	
	public void Vendor_Invoice_Registration_SC38(WebDriver driver, ExtentTest test, ExtentTest test1,String selected_dataset,Map<String, String> Excel_data,String url) {

		String testcase_Name="TC_Vendor_Invoice_Registration_SC38";
		...
		String Bunker_Delivery_Note_Module = Excel_data.get("Bunker_Delivery_Note_Module");
		String Staus_Active = Excel_data.get("Staus_Active");
		...
		Step_Start(1, "Enter the module name as \"\"Vendor Invoice Registration\"\" in the search.\r\n"
				+ "", test, test1);
		moduleNavigate(driver, Module_Name);
		Step_End(1, "Enter the module name as \"\"Vendor Invoice Registration\"\" in the search.\r\n"
				+ "", test, test1);
	...
	Step_End(5, "Click the Vendor invoice field", test, test1);
		...
		Step_Start(13, "Default system select the BRQ sum no.", test, test1);
		...
		Step_End(13, "Default system select the BRQ sum no.", test, test1);
		Step_Start(14, "Click the BRQ sum no search and select the Required no", test, test1);
		...
		Step_End(14, "Click the BRQ sum no search and select the Required no", test, test1);
...
waitForDisplay(driver, FDR_Grid1);
		...
		moduleNavigate(driver, Bunker_Delivery_Note_Module);
		...
		waitForElement(driver, BBQ_Confirm_Button);
		String Status_confirm = getAttribute(driver, BBQ_Confirm_Button, "class");
		System.out.println(Status_confirm);
		
		
		

		if (Status_confirm.contains(Staus_Active)){

			System.out.println(
					"Expected result is : Reference number should be active and confirmed status  || Actual result is :  Reference number is active and confirmed status");
			Extent_pass_New(driver,
					"Expected result is :  Reference number should be active and confirmed status || Actual result is : Reference number is active and confirmed status",
					test, test1);
		} else {
			...
			System.out.println(
					"Expected result is :  Reference number should be active and confirmed status || Actual result is : Reference number is not active and confirmed status");
			Extent_fail(driver,
					"Expected result is :  Reference number should be active and confirmed status || Actual result is : Reference number is not active and confirmed status",
					test, test1);
