...
public class TC_Contract_Management_System_SC070 extends Keywords{

public void Contract_Management_System_SC70(WebDriver driver, ExtentTest test,ExtentTest test1, String selected_dataset,Map<String, String> Excel_data,String url) {
		
		String testCaseName = "TC_Contract_Management_System_SC070";
    ...
    Step_Start(1, "select N-N-FMC check box", test, test1);
     ...
     Step_Start(2, "select containerized check box", test, test1);
		...
		if (BackRoundColor.equals(color1)) {
			System.out.println("Excepted ||  Imported port pair lanes with no route in the master data should be highlighted in: " + BackRoundColor + " || Actual Imported Excel port pair lanes with no route is Highlighted in." + color1);
			Extent_pass(driver, "Excepted||  Imported port pair lanes with no route in the master data should be highlighted in "  + BackRoundColor+ " || Actual Imported  Excel port pair lanes with no route is Highlighted in. " + color1, test, test1);
			temp = false;
			break;
		} 
		}
		...
		if(temp) {
			System.out.println("UnExpected || Imported port pair lanes with no route in the master data should be highlighted in: " + BackRoundColor + " || Actual imported Excel file does not contain any lanes without a route.");
			Extent_fail(driver,"UnExpected || Imported port pair lanes with no route in the master data should be highlighted in: " + BackRoundColor + " || Actual imported Excel file does not contain any lanes without a route." , test, test1);
		}
