...
public class TC_Vendor_Contracts_SC004 extends Keywords {

	public void Vendor_Contracts_SC004(WebDriver driver, ExtentTest test, ExtentTest test1,
			String selected_Excel_dataset, Map<String, String> Excel_data, String url) {

		String testCaseName = "TC_Vendor_Contracts_SC004";
		...
		String GlobalSearch_Type_Predictable = Excel_data.get("GlobalSearch_Type_Predictable");
		String GlobalSearch_Condition_Predictable = Excel_data.get("GlobalSearch_Condition_Predictable");
		String Predictable_Activity_ID = Excel_data.get("Predictable_Activity_ID");
		...
		Step_Start(7, "Click on 'Yes' to get the activities not from predictable activity behaviour.   ", test, test1);
		...
		Step_End(7, "Click on 'Yes' to get the activities not from predictable activity behaviour.   ", test, test1);

		Step_Start(8, "Select the required predictable behaviour id .  ", test, test1);
		globalValueSearchWindow(driver, GlobalSearch_Condition_Predictable, GlobalSearch_Type_Predictable,
				Predictable_Activity_ID, "", "", "", "");
		Step_End(8, "Select the required predictable behaviour id .  ", test, test1);

		Step_Start(9, "Select the main activity which is present and Click the 'Enter edit input fields' option.   ",
				test, test1);
		Actions act = new Actions(driver);
		act.keyDown(Keys.CONTROL).build().perform();

		List<WebElement> allrows_VC = listOfElements(driver, vc_rows);
		for (WebElement Ele : allrows_VC) {
		...
		Step_End(9, "Select the main activity which is present and Click the 'Enter edit input fields' option.   ",
				test, test1);
		...
		Step_Start(12, "Select the currency.     ", test, test1);
		...
		Step_End(12, "Select the currency.     ", test, test1);
		...
		Step_Start(14,
				"Select the main activity in the grid and right click and select 'add main activity' option if required    ",
				test, test1);
		Step_Start(15, "Select the required main activity and click 'ok' option.", test, test1);
		Step_Start(16,
				"Select the main activity added and right click it and click on 'add item' option and select the required activities.",
				test, test1);
		...
		Step_End(14,
				"Select the main activity in the grid and right click and select 'add main activity' option if required    ",
				test, test1);
		Step_End(15, "Select the required main activity and click 'ok' option.", test, test1);
		Step_End(16,
				"Select the main activity added and right click it and click on 'add item' option and select the required activities.",
				test, test1);

		Step_Start(17, "Double click on the amount column an give the required amount for the activities", test, test1);
		...
		Step_End(17, "Double click on the amount column an give the required amount for the activities", test, test1);

		Step_Start(18, "Select the required activity and change the charge type as 'Sub to Formula' for the activity.",
				test, test1);
		...
		Step_End(23, "Click ok Option.  ", test, test1);
		...
		if (Vendor_Num_Act == null || Vendor_Num_Act.isEmpty() || Vendor_Num_Act.equals("<<New>>")) {
			System.out.println(
					"Expected : Vendor Contract Number should be generated ||  Actual : Vendor Contract Number is not generated,  Contract Number :  "
							+ Vendor_Num_Act);
			Extent_fail(driver,
					"Expected : Vendor Contract Number should be generated ||  Actual : Vendor Contract Number is not generated,  Contract Number :  "
							+ Vendor_Num_Act,
			...
			System.out.println(
					"Expected : Vendor Contract Number should be generated ||  Actual : Vendor Contract Number is generated,  Contract Number :  "
							+ Vendor_Num_Act);
