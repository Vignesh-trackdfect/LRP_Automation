...
public class TC_Cost_Activity_Report_TS055 extends Keywords {
	...
	public void Cost_Activity_Report_TS055(WebDriver driver, ExtentTest test, ExtentTest test1, String selected_dataset,Map<String, String> Excel_data,String url)
			throws Exception {
		...
		String Activity_Names = Excel_data.get("Activity_Names");
		...
		String Table_Headers_ExpenseTable = Excel_data.get("Table_Headers_ExpenseTable");
		...
		formatLocatorClick(driver, DropDown_Select, GivenContract_Type);

		waitForElement(driver, Select_Mode);
		click(driver, Select_Mode);
		formatLocatorClick(driver, DropDown_Select, GivenMode);
		...
		List<String> Table_Activities_Values=splitAndExpand(Activity_Names);
		List<String> Un_Predictable_Activities=splitAndExpand(Activity_Names);
		...
		for(int i=0;i<Table_Activities_Values.size();i++) {
			...
			click(driver, quantity_Input_CAR);
		...
		Step_Start(14, "Click on the tool bar save button", test, test1);
		waitForElement(driver, SaveButton_ToolBar);
		click(driver, SaveButton_ToolBar);
		Step_End(14, "Click on the tool bar save button", test, test1);

		Step_Start(15, "Check whether the system shows the information message as \"Container Activity Report Saved\". Click on the ok option", test, test1);

		waitForPopup(driver, popup_Message, CAR_Saved_Popup);
		String Act_Popup = getText(driver, popup_Message);

		if (Act_Popup.equals(CAR_Saved_Popup)) {
			System.out.println("Popup  Matched || Expected : " + CAR_Saved_Popup + " || Actual : "+ Act_Popup);
			Extent_pass_New(driver,	"Popup  Matched || Expected : " + CAR_Saved_Popup + " || Actual : "+ Act_Popup,test, test1);
		} else {
			System.out.println("Popup Not Matched || Expected : " + CAR_Saved_Popup + " || Actual : "+ Act_Popup);
			Extent_fail(driver,	"Popup Not Matched || Expected : " + CAR_Saved_Popup + " || Actual : "+ Act_Popup,test, test1);
		}

		waitForElement(driver, popup_Message_Ok_Button);
		click(driver, popup_Message_Ok_Button);
		Step_End(15, "Check whether the system shows the information message as \"Container Activity Report Saved\". Click on the ok option", test, test1);

		Step_Start(16, "Click on the expense report option. ", test, test1);
		...
		Step_End(16, "Click on the expense report option. ", test, test1);

		Step_Start(17, "Ensure that the same quantities with the same cost is shown for the activities based on the quantity and cost for the activities from the grid.", test, test1);
		...
		List<String> ExpensiveTable_Headers=splitAndExpand(Table_Headers_ExpenseTable);
		...
		for(int k=0;k<Table_Activities_Values.size();k++) {
			String Actvity=Table_Activities_Values.get(k);
			String Head=Activity_Head_Values.get(k);
			String EqpType=Equipement_Types.get(k);
			String EqpValues=Equipement_Status.get(k);
			
			AG_Grid_FilterInputs(driver,"Activity Name",Actvity);
			AG_Grid_FilterInputs(driver,"Eqp Type",EqpType);
			AG_Grid_FilterInputs(driver,"Eqp Status",EqpValues);
			AG_Grid_FilterInputs(driver,"Head",Head);			
			
			if(!isdisplayed(driver, Open_Expense_Table_Rows)) {
				System.out.println("Expected : Quantity changed activity ("+Actvity+") details should display in expense || Actual : Quantity Changed Activity ("+Actvity+")  values not updated in the expense table" );
				Extent_fail(driver, "Expected : Quantity changed activity ("+Actvity+") details should display in expense || Actual : Quantity Changed Activity ("+Actvity+")  values not updated in the expense table" , test, test1);
			}
		}
		
		if(expense) {
			System.out.println("Expected : Quantity changed activity details should display in expense || Actual : Quantity Changed Activity values updated in the expense table" );
			Extent_pass_New(driver, "Expected : Quantity changed activity details should display in expense || Actual : Quantity Changed Activity values updated in the expense table" , test, test1);
		}
		Step_End(17, "Ensure that the same quantities with the same cost is shown for the activities based on the quantity and cost for the activities from the grid.", test, test1);
