...
public class TC_Payable_Payment_PP07 extends Keywords{
	
	public void Payable_Payment_PP07(WebDriver driver, ExtentTest test, ExtentTest test1,String selected_dataset,Map<String, String> Excel_data,String url) {
		
		String testCaseName="TC_Payable_Payment_PP07";
		...
		String Account_Head_Header =Excel_data.get("Account_Head_Header");
		...
		Step_End(3, "Open period will get automatically populated based on receipt date", test, test1);
			...
			if (payment_Date.equals(actual_Payment_Date)) {
				System.out.println("Matched || Payment Date Automatically Selected Based On Month || Expected Current Date : "+payment_Date+" || The Actual Payment Date Selected Based on Month : "+actual_Payment_Date);
				Extent_pass_New(driver,"Matched || Payment Date Automatically Selected Based On Month || Expected Current Date : "+payment_Date+" || The Actual Payment Date Selected Based on Month : "+actual_Payment_Date, test, test1);
			} else {
				System.out.println("Not Matched || Payment Date Automatically Not Selected Based On Month || Expected Current Date : "+payment_Date+" || The Actual Payment Date : "+actual_Payment_Date);
				Extent_fail(driver,"Not Matched || Payment Date Automatically Not Selected Based On Month || Expected Current Date : "+payment_Date+" || The Actual Payment Date : "+actual_Payment_Date, test, test1);
			}
			Step_End(2, "Payment date will be automatically selected based on month", test, test1);
		...
		twoColumnSearchWindow(driver, Account_Head_Header, dropdownCondition,accountHead_BankCharges );
		...
		Step_Start(8, "Click on Add button without entering the value in the \"Bank Charges\"", test, test1);

		waitForElement(driver, add_Button_Cash_Bank_Details_PPT);
		click(driver, add_Button_Cash_Bank_Details_PPT);
		
		Step_End(8, "Click on Add button without entering the value in the \"Bank Charges\"", test, test1);
		Step_Start(9, "system should show pop up validation as \"Enter the Bank charges A/C Name\".", test, test1);
		...
		Step_End(9, "system should show pop up validation as \"Enter the Bank charges A/C Name\".", test, test1);
