...
public class TC_Contract_Management_System_SC229 extends Keywords {

	public void Contract_Management_System_SC229(WebDriver driver, ExtentTest test, ExtentTest test1,

			String selected_dataset,Map<String, String> Excel_data,String url) {

		String testCaseName = "TC_Contract_Management_System_SC229";
		...
		String DG_Details_Missing_popup1 = Excel_data.get("DG_Details_Missing_popup1");
		...
		Step_Start(6, "Import the port pair lanes with DG YES and and incorrect DG class", test, test1);
		...
		scrollBottom(driver);
		waitForDisplay(driver, CMS_Verify_Duplicate_Table_Page);
		if (isdisplayed(driver, CMS_Verify_Duplicate_Table_Page)) {
			...
			String DuplicateReportTable = TableDataForReport(driver, tableData, "INVALID UNNO details",
					bookingHeaders);
			
			
		    String DG_Class_Exceed_popup_1 = getText(driver, CMS_DG_Missing_window_Row_1);
			System.out.println("DG_Class_Exceed_popup is"+DG_Class_Exceed_popup_1);
			
			  String DG_Class_Exceed_popup_2 = getText(driver, CMS_DG_Missing_window_Row_2);
				System.out.println("DG_Class_Exceed_popup is"+DG_Class_Exceed_popup_2);
				
					if (DG_Details_Missing_popup1.equals(DG_Class_Exceed_popup_1)){
						System.out.println("After Clicking ON upload Button , If we try invalid UNNO  details, we will get this message "
								+ DuplicateReportTable);
						Extent_pass(driver, "After Clicking ON upload Button ,If we try invalid  UNNO  details, we will get this message "
					...
					Extent_fail(driver, "After Clicking ON upload Button ,If we try invalid  UNNO  details, wew will  not get this message  ", test, test1);
				}
				}
					else {
						
						Extent_fail(driver, "After Clicking ON upload Button , If we try invalid  UNNO  details, wew will  not get this Window ", test, test1);
					}
		Step_End(6, "Import the port pair lanes with DG YES and and incorrect DG class", test, test1);
