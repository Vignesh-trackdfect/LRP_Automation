...
public class TC_Journal_Upload_JU03 extends Keywords{
	public void Journal_Upload_JU03(WebDriver driver, ExtentTest test, ExtentTest test1,String selected_dataset,Map<String, String> Excel_data,String url) throws EncryptedDocumentException, InvalidFormatException, IOException{
		String testcase_Name="TC_Journal_Upload_JU03";
		...
		String Not_Mapped_Remarks = Excel_data.get("Not_Mapped_Remarks"); 
		...
		Step_Start(3, "Click select Excel file option and popup window will appear , now select the excel file which having  invalid data in Nominal code  and click Ok button", test, test1);
		...
		Step_End(3, "Click select Excel file option and popup window will appear , now select the excel file which having  invalid data in Nominal code  and click Ok button", test, test1);
		Step_Start(4, "Excel uploaded and showing batch details in that journal upload screen", test, test1);
		waitForElement(driver, JU_Excel_Upload_Button);
		click(driver, JU_Excel_Upload_Button);
		Step_End(4, "Excel uploaded and showing batch details in that journal upload screen", test, test1);
		Step_Start(5, "Select the batch that you want to verify", test, test1);
		waitForElement(driver, JU_Select_All_Check_Box);
		click(driver, JU_Select_All_Check_Box);
		Step_End(5, "Select the batch that you want to verify", test, test1);
		Step_Start(6, "Click verify button .Now popup message will appear as 'batch verified'", test, test1);
		...
		if (Verified_Popup_Msg.trim().equals(actual_Popup_Msg.trim())) {
			...
			Step_End(6, "Click verify button .Now popup message will appear as 'batch verified'", test, test1);
			Step_Start(7, "Click ok . ", test, test1);
			click(driver, popup_Message_Ok_Button);
			Step_End(7, "Click ok . ", test, test1);
			Step_Start(8, "Check remarks column . It should showing as nominal code not mapped as 'Chart of Accounts'", test, test1);
		} else {
		...
		for(int i=0;i<remarks_value.size();i++) {
			if (Not_Mapped_Remarks.contains(remarks_value.get(i))) {
				Extent_pass_New(driver, "Matched || Expected Remark is : " + Not_Mapped_Remarks + " || Actual Remarks in Journal Module : " + remarks_value.get(i), test,test1);
				System.out.println("Matched || Expected Remark is : " + Not_Mapped_Remarks + " || Actual Remarks in Journal Module : " + remarks_value.get(i));
			} else {
				System.out.println("Not Matched || Expected Remark is : " + Not_Mapped_Remarks + " || Actual Remarks in Journal Module : " + remarks_value.get(i));
				Extent_fail(driver,"Not Matched || Expected Remark is : " + Not_Mapped_Remarks + " || Actual Remarks in Journal Module : " + remarks_value.get(i), test,test1);
			}
		}
		Step_End(8, "Check remarks column . It should showing as nominal code not mapped as 'Chart of Accounts'", test, test1);
