...
public class TC_Contract_Management_System_SC207 extends Keywords {

	public void Contract_Management_System_SC207(WebDriver driver, ExtentTest test, ExtentTest test1,

			String selected_dataset, Map<String, String> Excel_data, String url) {

		String testCaseName = "TC_Contract_Management_System_SC207";
		...
		String MainSheetName1 = Excel_data.get("MainSheetName1");
		...
		String MainSheet_option1 = String.format(SelectMainSheet, MainSheetName1);
		...
		if (gridcount.equals(ExcelrowCount)) {
			...
			System.out.println("Some of the row values are not imported in the portpair grid || Expected rows : "
					+ ExcelrowCount + " Actual rows : " + gridcount);
			Extent_fail(driver, "Some of the row values are not imported in the portpair grid || Expected rows : "
					+ ExcelrowCount + " Actual rows : " + gridcount, test, test1);
		}
		...
		Step_Start(10,"click on new button and enter the mandatory details.", test, test1);
		...
		Step_End(10, "click on new button and enter the mandatory details.", test, test1);

		
		Step_Start(11, "click on the import button.", test, test1);
	

		waitForElement(driver, ImportExcel);
		click(driver, ImportExcel);
		
		Step_End(11, "click on the import button.", test, test1);
		
		Step_Start(12, "and in the export download the import sheet will be available.", test, test1);
		
		Step_Start(13, "select the import sheet in import option and upload.", test, test1);

		uploadfile(driver, ImportInput, filePath);

		waitForElement(driver, SelectSheet_dropdown1);
		click(driver, SelectSheet_dropdown1);

		waitForElement(driver, MainSheet_option1);
		click(driver, MainSheet_option1);
		...
		if (Contract_saved.equals(saved_popup1)) {
			Extent_pass(driver, "Matched || Expected popup value was : " + Contract_saved
					+ " || Actual popup value was is display : " + saved_popup1, test, test1);
			System.out.println("Matched || Expected popup value was : " + Contract_saved
					+ " || Actual popup value was is display: " + saved_popup1);
		} else {
			System.out.println("Not Matched || Expected popup value was : " + Contract_saved
					+ " || Actual popup value was not display: " + saved_popup1);
			Extent_fail(driver, "Not Matched || Expected popup value was : " + Contract_saved
					+ " || Actual popup value was not display: " + saved_popup1, test, test1);
