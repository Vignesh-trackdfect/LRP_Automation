...
public class TC_Endpoint_Configuration_TS15 extends Keywords {

	public void Endpoint_Configuration_TS15(WebDriver driver, ExtentTest test, ExtentTest test1,
			String selected_dataset, Map<String, String> Excel_data, String url) {

		String testcase_Name = "TC_Endpoint_Configuration_TS15";
		...
		clear(driver, AS2_ProfileID);
		sendKeys(driver, AS2_ProfileID, ProfileID);

		Step_End(4, "Change the profile id", test, test1);

		Step_Start(5, "Change the AS2 id", test, test1);

		waitForElement(driver, AS2_AS2Id);
		clear(driver, AS2_AS2Id);
		sendKeys(driver, AS2_AS2Id, AS2_Id);

		Step_End(5, "Change the AS2 id", test, test1);

		Step_Start(6, "Change the password", test, test1);

		waitForElement(driver, AS2_Password);
		clear(driver, AS2_Password);
		sendKeys(driver, AS2_Password, AS2_PasswordValue);

		Step_End(6, "Change the password", test, test1);

		Step_Start(7, "Change the alias", test, test1);

		waitForElement(driver, AS2_Alias);
		clear(driver, AS2_Alias);
		sendKeys(driver, AS2_Alias, Alias_Value);

		Step_End(7, "Change the alias", test, test1);

		Step_Start(8, "Change the keystore", test, test1);

		waitForElement(driver, AS2_Keystore);
		clear(driver, AS2_Keystore);
		sendKeys(driver, AS2_Keystore, KeystoreValue);

		Step_End(8, "Change the keystore", test, test1);

		Step_Start(9, "Change the gateway URL", test, test1);

		waitForElement(driver, AS2_GatewayURL);
		clear(driver, AS2_GatewayURL);
		sendKeys(driver, AS2_GatewayURL, Gateway_URL);

		Step_End(9, "Change the gateway URL", test, test1);

		Step_Start(10, "Click the save button now the popup should show like EDI Transmission profiler details updated",
				test, test1);
		...
		if (isdisplayed(driver, AS2_Row1)) {
			...
			System.out.println(
					"Expected result : The given details should be updated in the grid || Actual result : The given details are updated in the grid");
			...
			System.out.println(
					"Expected result : The given details should be updated in the grid || Actual result : The given details are not updated in the grid");
		...
		Step_End(10, "Click the save button now the popup should show like EDI Transmission profiler details updated",
				test, test1);
