...
public class TC_Feeder_Contract_SC43 extends Keywords {

	public void Feeder_Contract_SC43(WebDriver driver, ExtentTest test, ExtentTest test1, String selected_dataset,
			Map<String, String> Excel_data, String url) {

		String testcase_Name = "TC_Feeder_Contract_SC43";
		...
		String Grid_Chargecode_Header = Excel_data.get("Grid_Chargecode_Header");
		...
		for (Map<String, String> row : Surcharge_Grid) {
			String chargeCode = row.get(Grid_Chargecode_Header); // Use the exact column name as it appears in the table
			if (chargeCode != null && !chargeCode.isEmpty()) {
				chargeCodeList.add(chargeCode);
			}
		}

		List<String> Surcharge_Codes = splitAndExpand(Surcharge_Value);

		for (String Surcharge_Code : Surcharge_Codes) {

			if (chargeCodeList.contains(Surcharge_Code)) {

				System.out.println("Expected result : The Selected surcharge code " + Surcharge_Code
						+ " should be added in the grid || Actual result : The Selected surcharge code "
						+ Surcharge_Code + " was added in the grid");
				Extent_pass_New(driver,
						"Expected result : The Selected surcharge code " + Surcharge_Code
								+ " should be added in the grid || Actual result : The Selected surcharge code "
								+ Surcharge_Code + " was added in the grid",
						test, test1);
				...
				System.out.println("Expected result : The Selected surcharge code " + Surcharge_Code
						+ " should be added in the grid || Actual result : The Selected surcharge code "
						+ Surcharge_Code + " was not added in the grid");
				Extent_fail(driver,
						"Expected result : The Selected surcharge code " + Surcharge_Code
								+ " should be added in the grid || Actual result : The Selected surcharge code "
								+ Surcharge_Code + " was not added in the grid",
