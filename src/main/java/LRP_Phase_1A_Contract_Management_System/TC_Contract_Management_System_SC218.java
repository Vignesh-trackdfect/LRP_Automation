...
public class TC_Contract_Management_System_SC218 extends Keywords {
	public void Contract_Management_System_SC218(WebDriver driver, ExtentTest test, ExtentTest test1,
			String selected_dataset, Map<String, String> Excel_data, String url) {

		String testCaseName = "TC_Contract_Management_System_SC218";
		...
		String OOG_flag_popup = Excel_data.get("OOG_flag_popup");
		...
		Step_Start(6, "Import the port pair lanes with OOG NO and with  the OOG details in OOG  tab.", test, test1);
		...
		if (isdisplayed(driver, CMS_Verify_Duplicate_Table_Page)) {
			...
			List<String> OOG_flag = splitAndExpand(CMS_missing_table_Header, ",");

			String Oogflag_Table = TableDataForReport(driver, tableData, "OOG FLAG details", OOG_flag);

			String OOG_missing_popup = getText(driver, CMS_DG_Missing_window_Row);
			System.out.println("OOG flag popup is " + OOG_missing_popup);

			if (OOG_flag_popup.equals(OOG_missing_popup)) {

				Extent_pass(driver,
						"After Clicking ON upload Button , OOG missing details should be displayed  " + Oogflag_Table,
						test, test1);
			} else {
				Extent_fail(driver,
						"After Clicking ON upload Button , OOG missing details should be displayed  " + Oogflag_Table,
			...
			Extent_fail(driver,
					"After Clicking ON upload Button , By Importing the port pair lanes with OOG NO and with  the OOG details in OOG  tab. , OOG flag details is not displayed  ",
					test, test1);
		}

		Step_End(6, "Import the port pair lanes with OOG NO and with  the OOG details in OOG  tab.", test, test1);
