...
public class TC_Haulage_Contract_SC26 extends Keywords{
	public void Haulage_Contract_SC26(WebDriver driver, ExtentTest test, ExtentTest test1, String selected_dataset,Map<String, String> Excel_data,String url) {
		String testcase_Name="TC_Haulage_Contract_SC26";
		...
		Step_Start(1, "Click the others in the Payat details and select the Payment loaction using search.", test, test1);

		waitForElement(driver, Payable_At_Others);
		click(driver, Payable_At_Others);
		
		waitForElement(driver, Location_Search_Button);
		click(driver, Location_Search_Button);
		twoColumnMultipleSearchWindow(driver, Pickup_Location_Header, condition, Location_Value);
		...
		if(Location_Value.equals(Act_Pickup_Location)) {
			System.out.println("Matched || Expected Result is : "+Location_Value + " || Actual Result is : "+Act_Pickup_Location  );
			Extent_pass_New(driver, "Matched || Expected Result is : "+Location_Value + " || Actual Result is : "+Act_Pickup_Location  , test, test1);
		}else {
			System.out.println("Matched || Expected Result is : "+Location_Value + " || Actual Result is : "+Act_Pickup_Location  );
			Extent_fail(driver, "Matched || Expected Result is : "+Location_Value + " || Actual Result is : "+Act_Pickup_Location  , test, test1);
		}
		
		
		Step_End(1, "Click the others in the Payat details and select the Payment loaction using search.", test, test1);
