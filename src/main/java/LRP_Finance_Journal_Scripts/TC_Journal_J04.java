...
public class TC_Journal_J04 extends Keywords {

	public void Journal_J04(WebDriver driver, ExtentTest test, ExtentTest test1,String selected_dataset,Map<String, String> Excel_data,String url) {
		String testcase_Name="TC_Journal_J04";
		...
		String version_Number_Select = Excel_data.get("version_Number_Select");
		String colour_After_Alter = Excel_data.get("colour_After_Alter");
		...
		Step_Start(1, "Retrieve the record", test, test1);
		waitForElement(driver, SearchButton_Toolbar);
		click(driver, SearchButton_Toolbar);
		globalValueSearchWindow(driver, Dropdown_Condition, Global_Search_Option1_Journal, Trans_Number_Input, "", "", "", "");
		Step_End(1, "Retrieve the record", test, test1);
		Step_Start(4, "Click edit on toolbar and edit some data's and click save", test, test1);
		waitForElement(driver, Edit_Button_toolBar);
		click(driver, Edit_Button_toolBar);
		Step_End(4, "Click edit on toolbar and edit some data's and click save", test, test1);
		...
		click(driver, popup_Message_Ok_Button);
		Step_Start(2, "Click audit button", test, test1);
		waitForElement(driver, audit_Btn_Jnl);
		click(driver, audit_Btn_Jnl);
		Step_End(2, "Click audit button", test, test1);
		Step_Start(5, "Now click audit and choose the version, click compare, edited records should be highlighted in red color", test, test1);
		waitForElement(driver, right_Side_Version_Jnl);
		click(driver, right_Side_Version_Jnl);
		String version_Option=String.format(Version_Option, version_Number_Select);
		waitForElement(driver, version_Option);
		click(driver, version_Option);
		waitForElement(driver, compare_Btn_Jnl);
		click(driver, compare_Btn_Jnl);
		Step_Start(3, "Audit trail report shows all master level and detail level records", test, test1);
		waitForElement(driver, auditform_Table_Jnl);
		if(isDisplayed(driver, auditform_Table_Jnl)) {
			System.out.println("Matched || Expected Result is : AUDIT FORM TABLE SHOULD BE DISPLAY || The Actual Result is : AUDIT FORM TABLE IS DISPLAYING");
			Extent_pass_New(driver, "Matched || Expected Result is : AUDIT FORM TABLE SHOULD BE DISPLAY || The Actual Result is : AUDIT FORM TABLE IS DISPLAYING", test, test1);
		}else {
			System.out.println("Not Matched || Expected Result is : AUDIT FORM TABLE SHOULD BE DISPLAY || The Actual Result is : AUDIT FORM TABLE IS NOT DISPLAYING");
			Extent_fail(driver, "Not Matched || Expected Result is : AUDIT FORM TABLE SHOULD BE DISPLAY || The Actual Result is : AUDIT FORM TABLE IS NOT DISPLAYING", test, test1);
		}
		Step_End(3, "Audit trail report shows all master level and detail level records", test, test1);
		waitForElement(driver, altered_Debit_Amount);
		mouseOverToElement(driver, altered_Debit_Amount);
		String debit_Amount_Altered_Actual = getText(driver, altered_Debit_Amount);
		if(debit_Amount_Altered_Actual.equals(amount_Txtfield_Data)) {
			System.out.println("MATCHED || EXPECTED DEBIT AMOUNT VALUE -> "+amount_Txtfield_Data+" || ACTUAL DEBIT AMOUNT VALUE -> "+debit_Amount_Altered_Actual);
			Extent_pass_New(driver, "MATCHED || EXPECTED DEBIT AMOUNT VALUE -> "+amount_Txtfield_Data+" || ACTUAL DEBIT AMOUNT VALUE -> "+debit_Amount_Altered_Actual, test, test1);
		}else {
			System.out.println("NOT MATCHED || EXPECTED DEBIT AMOUNT VALUE -> "+amount_Txtfield_Data+" || ACTUAL DEBIT AMOUNT VALUE -> "+debit_Amount_Altered_Actual);
			Extent_fail(driver, "NOT MATCHED || EXPECTED DEBIT AMOUNT VALUE -> "+amount_Txtfield_Data+" || ACTUAL DEBIT AMOUNT VALUE -> "+debit_Amount_Altered_Actual, test, test1);
		}
		String altered_Debit_Amount_Colour_Code = getTextBackgroundColor(driver, altered_Debit_Amount);
		String altered_Debit_Amount_Colour=getColorName(altered_Debit_Amount_Colour_Code);
		System.out.println("colour after alter is : "+altered_Debit_Amount_Colour);
		if(altered_Debit_Amount_Colour.equals(colour_After_Alter)) {
			System.out.println("MATCHED || EXPECTED COLOUR OF THE ALTERED ELEMENT : "+colour_After_Alter+" || ACTUAL COLOUR OF THE ALTERED ELEMENT : "+altered_Debit_Amount_Colour);
			Extent_pass_New(driver, "MATCHED || EXPECTED COLOUR OF THE ALTERED ELEMENT : "+colour_After_Alter+" || ACTUAL COLOUR OF THE ALTERED ELEMENT : "+altered_Debit_Amount_Colour, test, test1);
		}else {
			System.out.println("NOT MATCHED || EXPECTED COLOUR OF THE ALTERED ELEMENT : "+colour_After_Alter+" || ACTUAL COLOUR OF THE ALTERED ELEMENT : "+altered_Debit_Amount_Colour);
			Extent_fail(driver, "NOT MATCHED || EXPECTED COLOUR OF THE ALTERED ELEMENT : "+colour_After_Alter+" || ACTUAL COLOUR OF THE ALTERED ELEMENT : "+altered_Debit_Amount_Colour, test, test1);
		}
		click(driver, audit_Form_Close_Btn);
		Step_End(5, "Now click audit and choose the version, click compare, edited records should be highlighted in red color", test, test1);
