...
public class TC_Import_Documentation_SC013 extends Keywords {

	public void Import_Documentation_SC013(WebDriver driver, ExtentTest test, ExtentTest test1, String selected_dataset,Map<String, String> Excel_data,String url) {
		String testCaseName="TC_Import_Documentation_SC013";
		...
		String New_Status_Check = Excel_data.get("New_Status_Check");
		...
		String POL_Search_Header=Excel_data.get("POL_Search_Header");
			...
			Step_Start(15, "Verify that All the BL should be show in New status in the Voyage. (Navigate all the bl's)", test, test1);
			...
			for(int i=1;i<Endvalue;i++) {
				...
				if(!BL_Status_Act.equals(New_Status_Check)) {
			...
			if(Bl_Status_Check) {
				System.out.println("Matched || Expected : All the BL Number's Status should be equalt to "+New_Status_Check+" When selected 'UnSaved' in the dropdown || Actual : All the BL Number's Status equal to "+New_Status_Check);
				Extent_pass_New(driver, "Matched || Expected : All the BL Number's Status should be equalt to "+New_Status_Check+" When selected 'UnSaved' in the dropdown || Actual : All the BL Number's Status equal to "+New_Status_Check, test, test1);
			}else {
				System.out.println("Not Matched || Expected : All the BL Number's Status should be equalt to "+New_Status_Check+" When selected 'UnSaved' in the dropdown || Actual : Some BL Number's Status is not equal to "+New_Status_Check+" || BL Status Mismatched BL Numbers : "+UnMatched_BL_Numbers);
				Extent_fail(driver, "Not Matched || Expected : All the BL Number's Status should be equalt to "+New_Status_Check+" When selected 'UnSaved' in the dropdown || Actual : Some BL Number's Status is not equal to "+New_Status_Check+" || BL Status Mismatched BL Numbers : "+UnMatched_BL_Numbers, test, test1);
			}
			
			Step_End(15, "Verify that All the BL should be show in New status in the Voyage. (Navigate all the bl's)", test, test1);			
