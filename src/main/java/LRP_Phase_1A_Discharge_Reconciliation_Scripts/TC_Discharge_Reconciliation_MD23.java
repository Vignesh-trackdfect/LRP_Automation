...
public class TC_Discharge_Reconciliation_MD23 extends Keywords{
	
public void Discharge_Reconciliation_MD23(WebDriver driver,ExtentTest test,ExtentTest test1,String selected_dataset,Map<String, String> Excel_data,String url) {
		
		String testcase_Name="TC_Discharge_Reconciliation_MD23";
		...
		Step_Start(9, "Window will be open Count the rows as per the BL based", test, test1);
		 int i=0;
	        Set<String>  newValue=new HashSet<String>();
	        String blnums=String.format(ManifiesBL_Numbers, i);    
	        ...
	        while(isdisplayed(driver, blnums)) {
	        	...
	        	newValue.add(BLNum);
	        	i++;
	        	blnums=String.format(ManifiesBL_Numbers, i);    
	        }
	        
	        int BL_Num_Cout=newValue.size();
		
		Step_End(9, "Window will be open Count the rows as per the BL based", test, test1);
		
        Step_Start(10, "Close the BL(s) Without MF Closed", test, test1);
		
        waitForElement(driver, Bl_without_Mf_close_btn);
		click(driver, Bl_without_Mf_close_btn);
		
		Step_End(10, "Close the BL(s) Without MF Closed", test, test1);
		
        Step_Start(11, "go to Number of B/Ls Manifest not closed label Place", test, test1);
		
        Step_Start(12, "Capture the Values", test, test1);
        waitForElement(driver, N0_bl_manifest_notclos);
		String No_Bl_Manifest_Not_Close =getAttribute(driver, N0_bl_manifest_notclos,"value");
		Step_End(12, "Capture the Values", test, test1);
        
		Step_End(11, "go to Number of B/Ls Manifest not closed label Place", test, test1);
		
		
		Step_Start(13, "Verify that BL(s) Without MF Closed count and Number of B/Ls Manifest not closed label count has to matched", test, test1);
		if(No_Bl_Manifest_Not_Close.equals(String.valueOf(BL_Num_Cout))) {
			 System.out.println("Matched || Expected :  Number of B/Ls Manifest not closed label count has to matched as "+BL_Num_Cout+ " || Actual  : Number of B/Ls Manifest not closed label count : BL(s) Without MF Closed count : "+ No_Bl_Manifest_Not_Close);
			 Extent_pass(driver,"Matched || Expected :  Number of B/Ls Manifest not closed label count has to matched as "+BL_Num_Cout+ " || Actual  : Number of B/Ls Manifest not closed label count : BL(s) Without MF Closed count : "+ No_Bl_Manifest_Not_Close,test, test1);
		 }else {
			 System.out.println("Not Matched || Expected :  Number of B/Ls Manifest not closed label count has to matched as "+BL_Num_Cout+ " || Actual  : Number of B/Ls Manifest not closed label count : "+ No_Bl_Manifest_Not_Close);
			 Extent_fail(driver,"Not Matched || Expected :  Number of B/Ls Manifest not closed label count has to matched as "+BL_Num_Cout+ " || Actual  : Number of B/Ls Manifest not closed label count : BL(s) Without MF Closed count : "+ No_Bl_Manifest_Not_Close,test, test1);
		 }
		
		Step_End(13, "Verify that BL(s) Without MF Closed count and Number of B/Ls Manifest not closed label count has to matched", test, test1);
