...
public class TC_Rate_Request_SC085 extends Keywords {

	public void Rate_Request_SC085(WebDriver driver, ExtentTest test, ExtentTest test1, String selected_dataset,
			Map<String, String> Excel_data, String url) {

		String testcase_Name = "Rate_Request_SC085";
		...
		String Save_As_popup = Excel_data.get("Save_As_popup");
		String WantToSavePopup = Excel_data.get("WantToSavePopup");
		...
		Step_Start(16, "click save as option in the tool bar.", test, test1);

		waitForElement(driver, SaveAs_button_toolBar);
		click(driver, SaveAs_button_toolBar);
		
		
		Step_End(16, "click save as option in the tool bar.", test, test1);

		Step_Start(17, "click ok.", test, test1);

		
		waitForPopup(driver, popup_Message, Save_As_popup);
		String Act_save_as_Popup = getText(driver, popup_Message);
		System.out.println("Act_save_as_Popup :"+Act_save_as_Popup);
		
		if(Act_save_as_Popup.equals(Save_As_popup)) {
			Extent_pass_New(driver,"Matched || Expected : popup message should be shown as : "+Save_As_popup+" || Actual : popup message is shown as : "+ Act_save_as_Popup, test,test1);
			System.out.println("Matched || Expected : popup message should be shown as : "+Save_As_popup+" || Actual : popup message is shown as : "+ Act_save_as_Popup);

		}else {
			Extent_fail(driver,"Not Matched || Expected : popup message should be shown as : "+Save_As_popup+" || Actual : popup message is shown as : "+ Act_save_as_Popup, test,test1);
			System.out.println("Not Matched || Expected : popup message should be shown as : "+Save_As_popup+" || Actual : popup message is shown as : "+ Act_save_as_Popup);
		...
		Step_Start(18, "Click save button in to tool bar", test, test1);
		...
		Step_End(18, "Click save button in to tool bar", test, test1);
