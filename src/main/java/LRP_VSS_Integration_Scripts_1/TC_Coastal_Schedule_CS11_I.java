...
public class TC_Coastal_Schedule_CS11_I extends Keywords {

	public void coastalschedule11I(WebDriver driver, ExtentTest test,ExtentTest test1,String selected_dataset,Map<String, String> Excel_data,String url) {
		String testcase_Name="TC_Coastal_Schedule_CS11_I";
		...
		String  terimal_Code = String.format(Coastal_Schedule_Locators.terminalCode, TerminalInput);
...
LRP_Login(driver, Username_CS11, Password_CS11);
		...
		if(Change_Future_Voyages.equalsIgnoreCase("Yes")) {
			...
			if(Terminal_Popup.equals(FutureTerminalRenamed)) {
				...
				System.out.println("Not Matched || Expected Popup : "+FutureTerminalRenamed+" || Actual Popup : "+Terminal_Popup);
				Extent_fail(driver,  "Not Matched || Expected Popup : "+FutureTerminalRenamed+" || Actual Popup : "+Terminal_Popup, test,test1);
			}
			...
			if(terminal.contains(TerminalInput)) {
				...
				System.out.println("Not Matched || Expected Terminal : "+TerminalInput+" || Actual Terminal : "+terminal);
				Extent_fail(driver, "Not Matched || Expected Terminal : "+TerminalInput+" || Actual Terminal : "+terminal, test,test1);
			}
			...
			if(isDisplayed(driver, future_Voyage)) {
				...
				if(terminal1.contains(TerminalInput)) {
					...
					Extent_fail(driver, "Not Matched || Expected Terminal : "+TerminalInput+" || Actual Terminal : "+terminal1, test,test1);
				}
			}
		...
		Step_Start(10, "Check the Terminal in Terminal module", test, test1);
		
		moduleNavigate(driver, search2);
		
		waitForElement(driver, terminal_Code_Filter);
		sendKeys(driver, terminal_Code_Filter, TerminalInput);
		
		waitForDisplay(driver, terimal_Code);
		if(isdisplayed(driver, terimal_Code)) {
			Extent_pass_New(driver,"Matched || Terminal code should be present in Terminal module || Actual : Terminal code is present in Terminal module || "+TerminalInput+ "  terminal code is displayed", test, test1);
			System.out.println("Matched || Terminal code should be present in Terminal module || Actual : Terminal code is present in Terminal module || "+TerminalInput+ "  terminal code is displayed");
			doubleClick(driver, terimal_Code);
		}else {
			System.out.println("Not Matched || Terminal code should be present in Terminal module || Actual : Terminal code is not present in Terminal module ");
			Extent_fail(driver, "Not Matched || Terminal code should be present in Terminal module || Actual : Terminal code is not present in Terminal module ", test, test1);

		}
		Step_End(10, "Check the Terminal in Terminal module", test, test1);
