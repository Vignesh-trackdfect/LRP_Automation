...
public class TC_Rate_Request_SC60   extends Keywords {
	...
	public void Rate_Request_SC60(WebDriver driver, ExtentTest test, ExtentTest test1, String selected_dataset,Map<String, String> Excel_data,String url)
			throws Exception {
...
if(isdisplayed(driver, empty_Checkbox_RR)) {
	...
	formatLocatorClick(driver, on_Carriage_Radiobtn, On_Carriage_Radiobtn);
			...
			if(actualPopup.equals(wantToProceedPopup)) {
				Extent_pass_New(driver,"Matched || Expected : popup message should be shown as :  "+wantToProceedPopup+" Actual :popup message is shown "+ actualPopup, test,test1);
				System.out.println("Matched || Expected : popup message should be shown as :  "+wantToProceedPopup+" Actual :popup message is shown "+ actualPopup);
				...
				System.out.println("Not Matched || Expected : popup message should be shown as : "+wantToProceedPopup+" Actual :popup message is not shown "+ actualPopup);
			...
			if(actSubmittedPopup.equals(submittedPopup)) {
				...
				System.out.println("Not Matched || Expected :popup message should be shown "+submittedPopup+" Actual :popup message is not  shown "+ actSubmittedPopup);
				Extent_fail(driver,"Not Matched || Expected :popup message should be shown "+submittedPopup+" Actual :popup message is not shown "+ actSubmittedPopup, test,test1);
			}
		...
		click(driver, Mail_Cancel_button);
			...
			if (payment_Mode.equalsIgnoreCase(actual_Payment_Value)) {
				System.out.println("Matched || " + " Expected Report Activity is : " + payment_Mode+ " || Actual Report Activity is : " + actual_Payment_Value);
				Extent_pass_New(driver, "Matched || " + " Expected Report Activity is : " + payment_Mode+ " || Actual Report Activity is : " + actual_Payment_Value, test,test1);
			} else {
				System.out.println("Not Matched || " + " Expected Report Activity is : " + payment_Mode+ " || Actual Report Activity is : " + actual_Payment_Value);
				Extent_fail(driver, "Not Matched || " + " Expected Report Activity is : " + payment_Mode+ " || Actual Report Activity is : " + actual_Payment_Value, test,test1);
			}
