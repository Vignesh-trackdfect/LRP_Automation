...
public class TC_Import_Documentation_SC016 extends Keywords {

	public void Import_Documentation_SC016(WebDriver driver, ExtentTest test, ExtentTest test1, String selected_dataset,Map<String, String> Excel_data,String url) {
		String testCaseName="TC_Import_Documentation_SC016";
		...
		String CargoType_status = Excel_data.get("CargoType_status");
		...
		if(!Terminal.trim().equalsIgnoreCase("")) {
		...
		Step_Start(15, ".local Bl's only has to be shown\r\n"
		...
		for(int i=1;i<Endvalue;i++) {
			...
			if(!BL_Cargo_Status_Act.equals(CargoType_status)) {
		...
		if(Bl_Status_Check) {
			System.out.println("Matched || Expected : All the BL Number's Cargo Status should be equalt to "+CargoType_status+" When selected '"+all_Dropdown_2+"' in the dropdown || Actual : All the BL Number's Cargo Status equal to "+CargoType_status);
			Extent_pass_New(driver, "Matched || Expected : All the BL Number's Cargo Status should be equalt to "+CargoType_status+" When selected '"+all_Dropdown_2+"' in the dropdown || Actual : All the BL Number's Cargo Status equal to "+CargoType_status, test, test1);
		}else {
			System.out.println("Not Matched || Expected : All the BL Number's Cargo Status should be equalt to "+CargoType_status+" When selected '"+all_Dropdown_2+"' in the dropdown || Actual : Some BL Number's Cargo Status is not equal to "+CargoType_status+" || BL Cargo Status Mismatched BL Numbers : "+UnMatched_BL_Numbers);
			Extent_fail(driver, "Not Matched || Expected : All the BL Number's Cargo Status should be equalt to "+CargoType_status+" When selected '"+all_Dropdown_2+"' in the dropdown || Actual : Some BL Number's Cargo Status is not equal to "+CargoType_status+" || BL Cargo Status Mismatched BL Numbers : "+UnMatched_BL_Numbers, test, test1);
		}
		
		Step_End(15, ".local Bl's only has to be shown\r\n"
				+ "navigate all the Bl's", test, test1);
