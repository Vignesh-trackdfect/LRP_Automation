...
public class TC_Ecommerce_Gateway_Validator_004 extends Keywords {

	public void Ecommerce_Gateway_Validator_004(WebDriver driver, ExtentTest test, ExtentTest test1,
			String selected_dataset,Map<String, String> Excel_data,String url) {

		String testcase_Name = "TC_Ecommerce_Gateway_Validator_004";
		...
		Step_Start(1,
				"Enter the reference number and click the refresh button. transaction type and non-processed checkbox based on the booking and SI records should show in the grid",
				test, test1);
			...
			try {
				...
				if (!Reference_Number.isEmpty()) {
					...
					if (grid_RefNo.equals(Reference_Number)) {
						...
						System.out.println("All the reference number details are not showing in the grid || Expected : "
								+ Reference_Number + " || Actual : " + grid_RefNo);
						Extent_fail(driver,
								"All the reference number details are not showing in the grid || Expected : "
										+ Reference_Number + " || Actual : " + grid_RefNo,
								test, test1);
					}
		...
		for (String Expected : ExpectedStatuses) {
			...
			if (NonProcessed.contains(Expected)) {
				...
				System.out.println("Expected :" + Expected + " status data should be displayed in the Grid || Actual : "
						+ Expected + " Status data's are showing in the Grid and the Displayed status was : "
						+ NonProcessed);
				Extent_pass_New(driver,
						"Expected :" + Expected + " status data should be displayed in the Grid || Actual : " + Expected
