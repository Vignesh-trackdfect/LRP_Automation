...
public class TC_Rate_Request_SC05 extends Keywords {
	...
	public void SpecialType(WebDriver driver, ExtentTest test, ExtentTest test1, String selected_dataset,Map<String, String> Excel_data,String url)
			throws Exception {
		...
		String expected_Request_Type = Excel_data.get("Expected_Request_Type");
...
newButton(driver);
		...
		if(actual_Req_Type.equals(expected_Request_Type)) {
			...
			Extent_pass_New(driver, "Matched|| "+" Expected REQUEST TYPE is : "+expected_Request_Type+" || Actual REQUEST TYPE is : "+ actual_Req_Type, test, test1);
		}else {
		...
		if (flag) {
			System.out.println("Expected Result is : system Should allows to Submit an Special type rate request || Actual Result is : system allows to Submit an Special type rate request");
			Extent_pass_New(driver, "Expected Result is : system Should allows to Submit an Special type rate request || Actual Result is : system allows to Submit an Special type rate request", test, test1);

		} else {
			System.out.println("Expected Result is : system Should allows to Submit an Special type rate request || Actual Result is : system not allows to Submit an Special type rate request");
			Extent_fail(driver, "Expected Result is : system Should allows to Submit an Special type rate request || Actual Result is : system not allows to Submit an Special type rate request", test, test1);
		...
		if(Do_You_Want_Delete.equalsIgnoreCase("Yes")){
			...
			if(submitted_ReqNum.equals(actualReqNo)) {
				...
				Extent_pass_New(driver,"Matched || " + " Expected Value is : " + actualReqNo + " || ActualValue is : " + submitted_ReqNum,test,test1);
				Extent_pass_New(driver, actualReqNo+"  is submitted", test,test1);
			...
			if(pending_ReqNum.equals(actualReqNo)) {
				...
				Extent_pass_New(driver,"Matched || " + " Expected Value is : " + actualReqNo + " || ActualValue is : " + pending_ReqNum,test,test1);
				Extent_pass_New(driver, actualReqNo+"  is Deleted", test,test1);
				...
				globalValueSearchWindow(driver, Condition_Value,Rate_Request_NO,actualReqNo,"", "", "","");
