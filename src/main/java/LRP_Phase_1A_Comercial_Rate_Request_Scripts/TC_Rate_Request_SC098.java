...
public class TC_Rate_Request_SC098 extends Keywords{
	
	public void Rate_Request_SC098(WebDriver driver, ExtentTest test, ExtentTest test1, String selected_dataset,
			Map<String, String> Excel_data, String url) {

		String testcase_Name = "Rate_Request_SC098";
		...
		String Status_Name = Excel_data.get("Status_Name");
		String Module_SRR_Gate = Excel_data.get("Module_SRR_Gate");
		String SRR_RequestType = Excel_data.get("SRR_RequestType");
		...
		waitForPopup(driver, status_Field,Status_Name);
		String Act_Status =getAttribute(driver, status_Field,"value");
		
		waitForElement(driver, status_Field);
		if(Act_Status.equals("FILED")) {
			...
			String select_ReqestType=String.format(DropDown_Select, SRR_RequestType);
			...
			sendKeys(driver, filter_Inputfield, Act_Pending_Rer_num);
			enter(driver);

			waitForElement(driver, select_Actual_ReqNo);
			String submitted_ReqNum1=getText(driver, select_Actual_ReqNo);
			if(submitted_ReqNum1.equals(Act_Pending_Rer_num)) {
				System.out.println("Matched || " + " Expected Value is : " + Act_Pending_Rer_num + " || Actual Value is : " + submitted_ReqNum1);
				Extent_pass_New(driver,"Matched || " + " Expected Value is : " + Act_Pending_Rer_num + " || ActualValue is : " + submitted_ReqNum1,test,test1);
				Extent_pass_New(driver, Act_Pending_Rer_num+"  is submitted", test,test1);
				System.out.println(Act_Pending_Rer_num+" is submitted");
				waitForElement(driver, select_Actual_ReqNo);
				doubleClick(driver, select_Actual_ReqNo);
			}else {
				System.out.println("Not matched || " + " Expected Value is : " + Act_Pending_Rer_num + " || Actual Value  is : " + submitted_ReqNum1);
				Extent_fail(driver,"Not matched || " + " Expected Value is : " + Act_Pending_Rer_num + " || Actual Value is : " + submitted_ReqNum1,test,test1);
				Extent_fail(driver, Act_Pending_Rer_num+" Error is not displayed", test,test1);
				System.out.println(Act_Pending_Rer_num+" is not submitted");
			}	
