...
public class TC_Detention_Waiver_TS_010B extends Keywords {

	public void Detention_Waiver_TS_010B(WebDriver driver, ExtentTest test, ExtentTest test1, String selected_dataset,Map<String, String> Excel_data,String url) {

		String testcase_Name = "TC_Detention_Waiver_TS_010B";
		...
		String WaiverPercentage_above_hundred_popup = Excel_data.get("WaiverPercentage_above_hundred_popup");
		String WaiverPercentage_Above_hundred = Excel_data.get("WaiverPercentage_Above_hundred");
...
if (Seletct_Container_All.equalsIgnoreCase("Yes")) {
	...
	ContainersList.addAll(Containers_Select);
		...
		Step_Start(6, "Go to % Field and enter the Value as 101 ", test, test1);
		...
		waitForElement(driver, DW_Percenteage_Input_Field);
		sendKeys(driver, DW_Percenteage_Input_Field, WaiverPercentage_Above_hundred);
		}
		Step_End(6, "Go to % Field and enter the Value as 101 ", test, test1);


		waitForElement(driver, Waier_plus_button);
		click(driver, Waier_plus_button);
		Step_Start(7,
				"Verify that System shows Validation for Negative  'Waiver percentage should be greater than zero",
				test, test1);
		waitForPopup(driver, popup_Message, WaiverPercentage_above_hundred_popup);
		String positive_value_above_hundered_validation = getText(driver, popup_Message);

		if (positive_value_above_hundered_validation.equals(WaiverPercentage_above_hundred_popup)) {
			System.out.println("Matched || Excepted popup : " + WaiverPercentage_above_hundred_popup
					+ " || Actual Value : " + positive_value_above_hundered_validation);
			Extent_pass_New(driver, "  Matched ||   Expected popup : " + WaiverPercentage_above_hundred_popup
					+ "   ||   Actual Value    : " + positive_value_above_hundered_validation, test, test1);
		} else {
			System.out.println("Not Matched ||   Expected popup  : " + WaiverPercentage_above_hundred_popup
					+ "   ||   Actual Value    : " + positive_value_above_hundered_validation);
			Extent_fail(driver, "Not Matched ||   Expected popup  : " + WaiverPercentage_above_hundred_popup
					+ "   ||   Actual Value    : " + positive_value_above_hundered_validation, test, test1);
		}
		...
		Step_End(7, "Verify that System shows Validation for Negative  'Waiver percentage should be greater than zero",
				test, test1);
