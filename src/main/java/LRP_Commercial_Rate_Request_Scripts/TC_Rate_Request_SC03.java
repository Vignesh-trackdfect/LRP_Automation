...
public class TC_Rate_Request_SC03 extends Keywords {
	...
	public void DGSubmit(WebDriver driver, ExtentTest test, ExtentTest test1, String selected_dataset,Map<String, String> Excel_data,String url)
			throws Exception {
...
newButton(driver);
		...
		if(dg_Details_Actual.equals(dg_Details_Expected)) {
			Extent_pass_New(driver, "DG details available in the submitted rate request is matched with excel input", test, test1);
			...
			Extent_pass_New(driver, "Matched || Expected Dg details -> "+dg_Details_Expected +"|| Actual dg details -> "+ dg_Details_Actual, test, test1);
		}else {
		...
		if(Do_You_Want_Delete.equalsIgnoreCase("Yes")){
			...
			if(pending_ReqNum.equals(actualReqNo)) {
				...
				Extent_pass_New(driver, actualReqNo+"  is moved To pending", test,test1);
