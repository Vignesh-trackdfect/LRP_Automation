...
public class TC_Import_Invoice_SC012B extends Keywords {

	public void Import_Invoice_SC012B(WebDriver driver, ExtentTest test, ExtentTest test1, String selected_dataset,Map<String, String> Excel_data,String url) {

		String testcase_Name = "TC_Import_Invoice_SC012B";
		...
		String columnHeaders_nonManifestCharges = Excel_data.get("ColumnHeaders_nonManifestCharges");
		String non_Manifest_Table_Name_In = Excel_data.get("Non_Manifest_Table_Name_In");
		String non_Manifest_Charges_Tab = Excel_data.get("Non_Manifest_Charges_Tab");
...
//		Non Manifest Charges Tab	
		...
		List<WebElement> total_Rows_Non_Manifest = listOfElements(driver, nonManifestCharges_Table_Rows_IN);
		int manifestRows_Non_Manifest = total_Rows_Non_Manifest.size();
		...
		List<String> nonManifestCharges_headers = splitAndExpand(columnHeaders_nonManifestCharges);

		String non_Manifest_Tabledata = TableDataForReport(driver, nonManifestCharges_Tabledata,
				non_Manifest_Table_Name_In, nonManifestCharges_headers);

		ArrayList<String> non_Manifest_ChargeCode = new ArrayList<String>();
		List<String> non_Manifest_Values = getValuesByHeader(nonManifestCharges_Tabledata, Charge_code_Header);
		...
		List<String> inv_Amount_Non_Manifest = getValuesByHeader(nonManifestCharges_Tabledata, Amount_Header);

		ArrayList<String> invoice_Amount_Non_Manifest = new ArrayList<String>();
		ArrayList<String> invoice_Amount_Neg_Non_Manifest = new ArrayList<String>();

		for (int i = 0; i < inv_Amount_Non_Manifest.size(); i++) {
			invoice_Amount_Non_Manifest.add(inv_Amount_Non_Manifest.get(i));
			String negative = "-" + invoice_Amount_Non_Manifest.get(i);
			invoice_Amount_Neg_Non_Manifest.add(negative);
		}
		System.out.println("Invoice Amount : " + invoice_Amount_Non_Manifest);
		System.out.println("Invoice Amount Negative : " + invoice_Amount_Neg_Non_Manifest);
...
//	BL Details
		...
		Step_End(1, "bl details,(All the Columns)", test, test1);



//	Non Manifest Charges Tab	

		Step_Start(3, "non manifest charge(All the Columns)", test, test1);

		if (!non_Manifest_Charges_Tab.equals("")) {

			waitForElement(driver, nonManifestCharges_Tab_IN);
			click(driver, nonManifestCharges_Tab_IN);

			waitForElement(driver, nonManifestCharges_Table_Rows_IN);
			List<WebElement> total_Rows_Non_Manifest1 = listOfElements(driver, nonManifestCharges_Table_Rows_IN);
			int manifestRows_Non_Manifest1 = total_Rows_Non_Manifest1.size();

			List<Map<String, String>> nonManifestCharges_Tabledata1 = getTableData(driver,
					nonManifestCharges_Table_Columns_IN, nonManifestCharges_Table_Rows_IN);

			List<String> nonManifestCharges_headers1 = splitAndExpand(columnHeaders_nonManifestCharges);

			String non_Manifest_Tabledata1 = TableDataForReport(driver, nonManifestCharges_Tabledata1,
					non_Manifest_Table_Name_In, nonManifestCharges_headers1);

			ArrayList<String> non_Manifest_ChargeCode1 = new ArrayList<String>();
			List<String> non_Manifest_Values1 = getValuesByHeader(nonManifestCharges_Tabledata1, Charge_code_Header);

			for (int i = 0; i < non_Manifest_Values1.size(); i++) {

				System.out.println("manifest_Values : " + non_Manifest_Values1);
				non_Manifest_ChargeCode1.add(non_Manifest_Values1.get(i));

			}

			System.out.println("non_Manifest_ChargeCode : " + non_Manifest_ChargeCode1);

			List<String> inv_Amount_Non_Manifest1 = getValuesByHeader(nonManifestCharges_Tabledata1, Amount_Header);

			ArrayList<String> invoice_Amount_Non_Manifest1 = new ArrayList<String>();

			for (int i = 0; i < inv_Amount_Non_Manifest1.size(); i++) {
				invoice_Amount_Non_Manifest1.add(inv_Amount_Non_Manifest1.get(i));
			}
			System.out.println("Invoice Amount : " + inv_Amount_Non_Manifest1);

			if (manifestRows_Non_Manifest == manifestRows_Non_Manifest1) {
				System.out.println("Matched || Charge code count before reversal : " + manifestRows_Non_Manifest
						+ " || Charge code count after reversal : " + manifestRows_Non_Manifest1);
				Extent_pass_New(driver, "Matched || Charge code count before reversal : " + manifestRows_Non_Manifest
						+ " || Charge code count after reversal : " + manifestRows_Non_Manifest1, test, test1);
			} else {
				System.out.println("Not Matched || Charge code count before reversal : " + manifestRows_Non_Manifest
						+ " || Charge code count after reversal : " + manifestRows_Non_Manifest1);
				Extent_fail(driver, "Not Matched || Charge code count before reversal : " + manifestRows_Non_Manifest
						+ " || Charge code count after reversal : " + manifestRows_Non_Manifest1, test, test1);
			}

			boolean pass1 = false;
			for (int i = 0; i < manifestRows_Non_Manifest; i++) {
				if (inv_Amount_Non_Manifest1.contains(invoice_Amount_Neg_Non_Manifest.get(i))
						&& non_Manifest_ChargeCode1.contains(non_Manifest_ChargeCode.get(i))) {
					pass1 = true;
				} else {
					pass1 = false;
				}
			...
			if (pass1 == true) { 
				System.out.println(
						"Matched || Expected result is : Non Manifest Charges Table should be displayed in Import Invoice Non Manifest Charges Tab || Non Manifest Charges Table is displayed in Import Invoice Non Manifest Charges Tab");
				
				Extent_pass_New(driver,
						"Matched || Before Reversal: \n" + "<pre>" + non_Manifest_Tabledata + "</pre>"
								+ "\r\n || After Reversal : \r\n" + "<pre>" + non_Manifest_Tabledata1 + "</pre>",
						test, test1);
			} else {
				System.out.println(
						"Not Matched || Expected result is : Non Manifest Charges Table should be displayed in Import Invoice Non Manifest Charges Tab || Non Manifest Charges Table is not displayed in Import Invoice Non Manifest Charges Tab");
				Extent_fail(driver,
						"Not Matched || Expected result is : Non Manifest Charges Table should be displayed in Import Invoice Non Manifest Charges Tab || Non Manifest Charges Table is not displayed in Import Invoice Non Manifest Charges Tab",
						test, test1);
			}
