...
public class TC_Feeder_Contract_SC24 extends Keywords {
	public void Feeder_Contract_SC24(WebDriver driver, ExtentTest test, ExtentTest test1, String selected_dataset,
			Map<String, String> Excel_data, String url) {
		String testcase_Name = "TC_Feeder_Contract_SC24";
		...
		Step_Start(5, "Select the Neg on date to click the field.", test, test1);

		waitForElement(driver, FC_NegotiatedOn);
		selectDatePicker(driver, FC_NegotiatedOn, Negotiated_Date);

		waitForElement(driver, FC_NegotiatedOn);
		String HC_From_Date_Act = getAttribute(driver, FC_NegotiatedOn, "value");

		if (Negotiated_Date.equalsIgnoreCase(HC_From_Date_Act)) {
			System.out.println("Matched ||Expected Value taken in test data for selecting Neg On :" + Negotiated_Date
					+ "|| Actual Value taken in Neg On Date in Application: " + HC_From_Date_Act);
			Extent_pass_New(driver,
					"Matched ||Expected Value taken in test data for selecting Neg On:" + Negotiated_Date
							+ " ||Actual Value taken in Neg On Date in Application  : " + HC_From_Date_Act,
					test, test1);
		} else {
			System.out.println("NotMatched ||Expected Value taken in test data for selecting Neg On:" + Negotiated_Date
					+ " ||Actual Value taken in Neg On Date in Application : " + HC_From_Date_Act);
			Extent_fail(driver, "NotMatched ||Expected Value taken in test data for selecting Neg On:" + Negotiated_Date
					+ " ||Actual Value taken in Neg On Date in Application: " + HC_From_Date_Act, test, test1);
		}

		Step_End(5, "Select the Neg on date to click the field", test, test1);
