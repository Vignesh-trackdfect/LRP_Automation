...
public class TC_Vendor_Invoice_Registration_SC24 extends Keywords{
	
	public void Vendor_Invoice_Registration_SC24(WebDriver driver, ExtentTest test,ExtentTest test1,String selected_Excel_dataset,Map<String, String> Excel_data,String url) {
		
		String testCaseName="TC_Vendor_Invoice_Registration_SC24";
		//  Get data from excel sheet	
			...
			String Storage_Expense_Status_Exp = Excel_data.get("Storage_Expense_Status_Exp");
			String Storage_ID_Search = Excel_data.get("Storage_ID_Search");
			...
			String Storage_Expense_Report_Module = Excel_data.get("Storage_Expense_Report_Module");
...
//			Vendor Invoice Registration
		...
		// Vendor invoice number
           ...
           Step_Start(11, "In the Activity details, Click Activity type dropdown and select the activity type as a \"STC\"", test, test1); 
			...
			Step_End(11, "In the Activity details, Click Activity type dropdown and select the activity type as a \"STP\"", test, test1);
			Step_Start(12, "Click the Route", test, test1);
			waitForElement(driver, stp_Route_Button_VIR);
			click(driver, stp_Route_Button_VIR);
			Step_End(12, "Click the Route", test, test1);
			
			Step_Start(13, "Click the Port search and select the Required Port", test, test1);
			waitForElement(driver, stp_Route_Port_Search_Button_VIR);
			click(driver, stp_Route_Port_Search_Button_VIR);
			
			twoColumnSearchWindow(driver, Port_Code_Header, condition, Port_Code_Value);
			Step_End(13, "Click the Port search and select the Required Port", test, test1);
			
			Step_Start(14, "Click the Terminal search and select the Required Terminal", test, test1);
			waitForElement(driver, stp_Route_Terminal_Search_Button_VIR);
			click(driver, stp_Route_Terminal_Search_Button_VIR);
	          
			twoColumnSearchWindow(driver, Terminal_Code_Header, condition, Terminal_Code_Value);
			Step_End(14, "Click the Terminal search and select the Required Terminal", test, test1);
           
			Step_Start(15, "Click the \"+ADD\" button, Details added in the Grid", test, test1);
			waitForElement(driver, stp_Route_Add_Button);
			click(driver, stp_Route_Add_Button);
			...
			scrollBottom(driver);
  			...
  			waitForElement(driver, stp_Route_Grid_Data);
  			List<Map<String, String>> Table_Data = extractTableDataByColumn(driver, stp_Route_Grid_Data);	
  			String Activity_No_Value = getFirstValueByHeader(Table_Data, "Activity No");              
            System.out.println("Activity_No_Value :"+ Activity_No_Value);
        ...
        moduleNavigate(driver, Storage_Expense_Report_Module);
        ...
        globalValueSearchWindow(driver, dropdownCondition, Storage_ID_Search, Activity_No_Value, "", "", "", "");
	
		waitForElement(driver, storage_Expense_status);
		String Storage_Expense_Status_Act=getText(driver, storage_Expense_status);
		System.out.println("Storage_Expense_Status_Act :"+Storage_Expense_Status_Act);
		
		if(Storage_Expense_Status_Exp.equals(Storage_Expense_Status_Act)) {
			System.out.println("Matched || Expected status is : "+ Storage_Expense_Status_Exp  +" || Actual status is : "+ Storage_Expense_Status_Act);
			Extent_pass_New(driver, "Matched || Expected status is : "+ Storage_Expense_Status_Exp  +" || Actual status is : "+ Storage_Expense_Status_Act, test, test1);
			
		}else {
			System.out.println("Not Matched || Expected status is : "+ Storage_Expense_Status_Exp  +" || Actual status is : "+ Storage_Expense_Status_Act);
		    Extent_fail(driver, "Not Matched || Expected status is : "+ Storage_Expense_Status_Exp  +" || Actual status is : "+ Storage_Expense_Status_Act, test, test1);
