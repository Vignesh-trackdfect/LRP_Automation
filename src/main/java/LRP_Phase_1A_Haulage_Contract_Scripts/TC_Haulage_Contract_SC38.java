...
public class TC_Haulage_Contract_SC38	extends Keywords{
		public void Haulage_Contract_SC38(WebDriver driver, ExtentTest test, ExtentTest test1, String selected_dataset,Map<String, String> Excel_data,String url) throws AWTException {
			String testcase_Name="TC_Haulage_Contract_SC38";
		...
		Step_Start(8, "Click on the Equipment drop down and select the Equipment type ", test, test1);
			...
			Step_Start(10, "Click on the Hmode and Trip type drop down and select the value ", test, test1);
			...
			Step_End(10, "Click on the Hmode and Trip type drop down and select the value ", test, test1);
...
Step_End(11, "Click on the PIck up term drop down and select the value and also click on Drop term Drop down and select the Value ", test, test1);
         ...
         Step_Start(12, "Select the valid from and valid to date ", test, test1);
			...
			selectDatePicker(driver, HC_To_Date, To_Date_Input);
              ...
              Step_End(12, "Select the valid from and valid to date ", test, test1);
            ...
            Step_Start(13, "Click on the Neg by and select the value .", test, test1);
			...
			Step_End(13, "Click on the Neg by and select the value .", test, test1);

			Step_Start(14, "Select the amt type  drop dowm and select the type ", test, test1);
			waitForElement(driver, HC_Amount_Select);
			click(driver, HC_Amount_Select);
			formatLocatorClick(driver, DropDown_Select, Amount_Type);	 
			Step_End(14, "Select the amt type  drop dowm and select the type ", test, test1);


			Step_Start(15, "Select the amount field  and enter the amount on that field.", test, test1);
			waitForElement(driver, HC_Amount_Input);
			click(driver, HC_Amount_Input);
			clearAndType(driver, HC_Amount_Input, Amount_Input);
			Step_End(15, "Select the amount field  and enter the amount on that field.", test, test1);

Step_Start(16, "Click the currency and select the currency code", test, test1);
			...
			Step_End(16, "Click the currency and select the currency code", test, test1);


			Step_Start(17, "Click \"OK\" to save the details in haulage contract.", test, test1);
			waitForElement(driver, HC_Tab_Ok_Button);
			click(driver, HC_Tab_Ok_Button);
			Step_End(17, "Click \"OK\" to save the details in haulage contract.", test, test1);
    	...
    	Step_Start(18, "Click the \"+\" in surcharge and select the Charge code .", test, test1);
    		...
    		for (String surcharge : surcharge_Value) {
    		    if (surcharge != null && !surcharge.trim().isEmpty()) {
    		       ...
    		       waitForElement(driver, added);
    		        click(driver, added);
    		    }
    		    
    		}	
    		
    		Step_End(18, "Click the \"+\" in surcharge and select the Charge code .", test, test1);
    		
    		Step_Start(19, "Double click the charge and edit the amount.", test, test1);
    		...
    		for (String charge_Type : given_ChargeTypes) {
    			...
    			waitForDisplay(driver, HC_ChargeType_Dropdown);
    			formatLocatorClick(driver, HC_ChargeType_Dropdown, String.valueOf(i));
    			formatLocatorClick(driver, HC_ChargeType_Dropdown, String.valueOf(i));
    			...
    			if (charge_Type.equals("LumpSum") || charge_Type.equals("Percentage")) {
    				...
    				String AmountInput = String.format(HC_Amount_Input_data, i);

    				doubleClick(driver, AmountInput);

    				sendKeysUsingRobot(SurchargeAmount.get(i));

    				tab(driver);

    			} else {

    				formatLocatorClick(driver, HC_SurchargeRowSelect, String.valueOf(i));

    				waitForElement(driver, HC_Formula_Builder_Button);
    				click(driver, HC_Formula_Builder_Button);
    				...
    				if (Formula_Build_Option.equals("By_Sendkeys")) {
    					...
    					twoColumnSearchWindowMultipleValue(driver, Formula_Variable_Header, condition,
    							Formula_Variable);
    		...
    		Step_End(19, "Double click the charge and edit the amount.", test, test1);
