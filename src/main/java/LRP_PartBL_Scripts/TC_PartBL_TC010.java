...
public class TC_PartBL_TC010 extends Keywords {

	public void PartBL_TC010(WebDriver driver, ExtentTest test, ExtentTest test1, String selected_dataset,
			Map<String, String> Excel_data, String url) throws Exception {

		String testCaseName = "TC_PartBL_TC010";
		...
		for (Map<String, String> griddatas : commoditydatas_ChildBl) {
			...
			avai_Qty = griddatas.get(PBL_Avl_Header);
		...
		for (String Comm_Eqp_Id_Child : Comm_Eqp_Ids_ChildBl) {
			...
			waitForElement(driver, PBL_Quantity_Field);
			sendKeys(driver, PBL_Quantity_Field, Available_Qties_ChildBl.get(l));
		...
		Step_Start(21, "Retrieve the BL in the Bill of Lading module and check the commodity quantity.", test, test1);
		...
		for (int k = 0; k < Comm_Eqp_Ids_ChildBl.size(); k++) {
			...
			String qty = Available_Qties_ChildBl.get(k);
		...
		Step_End(21, "Retrieve the BL in the Bill of Lading module and check the commodity quantity.", test, test1);
