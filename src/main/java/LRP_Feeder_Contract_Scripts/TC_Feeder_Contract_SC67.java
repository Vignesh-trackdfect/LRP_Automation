...
public class TC_Feeder_Contract_SC67 extends Keywords {

	public void Feeder_Contract_SC67(WebDriver driver, ExtentTest test, ExtentTest test1, String selected_dataset,
			Map<String, String> Excel_data, String url) throws Exception {

		String testcase_Name = "TC_Feeder_Contract_SC67";
		...
		String ColumnsToRemove = Excel_data.get("ColumnsToRemove");
		String RemoveColumns_Sheetname = Excel_data.get("RemoveColumns_Sheetname");
		String Saved_Pop_Duplicate = Excel_data.get("Saved_Pop_Duplicate");
		...
		String Excel_Sheetnames = Excel_data.get("Excel_Sheetnames");
		...
		Step_Start(20, "Click the '+' in surcharge and select the Charge code .", test, test1);
		...
		Step_End(23, "Then click the save in the Tool bar ,Details should be saved. ", test, test1);

		Step_Start(24, "Click the Apply DG it apply for the Select the port details ", test, test1);

		waitForElement(driver, SaveButton_ToolBar);
		click(driver, SaveButton_ToolBar);

		Step_End(24, "Click the Apply DG it apply for the Select the port details ", test, test1);
		...
		if (Saved_Pop.equals(ActualSaved_popup)) {

			System.out.println("Matched || Expected popup message was : " + Saved_Pop
					+ " || Actual popup message was : " + ActualSaved_popup);
			Extent_pass_New(driver, "Matched || Expected popup message was : " + Saved_Pop
					+ " || Actual popup message was : " + ActualSaved_popup, test, test1);
		} else {

			System.out.println("Not Matched || Expected popup message was : " + Saved_Pop
					+ " || Actual popup message was : " + ActualSaved_popup);
			Extent_fail(driver, "Not Matched || Expected popup message was : " + Saved_Pop
					+ " || Actual popup message was : " + ActualSaved_popup, test, test1);
		...
		Step_Start(26, "click the export button, the excel should be download.", test, test1);
		...
		Step_End(26, "click the export button, the excel should be download.", test, test1);

		Step_Start(26, "click the export button, the excel should be download.", test, test1);

		removeColumns(filePath, RemoveColumns_Sheetname, ColumnsToRemove);

		Step_End(27, "click the export button, the excel should be download.", test, test1);
		...
		Step_Start(28, "click new button in tool bar.", test, test1);

		newButton(driver);

		Step_End(28, "click new button in tool bar.", test, test1);

		Step_Start(29, "select the same vendor.", test, test1);
		...
		Step_End(29, "select the same vendor.", test, test1);

		Step_Start(30, "Upload the excel file using import excel.", test, test1);

		Step_Start(31, "It show imported successfully and click Ok.", test, test1);
		...
		List<String> givensheets = splitAndExpand(Excel_Sheetnames);

		int m = 1;

		for (String sheetnames : givensheets) {

			String sheet1Count = Findrowcount(driver, editedFilepath, sheetnames);

			int sheet1 = Integer.parseInt(sheet1Count);

			if (sheet1 >= 1) {

				formatLocatorClick(driver, FC_ImportTabs, String.valueOf(m));

				waitForElement(driver, FC_SelectSheet);
				click(driver, FC_SelectSheet);

				formatLocatorClick(driver, DropDown_Select, sheetnames);
		...
		if (Expected_pop.equals(Act_pop1)) {
			System.out.println("Expected popup is :" + Expected_pop + " Actual popup is : " + Act_pop1);
			Extent_pass_New(driver, "Expected  popup is :" + Expected_pop + " Actual  popup is : " + Act_pop1, test,
					test1);
		} else {
			System.out.println("Expected  popup is :" + Expected_pop
					+ " Actual Result taken from application after two coloum search : " + Act_pop1);
			Extent_fail(driver,
		...
		Step_Start(32, "Click save button, It shows the validation pop message", test, test1);

		waitForElement(driver, SaveButton_ToolBar);
		click(driver, SaveButton_ToolBar);

		waitForPopup(driver, FC_Duplicate_pop, Saved_Pop_Duplicate);

		waitForElement(driver, FC_Duplicate_pop);
		String Act_pop_Dup = getText(driver, FC_Duplicate_pop);

		if (Saved_Pop_Duplicate.equals(Act_pop_Dup)) {
			System.out.println("Expected popup is :" + Saved_Pop_Duplicate + " Actual popup is : " + Act_pop_Dup);
			Extent_pass_New(driver, "Expected  popup is :" + Saved_Pop_Duplicate + " Actual  popup is : " + Act_pop_Dup,
					test, test1);
		} else {
			System.out.println("Expected  popup is :" + Saved_Pop_Duplicate + " Actual  popup is : " + Act_pop_Dup);
			Extent_fail(driver, "Expected  popup is :" + Saved_Pop_Duplicate + " Actual  popup is: " + Act_pop_Dup,
					test, test1);
		}
		...
		if (Saved_Pop.equals(Act_pop)) {
			System.out.println("Expected popup is :" + Saved_Pop + " Actual popup is : " + Act_pop);
			Extent_pass_New(driver, "Expected  popup is :" + Saved_Pop + " Actual  popup is : " + Act_pop, test, test1);
		} else {
			System.out.println("Expected  popup is :" + Saved_Pop + " Actual  popup is : " + Act_pop);
			Extent_fail(driver, "Expected  popup is :" + Saved_Pop + " Actual  popup is: " + Act_pop, test, test1);
		}

		waitForElement(driver, popup_Message_Ok_Button);
		click(driver, popup_Message_Ok_Button);

		Step_End(32, "Click save button, It shows the validation pop message", test, test1);
