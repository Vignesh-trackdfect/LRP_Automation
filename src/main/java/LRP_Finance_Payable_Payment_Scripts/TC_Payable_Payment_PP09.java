...
public class TC_Payable_Payment_PP09 extends Keywords {

	
	public void Payable_Payment_PP09(WebDriver driver, ExtentTest test, ExtentTest test1,String selected_dataset,Map<String, String> Excel_data,String url) {
		
		
		String testcase_Name="TC_Payable_Payment_PP09";
		...
		waitForElement(driver, description_Txtfld_PPT);
		sendKeys(driver, description_Txtfld_PPT, description_Data);
		
		Step_End(7, "Click 'Cash or Bank detail' and enter the values in the respective fields", test, test1);

		Step_Start(8, "Click on save button without 'clicking post'", test, test1);
		
		waitForElement(driver, SaveButton_ToolBar);
		click(driver, SaveButton_ToolBar);
		
		Step_End(8, "Click on save button without 'clicking post'", test, test1);

		Step_Start(9, "Click save, system should show pop up validation as 'Enter the Account Details'", test, test1);
		...
		if(expected_Popup.equals(actual_Popup)) {
			...
			System.out.println("MATCHED || Expected After click on save button Popup value should be display in the Payable Payment Screen "+expected_Popup+"||Actual: Click on save button Popup value Is display in the Payable Payment Screen : "+actual_Popup);
		Extent_pass_New(driver, "MATCHED ||Expected After click on save button Popup value should be display in the Payable Payment Screen "+expected_Popup+"|| Actual: Click on save button Popup value Is display in the Payable Payment Screen  : "+actual_Popup, test, test1);
		...
		Step_End(9, "Click save, system should show pop up validation as 'Enter the Account Details'", test, test1);
