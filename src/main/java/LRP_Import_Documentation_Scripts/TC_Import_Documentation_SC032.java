...
public class TC_Import_Documentation_SC032  extends Keywords{
	public void Import_Documentation_SC032(WebDriver driver, ExtentTest test, ExtentTest test1, String selected_dataset,Map<String, String> Excel_data,String url) {

		String testcase_Name="TC_Import_Documentation_SC032";
		...
		deleteSavedBL(driver,"NEW",dropdownCondition1,bl_Number);
		...
		Map<String,List<String>> BL_Number_Commodity=new HashMap<String, List<String>>();
		...
		for(int i=0;i<HouseBL_NewNumbers.size();i++) {
			...
			Step_Start(6, ".list the containers for that bl -select the container and add.", test, test1);
			Step_Start(7, "After adding the House bl in HBL updation tab.", test, test1);
			...
			Step_End(6, ".list the containers for that bl -select the container and add.", test, test1);
			Step_End(7, "After adding the House bl in HBL updation tab.", test, test1);
		}

		Step_Start(8, "After adding the House bl in HBL updation tab", test, test1);
		Step_Start(9, "Go to the Split charge tab -and abve HBLno dropdownfield is there -select the hosue bl no", test, test1);
		Step_Start(10, "select the check box and click the add same as another hosue no  bl and then save the import documentation.", test, test1);
		
		//Storing the Split Charegs Name with Amount for each BL number
		Map<String,Map<String,String>>  BL_SplitChargeAmount=new HashMap<String,Map<String, String>>();
		...
		for(String hbl_Charge : BL_SplitCharge.keySet()) {
			...
			for(String check:splitCheckbox) {
				...
				double Amount_Value=getValuesByFirstTwoColumnAndHeader(split_Chareges_Tabledata,SplitChargeCode_Header,check,"Rate Ref Number",RefNo,SplitChargeAmount_Header);
				String AmountValue=String.valueOf(Amount_Value);
				//
//				String AmountValue=getTableCellValue(split_Chareges_Tabledata,SplitChargeCode_Header,check,SplitChargeAmount_Header);
				chargesAmount.put(check, AmountValue);
				ref++;
			}
			
			BL_SplitChargeAmount.put(hbl_Charge, chargesAmount);//add the BL number and its chargecode&Amount Value
		...
		Step_End(9, "Go to the Split charge tab -and abve HBLno dropdownfield is there -select the hosue bl no", test, test1);
		Step_End(10, "select the check box and click the add same as another hosue no  bl and then save the import documentation.", test, test1);

			Step_Start(11, "once saved the import documentation system has validate import documentation is saved.", test, test1);
		...
		Step_End(11, "once saved the import documentation system has validate import documentation is saved.", test, test1);

		Step_Start(12, "Verify that Captured charges Must be there under HBL No in the collect charges tab.", test, test1);
		...
		for (String hbl_Charge : BL_SplitCharge.keySet()) {
			...
			Map<String,String> Exp_CollectionCharge=BL_SplitChargeAmount.get(hbl_Charge);
		...
		Step_End(12, "Verify that Captured charges Must be there under HBL No in the collect charges tab.", test, test1);
