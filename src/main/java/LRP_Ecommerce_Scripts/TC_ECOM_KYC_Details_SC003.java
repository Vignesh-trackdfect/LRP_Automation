...
public class TC_ECOM_KYC_Details_SC003 extends Keywords {

	public void ECOM_KYC_Details_SC003(WebDriver driver, ExtentTest test, ExtentTest test1, String selected_dataset,Map<String, String> Excel_data,String url) {

		String testcase_Name = "TC_ECOM_KYC_Details_SC003";
		...
		String Country_Details_Update = Excel_data.get("Country_Details_Update");

		String Field_Name_Change = Excel_data.get("Field_Name_Change");
		String Mandatory_Type_Change = Excel_data.get("Mandatory_Type_Change");
		String KYC_details_Updated_popup = Excel_data.get("KYC_details_Updated_popup");
		...
		newButton(driver);
		Step_Start(1, " Select the record and click on it to set it to the respective field.", test, test1);

		waitForElement(driver, Country_Filter_Input_KYC_Details);
		sendKeys(driver, Country_Filter_Input_KYC_Details, Country_Details_Update);

		String Country_To_Edit = String.format(KYC_Detail_Table_Row_Edit, Country_Details_Update);
		waitForElement(driver, Country_To_Edit);
		doubleClick(driver, Country_To_Edit);
		Step_End(1, " Select the record and click on it to set it to the respective field.", test, test1);

		Step_Start(2, " Click on the \"Edit\" option in the toolbar.", test, test1);

		waitForElement(driver, Edit_Button_toolBar);
		click(driver, Edit_Button_toolBar);
		Step_End(2, " Click on the \"Edit\" option in the toolbar.", test, test1);

		Step_Start(3, " Modify the required field(s) as needed.", test, test1);
		if (!Field_Name_Change.equals("")) {
			waitForElement(driver, FieldName_Input_KYC_Details);
			Newclear(driver, FieldName_Input_KYC_Details);
			sendKeys(driver, FieldName_Input_KYC_Details, Field_Name_Change);
		} else {
			Field_Name_Change = getAttribute(driver, FieldName_Input_KYC_Details, "value");
		}

		waitForElement(driver, Mandatory_Fields_KYC_Details);
		click(driver, Mandatory_Fields_KYC_Details);

		String MandatorySelect = String.format(Mandatory_Field_Select_KYC_Details, Mandatory_Type_Change);
		waitForElement(driver, MandatorySelect);
		click(driver, MandatorySelect);
		Step_End(3, " Modify the required field(s) as needed.", test, test1);

		Step_Start(4, " Click on the \"Save\" button.", test, test1);
		waitForElement(driver, SaveButton_ToolBar);
		click(driver, SaveButton_ToolBar);

		waitForPopup(driver, Popup_Message, KYC_details_Updated_popup);
		String Act_popup = getText(driver, Popup_Message);
		if (KYC_details_Updated_popup.equals(Act_popup)) {
			System.out
					.println("Popup Matched || Expected : " + KYC_details_Updated_popup + " || Actual : " + Act_popup);
			Extent_pass_New(driver,
					"Popup Matched || Expected : " + KYC_details_Updated_popup + " || Actual : " + Act_popup, test,
					test1);
		} else {
			System.out.println(
					"Popup Not Matched || Expected : " + KYC_details_Updated_popup + " || Actual : " + Act_popup);
			Extent_fail(driver,
					"Popup Not Matched || Expected : " + KYC_details_Updated_popup + " || Actual : " + Act_popup, test,
					test1);
		}
		Step_End(4, " Click on the \"Save\" button.", test, test1);

		Step_Start(5, " The updated details should be displayed in the list below.", test, test1);
		...
		sendKeys(driver, FieldName_Filter_Input_KYC_Details, Field_Name_Change);

		waitForElement(driver, Mandatory_Filter_Input_KYC_Details);
		sendKeys(driver, Mandatory_Filter_Input_KYC_Details, Mandatory_Type_Change);

		if (isDisplayed(driver, KYC_Details_Table_Column)) {
			System.out.println(
					"Expected : Updated KYC Details Should be Added in the Table || Actual : Updated KYC Details Added in the Table Successfully");
			Extent_pass_New(driver,
					"Expected : Updated KYC Details Should be Added in the Table || Actual : Updated KYC Details Added in the Table Successfully",
					test, test1);
		} else {
			System.out.println(
					"Expected : Updated KYC Details Should be Added in the Table || Actual : Updated KYC Details not Added in the Table");
			Extent_fail(driver,
					"Expected : Updated KYC Details Should be Added in the Table || Actual : Updated KYC Details not Added in the Table",
					test, test1);
		}

		Step_End(5, " The updated details should be displayed in the list below.", test, test1);

		Step_Start(6, " The saved details should be mandatory for Ecom registration.", test, test1);
		...
		click(driver, Next_Button_EComm_Register);
...
//		scrollBottom(driver);
		...
		String Mandatory_File = String.format(Mandatory_Field_Ecomm_Register, Field_Name_Change);
		waitForDisplay(driver, Mandatory_File);
		if (Mandatory_Type_Change.equalsIgnoreCase("Yes")) {
			if (isDisplayed(driver, Mandatory_File)) {
				System.out.println("Expected : " + Field_Name_Change
				...
				Extent_pass_New(driver, "Expected : " + Field_Name_Change
				...
				Extent_fail(driver, "Expected : " + Field_Name_Change
						+ "  Field Name should present as Mandatory in the Ecomm Registration  When it is Yes in the KYC Details || Actual : "
						+ Field_Name_Change
						+ "' Field is not present as Mandatory in the Ecomm Registration  When it is Yes in the KYC Details",
						test, test1);
		...
		Step_End(6, " The saved details should be mandatory for Ecom registration.", test, test1);
