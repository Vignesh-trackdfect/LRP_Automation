...
public class TC_Feeder_Contract_SC22 extends Keywords {
	public void Feeder_Contract_SC22(WebDriver driver, ExtentTest test, ExtentTest test1, String selected_dataset,
			Map<String, String> Excel_data, String url) {
		String testcase_Name = "TC_Feeder_Contract_SC22";
		...
		Step_Start(6, "Select the different currency to click search .", test, test1);

		waitForElement(driver, FC_CurrencySearch);
		click(driver, FC_CurrencySearch);

		twoColumnMultipleSearchWindow(driver, Currency_Code_Header, condition, Currency_Code);

		waitForElement(driver, FC_Currency_Input);
		String Pickup_Location_Value_Act1 = getAttribute(driver, FC_Currency_Input, "value");

		if (Currency_Code.equals(Pickup_Location_Value_Act1)) {
			System.out.println("|| Matched : Expected Result Taken from test data :" + Currency_Code
					+ " || Actual Result taken from application After change the Currency value : "
					+ Pickup_Location_Value_Act1);
			Extent_pass_New(driver,
					"|| Matched : Expected Result Taken from test data :" + Currency_Code
							+ " ||Actual Result taken from application After change the  Currency value : "
							+ Pickup_Location_Value_Act1,
					test, test1);

		} else {
			System.out.println("|| NotMatched Expected Result Taken from test data :" + Currency_Code
					+ " || Actual Result taken from application After change the  Currency value: "
					+ Pickup_Location_Value_Act1);
			Extent_fail(driver,
					"|| NotMatched Expected Result Taken from test data :" + Currency_Code
							+ " || Actual Result taken from application After change the  Currency value : "
							+ Pickup_Location_Value_Act1,
					test, test1);
		}

		Step_End(6, "Select the different currency to click search .", test, test1);
