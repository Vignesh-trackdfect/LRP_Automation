...
public class TC_Import_Documentation_SC064D extends Keywords {

	public void Import_Documentation_SC064D(WebDriver driver, ExtentTest test, ExtentTest test1, String selected_dataset,Map<String, String> Excel_data,String url) {

		String testcase_Name="TC_Import_Documentation_SC064D";
		...
		if(!checkboxSelected.contains("active")) {
			...
			waitForElement(driver, recieptNo_Textfield);
			String actual_Receipt_Date = getAttribute(driver, ReceiptDate_Input, "value");
			
			scrollTop(driver);
			waitForElement(driver, Close_Current_tab);
			click(driver,Close_Current_tab);
			Step_End(8, "saved the receipt .", test, test1);	
			Step_Start(9, "open the import documentation, Load the BL no , ", test, test1);	

			waitForElement(driver, SearchButton_Toolbar);
			click(driver, SearchButton_Toolbar);

			globalValueSearchWindow(driver, BL_Condition, ImportDoc_Select_Type1, BL_Num, ImportDoc_Select_Type2, ImportDoc_Select_Value2, ImportDoc_Select_Type3, ImportDoc_Select_Value3);
			Step_End(9, "open the import documentation, Load the BL no , ", test, test1);	

			Step_Start(10, "Go to Remarks tab,import invoice and receipt no check box is checked and invoiceno and receipt no also showing", test, test1);	
			...
			String Ref_value_Import=getValueByFirstColumnAndHeader(RemarksTable_Data, Remarks_Tab_Refer_Head, "Import Invoice Receipt", "Reference Value");
			String RefDate_Import=getValueByFirstColumnAndHeader(RemarksTable_Data, Remarks_Tab_Refer_Head, "Import Invoice Receipt", "Reference Date");

			if (actual_Receipt_No.equals(Ref_value_Import)) {
				System.out.println("Matched || Expected value was : " + actual_Receipt_No + " || Actual value was : " + Ref_value_Import);
				Extent_pass(driver, "Matched || Expected value was : " + actual_Receipt_No + " || Actual value was : " + Ref_value_Import, test,
						test1);
			} else {
				System.out.println("NotMatched || Expected value was : " + actual_Receipt_No + " || Actual value was : " + Ref_value_Import);
				Extent_fail(driver, "NotMatched || Expected value was : " + actual_Receipt_No + " || Actual value was : " + Ref_value_Import,
						test, test1);
			}
			
			if (CompareDateValues(actual_Receipt_Date,RefDate_Import)) {
				System.out.println("Matched || Expected value was : " + actual_Receipt_Date + " || Actual value was : " + RefDate_Import);
				Extent_pass(driver, "Matched || Expected value was : " + actual_Receipt_Date + " || Actual value was : " + RefDate_Import, test,
						test1);
			} else {
				System.out.println("NotMatched || Expected value was : " + actual_Receipt_Date + " || Actual value was : " + RefDate_Import);
				Extent_fail(driver, "NotMatched || Expected value was : " + actual_Receipt_Date + " || Actual value was : " + RefDate_Import,
						test, test1);
			}
			Step_End(10, "Go to Remarks tab,import invoice and receipt no check box is checked and invoiceno and receipt no also showing", test, test1);	

		}else {
			Extent_call(test, test1, "Verify that Import Invoice and receipt no  Check Box Button is Checked and invoiceno and receipt no also showing");
			waitForElement(driver, remarksTab_Table_Row_IMP_DOC);
			List<Map<String, String>>  RemarksTable_Data=getTableData(driver,remarksTab_Table_Header_IMP_DOC,remarksTab_Table_Row_IMP_DOC);
			...
			selectByText(driver, type_Select1, Receipt_SelectType);
			...
			waitForElement(driver, Receipt_Filter_Input);
			sendKeys(driver, Receipt_Filter_Input, Ref_value_Import);
			...
			if (CompareDateValues1(actual_Receipt_Date,RefDate_Import)) {
				System.out.println("Matched || Expected value was : " + actual_Receipt_Date + " || Actual value was : " + RefDate_Import);
				Extent_pass(driver, "Matched || Expected value was : " + actual_Receipt_Date + " || Actual value was : " + RefDate_Import, test,
						test1);
			} else {
