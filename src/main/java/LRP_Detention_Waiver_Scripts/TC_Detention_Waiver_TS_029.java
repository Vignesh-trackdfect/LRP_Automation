...
public class TC_Detention_Waiver_TS_029 extends Keywords {

	public void Detention_Waiver_TS_029(WebDriver driver, ExtentTest test, ExtentTest test1, String selected_dataset,Map<String, String> Excel_data,String url)
			throws Throwable {

		String testcase_Name = "TC_Detention_Waiver_TS_029";
		...
		Step_Start(5, "Verify that waiver Reason New Search option is listed", test, test1);
		waitForDisplay(driver, DW_Waiver_Reason_Input_Field);
		if (isDisplayed(driver, DW_Waiver_Reason_Input_Field)) {
			System.out.println(
					"Matched || Expected result : Waiver reason search Should be displayed || Actual result is : waiver Reason New Search option is listed");
			Extent_pass_New(driver,
					"Matched || Expected result : Waiver reason search Should be displayed || Actual result is :  waiver Reason New Search option is listed",
					test, test1);
		} else {
			System.out.println(
					"Not Matched || Expected result : Waiver reason search Should be displayed || Actual result is :  waiver Reason New Search option is Not listed");
			Extent_fail(driver,
					"Not Matched || Expected result : Waiver reason search Should be displayed || Actual result is :  waiver Reason New Search option is Not listed",
					test, test1);
		}

		Step_End(5, "Verify that waiver Reason New Search option is listed", test, test1);
