...
//Check whether while select the Rates based on that system will insert the commodity details in commodity tab
public class TC_Booking_SC034 extends Keywords {

	public void Booking_SC034(WebDriver driver, ExtentTest test, ExtentTest test1, String selected_dataset,Map<String, String> Excel_data,String url) {

		String test_Case_Name = "TC_Booking_SC034";
		...
		Step_Start(5, "System set the commodity details in commodity tab ", test, test1);
		waitForElement(driver, commodity_tab);
		safeclick(driver, commodity_tab);
		scrollBottom(driver);
		waitForDisplay(driver, Commodity_Grid_EditBtn);
		if (isDisplayed(driver, Commodity_Grid_EditBtn)) {
			System.out.println("Matched || Expected Result is : Commodity grid Should be display || Actual Result is : Commodity grid is displayed");
			Extent_pass_New(driver,"Matched || Expected Result is : Commodity grid Should be display || Actual Result is : Commodity grid is displayed", test, test1);
		} else {
			System.out.println("Not Matched || Expected Result is : Commodity grid Should be display || Actual Result is : Commodity grid is Not displayed");
			Extent_fail(driver,"Not Matched || Expected Result is : Commodity grid Should be display || Actual Result is : Commodity grid is Not displayed", test, test1);
		}

		List<Map<String, String>> Commodity_Data = getTableData(driver, Commodity_Tab_Grid_Header, Commodity_Tab_Grid_Row);
		String Commodity_Eqp_Type = getFirstRowValueByHeaderFromTableData(Commodity_Data, "Eqp. Type");
		System.out.println("Commodity_Eqp_Type :"+Commodity_Eqp_Type);
		if (Eqp_Type_Value.equals(Commodity_Eqp_Type)) {
			System.out
			.println("Matched || Expected value was : " + Eqp_Type_Value + " || Actual value was : " + Commodity_Eqp_Type);
			Extent_pass_New(driver,
					"Matched || Expected value was : " + Eqp_Type_Value + " || Actual value was : " + Commodity_Eqp_Type, test,
					test1);
		} else {
			System.out.println(
					"NotMatched || Expected value was : " + Eqp_Type_Value + " || Actual value was : " + Commodity_Eqp_Type);
			Extent_fail(driver,
					"NotMatched || Expected value was : " + Eqp_Type_Value + " || Actual value was : " + Commodity_Eqp_Type, test,
					test1);
		}
		Step_End(5, "System set the commodity details in commodity tab ", test, test1);
