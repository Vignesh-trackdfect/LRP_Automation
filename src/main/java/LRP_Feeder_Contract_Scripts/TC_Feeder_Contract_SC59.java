...
public class TC_Feeder_Contract_SC59 extends Keywords {

	public void Feeder_Contract_SC59(WebDriver driver, ExtentTest test, ExtentTest test1, String selected_dataset,
			Map<String, String> Excel_data, String url) throws AWTException {

		String testcase_Name = "TC_Feeder_Contract_SC59";
		...
		String SurchargeAmounts = Excel_data.get("SurchargeAmounts");
		...
		String ChargeTypes_DG = Excel_data.get("ChargeTypes_DG");
		String Consonant_Input_Slab = Excel_data.get("Consonant_Input_Slab");
		String FormulaGrid_Header = Excel_data.get("FormulaGrid_Header");
		...
		List<String> SurchargeAmount = splitAndExpand(SurchargeAmounts);

		int i1 = 0;

		for (String charge_Type : given_ChargeTypes) {

			waitForDisplay(driver, FC_ChargeType_Dropdown);
			formatLocatorjsClick(driver, FC_ChargeType_Dropdown, String.valueOf(i1));

			formatLocatorClick(driver, DropDown_Select, charge_Type);

			if (charge_Type.equals("LumpSum") || charge_Type.equals("Percentage")) {

				String AmountInput = String.format(FC_Amount_Input, i1);

				doubleClick(driver, AmountInput);

				sendKeysUsingRobot(SurchargeAmount.get(i1));

				Tab(driver);

			} else {

				formatLocatorClick(driver, FC_SurchargeRowSelect, String.valueOf(i1));

				waitForElement(driver, FC_FormulaBuilder);
				click(driver, FC_FormulaBuilder);
				...
				if (Formula_Build_Option.equals("By_Sendkeys")) {
					...
					twoColumnSearchWindowMultipleValue(driver, Formula_Variable_Header, Two_Column_Condition,
							Formula_Variable);
				...
				waitForElement(driver, Formula_Input_FormulaBuilder_VC);
				String addedformula = getText(driver, Formula_Input_FormulaBuilder_VC);

				waitForElement(driver, Formula_Ok_Btn_VC);
				click(driver, Formula_Ok_Btn_VC);

				waitForElement(driver, FC_SurchargeTable_Header);

				List<Map<String, String>> Surcharge_Grid = getTableData(driver, FC_SurchargeTable_Header,
						FC_SurchargeTable_value);

				List<String> Formulas = new ArrayList<>();

				for (Map<String, String> row : Surcharge_Grid) {
					String formulaa = row.get(FormulaGrid_Header); // Use the exact column name as it appears in the
																	// table
					if (formulaa != null && !formulaa.isEmpty()) {
						Formulas.add(formulaa);
					}
				}

				String gridformula = Formulas.get(j);

				if (addedformula.equals(gridformula)) {

					System.out.println("Expected result : The given formula " + addedformula
							+ " should be added in the grid || Actual result :  The given formula " + gridformula
							+ " was in the grid");
					Extent_pass_New(driver,
							"Expected result : The given formula " + addedformula
									+ " should be added in the grid || Actual result :  The given formula "
									+ gridformula + " was in the grid",
							test, test1);
				} else {

					System.out.println("Expected result : The given formula " + addedformula
							+ " should be added in the grid || Actual result :  Different formula " + gridformula
							+ " was in the grid");
					Extent_fail(driver,
							"Expected result : The given formula " + addedformula
									+ " should be added in the grid || Actual result :  Different formula "
									+ gridformula + " was in the grid",
							test, test1);
		...
		List<String> given_ChargeTypes1 = splitAndExpand(ChargeTypes_DG);
		List<String> DGAmount = splitAndExpand(DGClass_Amount);

		int k = 0;

		for (String charge_Type : given_ChargeTypes1) {
		...
		if (DG_Popup.equals(ActualDG_Popup)) {
			...
			System.out.println("Matched || Expected popup message was : " + DG_Popup + " || Actual popup message was : "
					+ ActualDG_Popup);
			Extent_pass_New(driver, "Matched || Expected popup message was : " + DG_Popup
					+ " || Actual popup message was : " + ActualDG_Popup, test, test1);
		} else {
		...
		Step_End(24, "Then click the save in the Tool bar ,Details should be saved. ", test, test1);
		...
		Step_Start(26, "After approve the details and Click the Reject,The details should reject.", test, test1);

		waitForElement(driver, AG_Grid_First_Row);
		click(driver, AG_Grid_First_Row);

		waitForElement(driver, FC_Approve_btn);
		click(driver, FC_Approve_btn);
		...
		waitForElement(driver, FC_Reject_btn);
		click(driver, FC_Reject_btn);

		waitForPopup(driver, popup_Message, Reject_popup);

		String Actualrejected_popup = getText(driver, popup_Message);

		if (Reject_popup.equals(Actualrejected_popup)) {

			System.out.println("Matched || Expected popup message was : " + Reject_popup
					+ " || Actual popup message was : " + Actualrejected_popup);
			Extent_pass_New(driver, "Matched || Expected popup message was : " + Reject_popup
					+ " || Actual popup message was : " + Actualrejected_popup, test, test1);
		} else {

			System.out.println("Not Matched || Expected popup message was : " + Reject_popup
					+ " || Actual popup message was : " + Actualrejected_popup);
			Extent_fail(driver, "Not Matched || Expected popup message was : " + Reject_popup
					+ " || Actual popup message was : " + Actualrejected_popup, test, test1);
		...
		Step_End(26, "After approve the details and Click the Reject,The details should reject.", test, test1);
