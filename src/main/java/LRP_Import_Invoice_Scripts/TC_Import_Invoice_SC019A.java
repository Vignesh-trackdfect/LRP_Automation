...
public class TC_Import_Invoice_SC019A extends Keywords {
	public void Import_Invoice_SC019A(WebDriver driver, ExtentTest test, ExtentTest test1, String selected_dataset,Map<String, String> Excel_data,String url) {

		String testcase_Name = "TC_Import_Invoice_SC019A";
		...
		if (BL_Retrive_Option.equalsIgnoreCase("By_BL")) {
			...
			if (!Customer_Code_from_Import_Doc.equalsIgnoreCase("Yes")) {
				...
				Step_Start(12, "Click on Customer search button", test, test1);

				waitForElement(driver, customer_Search_Icon);
				click(driver, customer_Search_Icon);

				Step_End(12, "Click on Customer search button", test, test1);

				Step_Start(13, "Enter the Customer via Test Data and Add by click on + button it", test, test1);
			...
			Step_End(13, "Enter the Customer via Test Data and Add by click on + button it", test, test1);
			...
			for (int i = 0; i < Tab_Names.size(); i++) {
				...
				if (Tab_Names.get(i).equals("Manifest Charges")) {
					Step_Start(15, "Select the charge codes it can be multiple as well", test, test1);
					...
					for (int j = 0; j < Charge_Codes.size(); j++) {
						...
						Step_End(15, "Select the charge codes it can be multiple as well", test, test1);
						Step_Start(16, "right click and select the newly added customer based category", test, test1);
						waitForElement(driver, charge_Code_Select);
						...
						Step_End(16, "right click and select the newly added customer based category", test, test1);
						Step_Start(17, "make sure that newly assign charge code moved to recently added customer", test,
								test1);
						...
						Step_End(17, "make sure that newly assign charge code moved to recently added customer", test,
								test1);
					}
				}
				...
				if (Tab_Names.get(i).equals("Detention Charges")) {
					...
					for (int j = 0; j < Charge_Codes.size(); j++) {
						...
						for (int k = 1; k <= detention_Rows.size(); k++) {
							...
							Step_Start(16, "right click and select the newly added customer based category", test,
									test1);
							waitForElement(driver, row_Select);
						...
						for (int k = 0; k < new_Customer_Codes.size(); k++) {
							if (CustomerCode.equals(new_Customer_Codes.get(k))) {
								System.out.println("Matched || Expected Customer Code : " + CustomerCode
										+ " || Actual Customer Code : " + new_Customer_Codes.get(k));
								Extent_pass_New(driver,
										"Matched || Expected Customer Code : " + CustomerCode
												+ " || Actual Customer Code : " + new_Customer_Codes.get(k),
										test, test1);
			...
			Step_Start(19, "Import invoice save msg received and click ok button", test, test1);
			...
			waitForElement(driver, popup_Message_Ok_Button);
			click(driver, popup_Message_Ok_Button);
			Step_End(19, "Import invoice save msg received and click ok button", test, test1);
			for (int i = 0; i < Tab_Names.size(); i++) {
				Step_Start(20, "Go to Manifest/Non Manifest/Detention charges /Storage charges Tab", test, test1);
				String tab_Select = String.format(import_Invoice_Tab, Tab_Names.get(i));
				waitForElement(driver, tab_Select);
				click(driver, tab_Select);
				Step_End(20, "Go to Manifest/Non Manifest/Detention charges /Storage charges Tab", test, test1);
				if (Tab_Names.get(i).equals("Manifest Charges")) {
					scrollBottom(driver);
