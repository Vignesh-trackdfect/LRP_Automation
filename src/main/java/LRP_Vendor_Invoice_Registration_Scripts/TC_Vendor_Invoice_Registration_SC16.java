...
public class TC_Vendor_Invoice_Registration_SC16 extends Keywords{
	
	public void Vendor_Invoice_Registration_SC16(WebDriver driver, ExtentTest test,ExtentTest test1,String selected_Excel_dataset,Map<String, String> Excel_data,String url) {
		
		String testCaseName="TC_Vendor_Invoice_Registration_SC16";
		//  Get data from excel sheet	
			...
			String Voyage_Bound_Header = Excel_data.get("Voyage_Bound_Header");
			String Voyage_Bound_Input = Excel_data.get("Voyage_Bound_Input");
			...
			String Port_Input = Excel_data.get("Port_Code");
			...
			String AG_Grid_Column_Headers_CAR = Excel_data.get("AG_Grid_Column_Headers_CAR");
...
//			Vendor Invoice Registration
		...
		// Vendor invoice number		
			...
			Step_Start(5, "Select the Contract type as MSC.", test, test1);
			...
			Step_End(5, "Select the Contract type as MSC.", test, test1);
			Step_Start(6, "Select the required Service,Vessel,Voyage,Port and Terminal.", test, test1);

			waitForElement(driver, service_SearchButton_VIR);
			click(driver, service_SearchButton_VIR);
			
			twoColumnSearchWindow(driver, Service_Header, condition, Service_Code_Input);

			waitForElement(driver, vessel_SearchButton_VIR);
			click(driver, vessel_SearchButton_VIR);
			
			twoColumnSearchWindow(driver, Vessel_Header, condition, Vessel_Code_Input);

			waitForElement(driver, voyage_SearchButton_VIR);
			click(driver, voyage_SearchButton_VIR);
			
			twoColumnMultipleSearchWindow(driver, Voyage_Bound_Header, condition, Voyage_Bound_Input);
			
			waitForElement(driver, port_SearchButton_VIR);
			click(driver, port_SearchButton_VIR);
			
			twoColumnSearchWindow(driver, Port_Header, condition, Port_Input);
			...
			Step_End(6, "Select the required Service,Vessel,Voyage,Port and Terminal.", test, test1);
			...
			Step_Start(9, "Copy the Activity No. form the AG grid by referring the Activity No. column.", test, test1);

			String activityNo_VIR=getText(driver, activity_No_Gridcell_VIR);
			System.out.println("activityNo_VIR : "+activityNo_VIR);
			
			Step_End(9, "Copy the Activity No. form the AG grid by referring the Activity No. column.", test, test1);
			Step_Start(10, "Open the Cost Activity Reporting module by using the module search.", test, test1);

			scrollTop(driver);
			moduleNavigate(driver, CAR_module);
			
			Step_End(10, "Open the Cost Activity Reporting module by using the module search.", test, test1);
			Step_Start(11, ".Click on the Global search button,a new tab will open.", test, test1);

			waitForElement(driver, SearchButton_Toolbar);
			click(driver, SearchButton_Toolbar);
			
			Step_End(11, ".Click on the Global search button,a new tab will open.", test, test1);
			Step_Start(12, "Paste the copied CAR No. in the CAR No. field and click on the search button", test, test1);
			Step_Start(13, "Our CAR no. will be automatically select in the tab.", test, test1);
			Step_Start(14, "Click on the select button.", test, test1);

			globalValueSearchWindow(driver, dropdownCondition, searchBy_CAR, activityNo_VIR, "", "", "", "");
			
			Step_End(12, "Paste the copied CAR No. in the CAR No. field and click on the search button", test, test1);
			Step_End(13, "Our CAR no. will be automatically select in the tab.", test, test1);
			Step_End(14, "Click on the select button.", test, test1);
			Step_Start(15, "Copy the reference no. in the AG grid for  the records by referring the Reference no. column and also it should not invoiced by referring the Invoice No. column and it should be either Manually approved or Auto approved by referring its status column.", test, test1);

			reArrangeAG_GridColumns(driver, AG_Grid_Column_Headers_CAR);
			waitForElement(driver, AG_Grid_Table_CAR);
			List<Map<String, String>> CAR_AG_Grid_Data = extractTableDataByColumn(driver, AG_Grid_Table_CAR);
			
			List<String> RefNo_Values_CAR = getValuesByHeader(CAR_AG_Grid_Data, ref_No_Value.get(1));
			...
			Step_Start(16, "Then click on the Vendor invoice registration module.", test, test1);

			waitForElement(driver, Close_Current_tab);
			jsClick(driver, Close_Current_tab);
			
			Step_End(16, "Then click on the Vendor invoice registration module.", test, test1);
			Step_Start(17, "Check whether the copied reference no. are available in the AG gid,by referring the Activity Ref. No column.", test, test1);
			...
			Step_End(17, "Check whether the copied reference no. are available in the AG gid,by referring the Activity Ref. No column.", test, test1);
			Step_Start(18, "Click on the save button which is available in the Tool bar.", test, test1);

			scrollTop(driver);
			waitForElement(driver, SaveButton_ToolBar);
			jsClick(driver, SaveButton_ToolBar);
			
			Step_End(18, "Click on the save button which is available in the Tool bar.", test, test1);
			Step_Start(19, "Check whether the system prompts a message as \"Vendor Invoice saved\".", test, test1);
			...
			Step_End(19, "Check whether the system prompts a message as \"Vendor Invoice saved\".", test, test1);
