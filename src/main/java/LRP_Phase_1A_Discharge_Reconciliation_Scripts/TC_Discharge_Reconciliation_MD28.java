...
public class TC_Discharge_Reconciliation_MD28 extends Keywords{
	
public void Discharge_Reconciliation_MD28(WebDriver driver,ExtentTest test,ExtentTest test1,String selected_dataset,Map<String, String> Excel_data,String url) {
		
		String testcase_Name="TC_Discharge_Reconciliation_MD28";
		...
		Step_Start(9, "Go to Booking without BL section", test, test1);
		
		Step_Start(10, "Capture the BLs List and store it", test, test1);
	    
	        int j=0;
	        Set<String>  newValue1=new HashSet<String>();
	        String blno=String.format(DR_Bbookno, j);    
	        ...
	        while(isdisplayed(driver, blno)) {
	        	...
	        	if(BLNum1!=null&& !BLNum1.isEmpty()) {
		        	newValue1.add(BLNum1);
	        	}
	        	j++;
	        	blno=String.format(DR_Bbookno, j);    
	        }
	        
	        int bl_num=newValue1.size();
        ...
        Step_End(10, "Capture the BLs List and store it", test, test1);
		
		Step_End(9, "Go to Booking without BL section", test, test1);
        ...
        Step_Start(16, "Verify that  BLs not available in Operation List view and Non-Container summary", test, test1);
 
        int i=0;
        Set<String>  newValue11=new HashSet<String>();
        ...
        while(isdisplayed(driver, blnums)) {
            ...
            String booknums=String.format(OLV_ManifiesBook_Numbers, i);    
            String bookno=getText(driver, booknums);
        	if(BLNum.isEmpty()) {
	        	newValue11.add(bookno);
        	}
        	i++;
        	blnums=String.format(OLV_ManifiesBL_Numbers, i);    
        }
        
        int BL_Num_Cout=newValue11.size();
        
        if (bl_num == BL_Num_Cout) {
            System.out.println("Matched || Expected Booking Count in Discharge Reconcillation : " + bl_num + " || Actual Booking count in Operation List View: " + BL_Num_Cout);
            Extent_pass(driver, "Matched || Expected Booking Count in Discharge Reconcillation : " + bl_num + " || Actual Booking count in Operation List View: " + BL_Num_Cout, test, test1);
            Extent_pass(driver, "Booking Numbers in Discharge Reconcillation : " + newValue1 + "       ||     Booking Numbers in Operation List View: " + newValue11, test, test1);
            
        } else {
        	 System.out.println("Not Matched || Expected Booking Count in Discharge Reconcillation : " + bl_num + " || Actual Booking count in Operation List View: " + BL_Num_Cout);
             Extent_pass(driver, "Booking Numbers in Discharge Reconcillation : " + newValue1 + "       ||     Booking Numbers in Operation List View: " + newValue11, test, test1);
        	 Extent_fail(driver, "Not Matched || Expected Booking Count in Discharge Reconcillation : " + bl_num + " || Actual Booking count in Operation List View: " + BL_Num_Cout, test, test1);
           
        }
		
 
		Step_End(16, "Verify that  BLs not available in Operation List view and Non-Container summary", test, test1);
