...
public class TC_Booking_SC017 extends Keywords {

	///// Check whether Tool bar contain both Search Button Working as per Requirement
	public void Booking_SC017(WebDriver driver, ExtentTest test, ExtentTest test1, String selected_dataset,Map<String, String> Excel_data,String url)throws Exception {
		String test_Case_Name="TC_Booking_SC017";
		...
		Step_Start(8, "Retrieve The Record In Local Search", test, test1);
		//Local search	
		scrollTop(driver);
		...
		String retrievedBookingNo=getAttribute(driver, Booking_No_Field, "value");
		if(retrievedBookingNo.equals(booknum)) {
			Extent_pass_New(driver, "Matched || booking number is retrieved by Local Search || Expected : "+booknum+" || Actual : "+retrievedBookingNo, test,test1);
			System.out.println("Matched || booking number is retrieved by Local Search || Expected : "+booknum+" || Actual : "+retrievedBookingNo);
		}else {
			System.out.println("Not Matched || booking number is not retrieved by Local Search || Expected : "+booknum+" || Actual : "+retrievedBookingNo);
			Extent_fail(driver, "Not Matched || booking number is not retrieved by Local Search || Expected : "+booknum+" || Actual : "+retrievedBookingNo, test,test1);
		}
		Step_End(8, "Retrieve The Record In Local Search", test, test1);
		//Global search
		Step_Start(9, " Retrieve The Record In Global Search ", test, test1);
		scrollTop(driver);
		waitForDisplay(driver, NewButton_ToolBar);	
		if(isDisplayed(driver, NewButton_ToolBar) && isElementAccessible(driver, NewButton_ToolBar)) {
			click(driver, NewButton_ToolBar);
		...
		if (retrievedBookingNoGlobal.equals(booknum)) {
			Extent_pass_New(driver, "Matched || booking number is retrieved by Global Search || Expected :" + booknum + " || Actual :" + retrievedBookingNoGlobal, test,test1);
			System.out.println("Matched || booking number is retrieved by Global Search || Expected :" + booknum + " || Actual :" + retrievedBookingNoGlobal);
		} else {
			System.out.println("Not Matched || booking number is not retrieved by Global Search || Expected :" + booknum + " || Actual :" + retrievedBookingNoGlobal);
			Extent_fail(driver, "Not Matched || booking number is not retrieved by Global Search || Expected :" + booknum + " || Actual :" + retrievedBookingNoGlobal, test,test1);
		}
		Step_End(9, " Retrieve The Record In Global Search ", test, test1);
