...
public class TC_Import_Invoice_SC028 extends Keywords {

	public void Import_Invoice_SC028(WebDriver driver, ExtentTest test, ExtentTest test1, String selected_dataset,Map<String, String> Excel_data,String url) {
		String testcase_Name = "TC_Import_Invoice_SC028";
		...
		Step_Start(2, "open the accounting configuration", test, test1);
		...
		Step_Start(12, "open the import invoice", test, test1);
		...
		Step_End(12, "open the import invoice", test, test1);

		Step_Start(13, "receipt button is not available in import invoice", test, test1);

		waitForDisplay(driver, II_receipt_button);
		if (!isdisplayed(driver, II_receipt_button)) {
			System.out.println(
					"Pass || Expected result is : Receipt button not available in Import invoice || Actual result is : Receipt button not available in Import invoice");
			Extent_pass_New(driver,
					"Pass || Expected result is : Receipt button not available in Import invoice || Actual result is : Receipt button not available in Import invoice",
					test, test1);
			Extent_call(test, test1, "Receipt button was not present in the import invoice");
			waitForElement(driver, Close_Current_tab);
			click(driver, Close_Current_tab);
		} else {
			System.out.println(
					"fail || Expected result is : Receipt button not available in Import invoice || Actual result is : Receipt button  available in Import invoice");
			Extent_fail(driver,
					"fail || Expected result is : Receipt button not available in Import invoice || Actual result is : Receipt button  available in Import invoice",
					test, test1);
		}

		Step_End(13, "receipt button is not available in import invoice", test, test1);
