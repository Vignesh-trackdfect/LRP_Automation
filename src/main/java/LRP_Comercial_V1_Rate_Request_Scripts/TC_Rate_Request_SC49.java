...
public class TC_Rate_Request_SC49 extends Keywords{
	...
	public void Rate_Request_SC49(WebDriver driver, ExtentTest test, ExtentTest test1, String selected_dataset,Map<String, String> Excel_data,String url){
		...
		String ofr_data_1 = Excel_data.get("ofr_data_1");
		...
		String Exp_Det_Days_Input_1 = Excel_data.get("Exp_Det_Days_Input_1");
		String Imp_Det_Days_Input_1 = Excel_data.get("Imp_Det_Days_Input_1");
		String Exp_Demmurage_Input_1 = Excel_data.get("Exp_Demmurage_Input_1");
		String Imp_Demmurage_Input_1 = Excel_data.get("Imp_Demmurage_Input_1");
		String remarks_txtfld_data = Excel_data.get("remarks_txtfld_data");
		String port_pair_Status_exp=Excel_data.get("port_pair_Status");
...
if(isdisplayed(driver, empty_Checkbox_RR)) {
	...
	formatLocatorClick(driver, on_Carriage_Radiobtn, On_Carriage_Radiobtn);
			...
			if(actSubmitPopup.equals(wantToSavePopup)) {
				Extent_pass_New(driver,"Matched || Expected : "+wantToSavePopup+" Actual : "+ actSubmitPopup, test,test1);
			...
			if(actualPopup.equals(wantToProceedPopup)) {
				...
				Extent_fail(driver,"Not Matched || Expected : popup message should be shown as : "+wantToProceedPopup+" Actual :popup message is not shown"+ actualPopup, test,test1);
			}
		...
		click(driver, Mail_Cancel_button);
			...
			if(srr_Status_actual.equals(srr_Status_exp)) {
				...
				Extent_pass_New(driver, "Matched || Expected srr status : "+srr_Status_exp+"|| actual srr status is: "+srr_Status_actual, test, test1);
			}else {
			...
			if(port_pair_Status_actual.equals(port_pair_Status_exp)) {
				...
				Extent_pass_New(driver, "Matched || Expected port pair status : "+port_pair_Status_exp+"|| actual srr status is: "+port_pair_Status_actual, test, test1);
			}else {
			...
			if(exp_det_actual.equals(Exp_Det_Days_Input_1)) {
				...
				Extent_pass_New(driver, "Matched || Expected export detention days -> "+Exp_Det_Days_Input_1+"|| actual export detention days -> "+exp_det_actual, test, test1);
			}else {
			...
			if(imp_det_actual.equals(Imp_Det_Days_Input_1)) {
				...
				Extent_pass_New(driver, "Matched || Expected import detention days -> "+Imp_Det_Days_Input_1+"|| actual import detention days -> "+imp_det_actual, test, test1);
			}else {
			...
			if(exp_demm_actual.equals(Exp_Demmurage_Input_1)) {
				...
				Extent_pass_New(driver, "Matched || Expected export demmurage days -> "+Exp_Demmurage_Input_1+"|| actual export demmurage days -> "+exp_demm_actual, test, test1);
			}else {
			...
			if(imp_demm_actual.equals(Imp_Demmurage_Input_1)) {
				...
				Extent_pass_New(driver, "Matched || Expected import demmurage days -> "+Imp_Demmurage_Input_1+"|| actual import demmurage days -> "+imp_demm_actual, test, test1);
			}else {
