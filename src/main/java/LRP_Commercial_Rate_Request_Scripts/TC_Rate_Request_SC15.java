...
public class TC_Rate_Request_SC15 extends Keywords {
	...
	public void Rate_RequestSC15(WebDriver driver, ExtentTest test, ExtentTest test1, String selected_dataset,Map<String, String> Excel_data,String url)
			throws Exception {
		...
		String Surcharge_Code=Excel_data.get("Surcharge_Code");
		...
		String Manual_Surcharge_Remove_Popup = Excel_data.get("Manual_Surcharge_Remove_Popup");
...
newButton(driver);
		...
		Actionsendkeys(driver, Commodity_Input, commodity);
			...
			waitForPopup(driver, popup_Message, Manual_Surcharge_Remove_Popup);
			String surcharge_Remove=getText(driver, popup_Message);
			if(Manual_Surcharge_Remove_Popup.equals(surcharge_Remove)) {
				System.out.println("Matched || Expected Popup was : "+Manual_Surcharge_Remove_Popup+" || Actual Popup was : "+surcharge_Remove);
				Extent_pass_New(driver, "Matched || Expected Popup was : "+Manual_Surcharge_Remove_Popup+" || Actual Popup was : "+surcharge_Remove, test, test1);
			}
			else {
				System.out.println("NotMatched || Expected Popup was : "+Manual_Surcharge_Remove_Popup+" || Actual Popup was : "+surcharge_Remove);
				Extent_fail(driver,  "NotMatched || Expected Popup was : "+Manual_Surcharge_Remove_Popup+" || Actual Popup was : "+surcharge_Remove, test, test1);
			}
			...
			String SaveRate_PopUp2= getText(driver, Rate_Sub_Popup_Msg);

			if(submit_msg.equals(SaveRate_PopUp2)) {
				System.out.println("Matched || Expected : popup message should be shown as : "+submit_msg+" || Actual : popup message is shown as : "+SaveRate_PopUp2);
				Extent_pass_New(driver, "Matched || Expected : popup message should be shown as : "+submit_msg+" || Actual : popup message is shown as : "+SaveRate_PopUp2, test, test1);
				...
				System.out.println("NotMatched || Expected : popup message should be shown as : "+submit_msg+" || Actual : popup message is shown as : "+SaveRate_PopUp2);
				Extent_fail(driver,  "NotMatched || Expected : popup message should be shown as : "+submit_msg+" || Actual : popup message is shown as : "+SaveRate_PopUp2, test, test1);
			}
			...
			if(status2.equals(status_value)) {
				...
				Extent_pass_New(driver, "Matched || Expected value was : "+status_value+" || Actual value was : "+status2, test, test1);
			}
			else {
		...
		click(driver, Mail_Cancel_button);
			...
			if(do_You_Want_Delete.equalsIgnoreCase("yes")) {
				...
				waitForElement(driver, RR_close_Tab);
				click(driver, RR_close_Tab);
				...
				if(submitted_ReqNum3.equals(Value3)) {
					...
					Extent_pass_New(driver,"Matched || " + " Expected Value is : " + Value3 + " || ActualValue is : " + submitted_ReqNum3,test, test1);
					Extent_pass_New(driver, Value3+"  is submitted", test, test1);
				...
				if(pending_ReqNum3.equals(Value3)) {
					...
					Extent_pass_New(driver,"Matched || " + " Expected Value is : " + Value3 + " || ActualValue is : " + pending_ReqNum3,test, test1);
					Extent_pass_New(driver, Value3+"  is submitted", test, test1);
					System.out.println(Value3+" is submitted");
					...
					globalValueSearchWindow(driver, Condition_Value,Rate_Request_NO,pending_ReqNum3,"", "", "","");
