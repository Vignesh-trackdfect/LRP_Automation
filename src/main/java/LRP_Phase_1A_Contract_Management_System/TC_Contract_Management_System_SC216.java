...
public class TC_Contract_Management_System_SC216 extends Keywords {
	public void Contract_Management_System_SC216(WebDriver driver, ExtentTest test, ExtentTest test1,
			String selected_dataset, Map<String, String> Excel_data, String url) {

		String testCaseName = "TC_Contract_Management_System_SC216";
		...
		String OOG_Details_Missing_popup = Excel_data.get("OOG_Details_Missing_popup");
		...
		Step_Start(6, "Import the port pair lanes with OOG YES and without the OOG details in OOG  tab.", test, test1);
		...
		if (isdisplayed(driver, CMS_Verify_Duplicate_Table_Page)) {
			...
			List<String> OOG_missing_Headers = splitAndExpand(CMS_Missing_table_Header, ",");

			String OOGmissing_Table = TableDataForReport(driver, tableData, "OOG missing details", OOG_missing_Headers);

			String OOG_missing_popup = getText(driver, CMS_DG_Missing_window_Row);
			System.out.println("OOG missing popup is " + OOG_missing_popup);

			if (OOG_Details_Missing_popup.equals(OOG_missing_popup)) {

				Extent_pass(driver, "After Clicking ON upload Button , OOG missing details should be displayed  "
						+ OOGmissing_Table, test, test1);
			} else {
				Extent_fail(driver, "After Clicking ON upload Button , OOG missing details should be displayed  "
						+ OOGmissing_Table, test, test1);
			}
			...
			Extent_fail(driver,
					"After Clicking ON upload Button , By Importing the port pair lanes with OOG YES and without the OOG details in OOG  tab , OOG missing details is not displayed  ",
					test, test1);
		}

		Step_End(6, "Import the port pair lanes with OOG YES and without the OOG details in OOG  tab.", test, test1);
