...
public class TC_Feeder_Contract_SC53 extends Keywords {

	public void Feeder_Contract_SC53(WebDriver driver, ExtentTest test, ExtentTest test1, String selected_dataset,
			Map<String, String> Excel_data, String url) throws AWTException {

		String testcase_Name = "TC_Feeder_Contract_SC53";
		...
		String AppliedDG_removed = Excel_data.get("AppliedDG_removed");
		String DG_remove_class = Excel_data.get("DG_remove_class");
		...
		Step_Start(24, "After apply and click the Remove DG,It should removed from the Location pair details. ", test,
				test1);

		List<String> removeDGs = splitAndExpand(DG_remove_class);

		for (String DGtypes : removeDGs) {

			formatLocatorClick(driver, FC_DGRowselect, DGtypes);

			waitForElement(driver, FC_Dgremove_aftrapply_Button);
			click(driver, FC_Dgremove_aftrapply_Button);

			waitForPopup(driver, popup_Message, AppliedDG_removed);

			String removed_popup = getText(driver, popup_Message);
			System.out.println("Removed popup :" + removed_popup);

			if (AppliedDG_removed.equals(removed_popup)) {
				Extent_pass(driver, "Matched || Expected popup is : " + AppliedDG_removed + " ||  Actual popup   is : "
						+ removed_popup, test, test1);
				System.out.println("Matched || Expected popup is : " + AppliedDG_removed + " || Actual popup  is : "
						+ removed_popup);
			} else {
				System.out.println("Not Matched || Expected popup  + is : " + AppliedDG_removed
						+ " || Actual popup  is : " + removed_popup);
				Extent_fail(driver, "Not Matched || Expected popup is : " + AppliedDG_removed
						+ " || Actual popup  is : " + removed_popup, test, test1);
		...
		Step_End(24, "After apply and click the Remove DG,It should removed from the Location pair details. ", test,
				test1);
