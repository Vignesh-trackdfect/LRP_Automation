...
public class TC_Vendor_Invoice_Registration_SC33 extends Keywords {

	public void Vendor_Invoice_Registration_SC33(WebDriver driver, ExtentTest test, ExtentTest test1,
			String selected_Excel_dataset,Map<String, String> Excel_data,String url) {

		String testCaseName = "TC_Vendor_Invoice_Registration_SC33";
		...
		if (extractstatus(FDR_Status)) {
			System.out.println("Expected result is : By default system should select the FDR Sum No Tab || Actual result : FDR Sum No tab is selected as default ");
			Extent_pass(driver,"Expected result is : By default system should select the FDR Sum No Tab || Actual result : FDR Sum No tab is selected as default",
					test, test1);
		} else {
			System.out.println("Expected result is : By default system should select the FDR Sum No Tab || Actual result : FDR Sum No tab is not selected as default ");
			Extent_fail(driver,"Expected result is : By default system should select the FDR Sum No Tab || Actual result : FDR Sum No tab is not selected as default",
					test, test1);
		}
		click(driver,FDR_Grid_SumNo);
	
		Step_End(13, "Default system select the FDR sum no", test, test1);

		Step_Start(14, "Click the Fdr sum no search and select the Required no", test, test1);
		...
		if (actual_popupmessage.trim().equals(InvoiceSaved_Popup)) {
			...
			System.out.println("Matched || Expected popup was : " + InvoiceSaved_Popup+ " || Actual popup message was : " + actual_popupmessage);
			Extent_pass(driver, "Matched || Expected popup was : " + InvoiceSaved_Popup+ " || Actual popup message was : " + actual_popupmessage, test, test1);
		} else {

			System.out.println("Not Matched || Expected popup was : " + InvoiceSaved_Popup+ " || Actual popup message was : " + actual_popupmessage);
			Extent_fail(driver, "Not Matched || Expected popup was : " + InvoiceSaved_Popup+ " || Actual popup message was : " + actual_popupmessage, test, test1);
		...
		if (isdisplayed(driver, FER_firstrow)) {
			...
			System.out.println("Expected result is : Saved ref no details should show in the Haulage Expense report || Actual result is : Saved ref no details are showing in the Haulage Expense report");
			Extent_pass(driver,"Expected result is : Saved ref no details should show in the Haulage Expense report || Actual result is : Saved ref no details are showing in the Haulage Expense report",
					test, test1);
		} else {

			System.out.println("Expected result is : Saved ref no details should show in the Haulage Expense report || Actual result is : Saved ref no details are not showing in the Haulage Expense report");
			Extent_fail(driver,"Expected result is : Saved ref no details should show in the Haulage Expense report || Actual result is : Saved ref no details are not showing in the Haulage Expense report",
					test, test1);
