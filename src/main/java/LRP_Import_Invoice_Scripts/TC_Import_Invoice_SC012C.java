...
public class TC_Import_Invoice_SC012C extends Keywords {
 
	public void Import_Invoice_SC012C(WebDriver driver, ExtentTest test, ExtentTest test1, String selected_dataset,Map<String, String> Excel_data,String url) {

		String testcase_Name = "TC_Import_Invoice_SC012C";
		...
		String detentionCharges_Tab = Excel_data.get("DetentionCharges_Tab");
		...
		if (container_Option.contains("All Container")) {
			...
			if (Date_Picker.equalsIgnoreCase("Yes")) {
				...
				waitForElement(driver, detention_Extention_Date);
				click(driver, detention_Extention_Date);
				clear(driver, detention_Extention_Date);
...
//	BL Details
		...
		System.out.println("Count of Detention Charges Before reversal: " + manifestRows_detention);
		...
		String detentionCharges_Tabledata_Report = TableDataForReport(driver, detentionCharges_Tabledata,
				detentionCharges_Table_Name_In, detentionCharges_headers);

		List<String> inv_Amount_detention = getValuesByHeader(detentionCharges_Tabledata,Amount_Header);

		ArrayList<String> invoice_Amount_detention = new ArrayList<String>();
		ArrayList<String> invoice_Amount_Neg_detention = new ArrayList<String>();

		for (int i = 0; i < inv_Amount_detention.size(); i++) {
			invoice_Amount_detention.add(inv_Amount_detention.get(i));
			String negative = "-" + invoice_Amount_detention.get(i);
			invoice_Amount_Neg_detention.add(negative);
		}
		System.out.println("Invoice Amount : " + invoice_Amount_detention);
		System.out.println("Invoice Amount Negative : " + invoice_Amount_Neg_detention);



		Step_End(14, "15A. Capture the details", test, test1);
...
//		Detention Charges 
		...
		Step_Start(4, "detention charge(All the Columns)", test, test1);

		if (!detentionCharges_Tab.equals("")) {

			waitForElement(driver, detentionCharges_Tab_IN);
			click(driver, detentionCharges_Tab_IN);

			waitForElement(driver, detentionCharges_Table_Rows_IN);
			List<WebElement> total_Rows_detention1 = listOfElements(driver, detentionCharges_Table_Rows_IN);
			int manifestRows_detention1 = total_Rows_detention1.size();


			List<Map<String, String>> detentionCharges_Tabledata1 = getTableData(driver,
					detentionCharges_Table_Columns_IN, detentionCharges_Table_Rows_IN);

			List<String> detentionCharges_headers1 = splitAndExpand(columnHeaders_DetentionCharges);

			String detentionCharges_Tabledata_Report1 = TableDataForReport(driver, detentionCharges_Tabledata1,
					detentionCharges_Table_Name_In, detentionCharges_headers1);

			List<String> inv_Amount_detention1 = getValuesByHeader(detentionCharges_Tabledata1,Amount_Header);

			ArrayList<String> invoice_Amount_detention1 = new ArrayList<String>();

			for (int i = 0; i < inv_Amount_detention1.size(); i++) {
				invoice_Amount_detention1.add(inv_Amount_detention1.get(i));
			}
			System.out.println("Invoice Amount : " + invoice_Amount_detention1);


			if (manifestRows_detention == manifestRows_detention1) {
				System.out.println("Matched || Charge code count before reversal : " + manifestRows_detention
						+ " || Charge code count after reversal : " + manifestRows_detention1);
				Extent_pass_New(driver, "Matched || Charge code count before reversal : " + manifestRows_detention
						+ " || Charge code count after reversal : " + manifestRows_detention1, test, test1);
			} else {
				System.out.println("Not Matched || Charge code count before reversal : " + manifestRows_detention
						+ " || Charge code count after reversal : " + manifestRows_detention1);
				Extent_fail(driver, "Not Matched || Charge code count before reversal : " + manifestRows_detention
						+ " || Charge code count after reversal : " + manifestRows_detention1, test, test1);
			}

			boolean pass1 = false;
			for (int i = 0; i < manifestRows_detention; i++) {
				if (invoice_Amount_detention1.contains(invoice_Amount_Neg_detention.get(i))) {
					pass1 = true;
				} else {
					pass1 = false;
				}
			...
			if (pass1 == true) {
				System.out.println(
						"Matched || Expected result is : Detention Charges Table should be displayed in Import Invoice Detention Charges Tab ||  Detention Charges Table is displayed in Import Invoice Detention Charges Tab");
				
				Extent_pass_New(driver,
						"Matched || Before Reversal: \n" + "<pre>" + detentionCharges_Tabledata_Report + "</pre>"
								+ "\r\n || After Reversal : \r\n" + "<pre>" + detentionCharges_Tabledata_Report1
								+ "</pre>",
						test, test1);
			} else {
				System.out.println(
						"Not Matched || Expected result is : Detention Charges Table should be displayed in Import Invoice Detention Charges Tab ||  Detention Charges Table is not displayed in Import Invoice Detention Charges Tab");
				Extent_fail(driver,
						"Not Matched || Expected result is : Detention Charges Table should be displayed in Import Invoice Detention Charges Tab ||  Detention Charges Table is not displayed in Import Invoice Detention Charges Tab",
						test, test1);
			}
		...
		Step_End(4, "detention charge(All the Columns)", test, test1);
