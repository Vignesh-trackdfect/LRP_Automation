...
public class TC_Discharge_Reconciliation_MD37 extends Keywords {
public void Discharge_Reconciliation_MD37(WebDriver driver ,ExtentTest test, ExtentTest test1, String selected_dataset,Map<String, String> Excel_data,String url) {
		
		
		
		String testcase_Name="TC_Discharge_Reconciliation_MD37";
		...
		Step_Start(8, "Go to Import Documentation not Saved  Label and Capture the Total Count of saved BLs.\r\n"
				+ "", test, test1);
		String Un_Count_Saved1 = getAttribute(driver, Count_UnSaved_Label, "value");
		Extent_pass(driver, "the Total Count of Unsaved BLs is :"+Un_Count_Saved1, test, test1);
	    System.out.println(Un_Count_Saved1);
	    Step_End(8, "Go to Import Documentation not Saved  Label and Capture the Total Count of saved BLs.\r\n"
	    		+ "", test, test1);	
		...
		Step_End(10, "Click on New button if New Button is not Available Go to Next Step", test, test1);
		 ...
		 waitForElement(driver, ServiceInput_IMD);
			...
			click(driver, grid_selectbtn_IMP_DOC);
			 ...
			 Step_Start(12, "Out of 3 select it  as UnSavedBL option ALL/ SavedBL/ UnSavedBL,(Bring it from Test data)", test, test1);
			 ...
			 waitForElement(driver, slection_Label_1);
				...
				click(driver, select_Option_1);
				 Step_End(12, "Out of 3 select it  as UnSavedBL option ALL/ SavedBL/ UnSavedBL,(Bring it from Test data)", test, test1);
				...
				click(driver, select_Option_2);
				 ...
				 waitForElement(driver, Show_BL_Button);
					...
					Step_End(14, " Click on SHOW BLs button", test, test1);
					 ...
					 Step_Start(15, "Go and Find the Count of UnSAved BL and Capture it", test, test1);
					 ...
					 if (valid2.contains("/")) {
					     String[] UnsavedBL = valid2.split("/");
					     String Unsaved_BL_Count = UnsavedBL[1].trim(); 
					     
						Extent_pass(driver, "The Total Count of Unsaved BLs IN import document is :"+Unsaved_BL_Count, test, test1);
					    System.out.println(Unsaved_BL_Count);
					 
					 Step_End(15, "Go and Find the Count of UnSAved BL and Capture it", test, test1);
			
					 Step_Start(16, " Verify  that not saved BL Count in Discharge reconciliation and  IMport Documentation has been matched.", test, test1);
					 

				        if (Unsaved_BL_Count.equals(Un_Count_Saved1.trim())) {
							
				        	System.out.println(" Expected ||Not saved BL Count in Discharge reconciliation is"+Un_Count_Saved1+"   ||Actual Value present in Import Document UnSaved Column  is : "+Unsaved_BL_Count);
				 			Extent_pass(driver, "Expected ||Not saved BL Count in Discharge reconciliation is "+Un_Count_Saved1+"   ||Actual Value present in Import Document UnSaved Column  is: "+Unsaved_BL_Count, test, test1);
				 		}else {
				 			System.out.println("UnExpected ||Not saved BL Count in Discharge reconciliation is "+Un_Count_Saved1+"   ||Actual Value present in Import Document UnSaved Column  is   : "+Unsaved_BL_Count);
				 			Extent_fail(driver, "UnExpected ||Not saved BL Count in Discharge reconciliation is "+Un_Count_Saved1+"   ||Actual Value present in Import Document UnSaved Column  is   : "+Unsaved_BL_Count, test, test1);
				 		}
}
					 Step_End(16, " Verify  that not saved BL Count in Discharge reconciliation and  IMport Documentation has been matched.", test, test1);
