...
public class TC_Detention_Waiver_TS_023 extends Keywords {

	public void Detention_Waiver_TS_023(WebDriver driver, ExtentTest test, ExtentTest test1, String selected_dataset,Map<String, String> Excel_data,String url)
			throws EncryptedDocumentException, InvalidFormatException, IOException {

		String testcase_Name = "TC_Detention_Waiver_TS_023";
...
Step_Start(1, "Open Detention weiver Module", test, test1);
		...
		Step_Start(5, "Verify that Inclusive and Exclusive options are listed under DropDown.", test, test1);

		waitForDisplay(driver, DW_Additional_Free_Days);
		if (isdisplayed(driver, DW_Additional_Free_Days)) {
			System.out.println(
					"Expected result : Additional free days field should be displayed || Actual result : Additional free days field was displayed");
			Extent_pass_New(driver,
					"Expected result : Additional free days field should be displayed || Actual result : Additional free days field was displayed",
					test, test1);
		} else {
			System.out.println(
					"Expected result : Additional free days field should be displayed || Actual result : Additional free days field was not displayed");
			Extent_fail(driver,
					"Expected result : Additional free days field should be displayed || Actual result : Additional free days field was not displayed",
					test, test1);
		}
		...
		waitForDisplay(driver, inclusive_Field);
		if (isdisplayed(driver, inclusive_Field)) {
			System.out.println(
					"Expected result : Inclusive option should be displayed || Actual result : Inclusive option was present in droupdown");
			Extent_pass_New(driver,
					"Expected result : Inclusive option should be displayed || Actual result : Inclusive option was present in droupdown",
					test, test1);
		} else {
			System.out.println(
					"Expected result : Inclusive option should be displayed || Actual result : Inclusive option was not present in droupdown");
			Extent_fail(driver,
					"Expected result : Inclusive option should be displayed || Actual result : Inclusive option was not present in droupdown",
					test, test1);
		}
		waitForDisplay(driver, exclusive_Field);
		if (isdisplayed(driver, exclusive_Field)) {
			System.out.println(
					"Expected result : Exclusive option should be displayed || Actual result : Exclusive option was present in droupdown");
			Extent_pass_New(driver,
					"Expected result : Exclusive option should be displayed || Actual result : Exclusive option was present in droupdown",
					test, test1);
		} else {
			System.out.println(
					"Expected result : Exclusive option should be displayed || Actual result : Exclusive option was not present in droupdown");
			Extent_fail(driver,
					"Expected result : Exclusive option should be displayed || Actual result : Exclusive option was not present in droupdown",
					test, test1);
		}

		Step_End(5, "Verify that Inclusive and Exclusive options are listed under DropDown.", test, test1);
