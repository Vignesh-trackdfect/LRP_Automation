...
public class TC_Import_Invoice_SC025G extends Keywords {
	public void Import_Invoice_SC025G(WebDriver driver, ExtentTest test, ExtentTest test1, String selected_dataset,
			Map<String, String> Excel_data, String url) {
		String testCaseName = "TC_Import_Invoice_SC025G";
		...
		String Additional_FreeDays_Inclusive = Excel_data.get("Additional_FreeDays_Inclusive");
		...
		String Approved_Success_popup_Detention = Excel_data.get("Approved_Success_popup_Detention");
				String Additional_FreeDays_Exclusive = Excel_data.get("Additional_FreeDays_Exclusive");
		...
		globalValueSearchWindow1(driver, Global_Search_Condition, BookingSearch_Option1, Global_Search_Input1,
				BookingSearch_Option2, Global_Search_Input2, BookingSearch_Option3, Global_Search_Input3);
		List<String> Status_Filer_Datas = splitAndExpand(Status_Filter_Input, ",");
		boolean temp=false;
		for(int i=0;i<Status_Filer_Datas.size();i++) {
			...
			if(isdisplayed(driver, retrivedGlobalValue)) {
				...
				horizontalscroll(driver, DO_horizontal_Scroll, 500);
				waitForElement(driver, Waiver_Status_Filter);
				clear(driver, Waiver_Status_Filter);
				sendKeys(driver, Waiver_Status_Filter, Status_Filer_Datas.get(i));
				if(isdisplayed(driver, retrivedGlobalValue)) {
		...
		if(!temp) {
			System.err.println("Not  Matched || This BL Number : "+Global_Search_Input1+" is Didn't having any Detention waiver");
			Extent_fail(driver,"Not  Matched || This BL Number : "+Global_Search_Input1+" is Didn't having any Detention waiver", test, test1);
		...
		List<String> waiverType = getValuesByHeader(waiverTable, "Incl/Excl");
		if (waiverType.stream().allMatch(value -> "Inclusive".equalsIgnoreCase(value))) {
			System.out.println("The detention waiver is created under the inclussive waiver type");
			Extent_pass_New(driver, "The detention waiver is created under the inclussive waiver type", test, test1);
		} else {
			scrollTop(driver);
			waitForDisplay(driver, Edit_Button_toolBar);
			...
			Actionsendkeys(driver, DW_Additional_Free_Days, Additional_FreeDays_Inclusive);
			waitForElement(driver, EX_IN_Field);
			click(driver, EX_IN_Field);
			formatLocatorClick(driver, DropDown_Select, "Inclusive");
			waitForElement(driver, DW_Add_button);
			click(driver, DW_Add_button);
			waitForElement(driver, Waiver_Value_Table);
			waiverTable = extractTableDataCountMissMatch(driver, Waiver_Value_Table);
			free_Days = getValuesByHeader(waiverTable, Container_columns_Headers);
			Expected_Freedays = convertStringListToDoubleList(driver, free_Days);
			givencontainers = getValuesByHeader(waiverTable, ContainerNumbers_Header);
			scrollTop(driver);
		...
		click(driver, DW_Additional_Free_Days);
		Newclear(driver, DW_Additional_Free_Days);
		Actionsendkeys(driver, DW_Additional_Free_Days, Additional_FreeDays_Exclusive);
		waitForElement(driver, EX_IN_Field);
		click(driver, EX_IN_Field);
		formatLocatorClick(driver, DropDown_Select, "Exclusive");
		waitForElement(driver, DW_Add_button);
		click(driver, DW_Add_button);
		waitForElement(driver, Waiver_Value_Table);
		List<Map<String, String>> waiverTable_Exc = extractTableDataCountMissMatch(driver, Waiver_Value_Table);
		List<String> free_Days_Exc = getValuesByHeader(waiverTable_Exc, Container_columns_Headers);
		List<Double> Expected_Freedays_Exc = convertStringListToDoubleList(driver, free_Days_Exc);
		scrollTop(driver);
		Step_Start(6, "Enter the Remarks in Remarks section", test, test1);
		waitForElement(driver, DW_Remarks);
		click(driver, DW_Remarks);
		...
		List<Double> Expected_Value = sumDoubleLists(Expected_Freedays, Expected_Freedays_Exc);
		SwitchProfile(driver, AgencyUser);
		// Import Invoice
		...
		if (Detention_Tab_Name_In_Iport_Invoice .contains(DetentionStorage_Option)) {
			...
			for (int i = 0; i < doubleList.size(); i++) {
				Double ExpectedFreeDays = Expected_Value.get(i);
				...
				if (Double.compare(ExpectedFreeDays, ActualFreeDays) == 0) {
					System.out.println("Matched || The Container Number is : " + givencontainers.get(i) + " || The "
							+ Container_columns_Headers +" The Free days Added From The Inclusive Detention Waiver value is : " + Expected_Freedays.get(i)
							+" || The Free days Added From The Exclusive Detention Waiver value is : " + Expected_Freedays_Exc.get(i)
					...
					Extent_fail(driver,"Not Matched || The Container Number is : " + givencontainers.get(i) + " || The "
							+ Container_columns_Headers +" The Free days Added From The Inclusive Detention Waiver value is : " + Expected_Freedays.get(i)
							+" The Free days Added From The Inclusive Detention Waiver value is : " + Expected_Freedays_Exc.get(i)
							+ " || The Actual Value from the Import Invoice Freedays Value is : " + ActualFreeDays
							+ " in " + Detention_Tab_Name_In_Iport_Invoice + " Tab.",
									test, test1);
				}
