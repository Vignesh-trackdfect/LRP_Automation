...
public class TC_Merge_Booking_SC10 extends Keywords{

public void Merge_Booking_SC10(WebDriver driver, ExtentTest test, ExtentTest test1, String selected_dataset,Map<String, String> Excel_data,String url) throws Exception {
		
		
	    String testCaseName="TC_Merge_Booking_SC10";
...
//	Master tab		
		...
		Step_Start(2, "In plugin configuration enable VVT plugin as flse", test, test1);
		
		setPluginConfig(driver, pluginConfiguration_Module_Name, plugin_Header, Serach_Condition, pluginID,
				PluginStatus);
		
		Step_End(2, "In plugin configuration enable VVT plugin as false", test, test1);
		
		Step_Start(3, " Then open merge booking now system will list service first and then vessel", test, test1);
		
		
		moduleNavigate(driver, Merge_Booking_Module_Name);
			
		String actualVessel_Service = String.format(MB_Vessel_Service_label,Service_label,Vessel_label);
		waitForDisplay(driver, actualVessel_Service);
		if (isdisplayed(driver, actualVessel_Service)) {
			System.out.println(
					"Expected result : Merge booking now system will list service first and then vessel should be displayed || Actual result : Merge booking now system will list service first and then vessel should be displayed  ");
			...
			System.out.println(
					"Expected result : Merge booking now system will list service first and then vessel should be displayed || Actual result : Merge booking now system will list service first and then vessel should Not be displayed ");
			Extent_fail(driver,
					"Expected result : Merge booking now system will list service first and then vessel should be displayed  || Actual result : Merge booking now system will list service first and then vessel should Not be displayed ",
					test, test1);
		...
		Step_End(3, "Then open merge booking now system will list service first and then vessel", test, test1);
