...
public class TC_Rate_Request_SC02 extends Keywords {
	...
	public void duplicateRecord(WebDriver driver, ExtentTest test, ExtentTest test1, String selected_dataset,Map<String, String> Excel_data,String url)
			throws Exception {
		...
		String CommodityExist = Excel_data.get("CommodityExist");
		String alreadyExistPopup = Excel_data.get("alreadyExistPopup");
...
newButton(driver);
		...
		Actionsendkeys(driver, Commodity_Input, CommodityExist);
		waitForElement(driver, Auto_Panel_First);
		safeclick(driver, Auto_Panel_First);
		...
		if(isdisplayed(driver, Popup_Message)) {
			String actExistPopup=getText(driver, Popup_Message);	
			if(actExistPopup.contains(alreadyExistPopup)) {
				Extent_pass_New(driver,"Matched || Expected : "+alreadyExistPopup+" Actual : "+ actExistPopup, test,test1);
				System.out.println("Matched || Expected : "+alreadyExistPopup+" Actual : "+ actExistPopup);
				Extent_pass_New(driver, alreadyExistPopup+" popup is displayed", test,test1);
