...
public class TC_Import_Documentation_SC114 extends Keywords {
	public void Import_Documentation_SC114(WebDriver driver, ExtentTest test, ExtentTest test1, String selected_dataset,Map<String, String> Excel_data,String url)
			throws Exception {
		String testcase_Name="TC_Import_Documentation_SC114";
		...
		String Table_Headers_Rearrange_DR = Excel_data.get("Table_Headers_Rearrange_DR");		
		...
		if(BL_Retrive_Option.equalsIgnoreCase("By_BL")) {
			...
			globalValueSearchWindow(driver, BL_Condition, Select_Search_Type_Import_Doc, BL_Num, Global_Search_Type2, Global_Search_Value2, Global_Search_Type3, Global_Search_Value3);
			...
			if(isdisplayed(driver, NewButton_ToolBar) && IsElementEnabled(driver, NewButton_ToolBar)) {
				...
				if (New_Import_Documentation.equals(NewImportDocumentation)) {
					...
					System.out.println("NotMatched || Expected popup was : " + New_Import_Documentation
							+ " || Actual popup was : " + NewImportDocumentation);
					Extent_fail(driver, "NotMatched || Expected popup was : " + New_Import_Documentation
		...
		Step_Start(13, "cargotype has shown", test, test1);

		waitForElement(driver, IMD_cargotype);
		String Cargo_Type = getText(driver, IMD_cargotype);
		Step_End(13, "cargotype has shown", test, test1);
		...
		String option_Select = String.format(DropDown_Select, DR_Plan_Option);
		...
		Step_Start(21, ".system has shown the bl's in below grid for that service based", test, test1);

		waitForElement(driver, Tool_Pannel);
		click(driver, Tool_Pannel);
		waitForElement(driver, Condition_Filter_);
		click(driver, Condition_Filter_);
		
		reArrangeAG_Grid_Outer_Column_CS(driver, Table_Headers_Rearrange_DR);
		
		waitForElement(driver, BL_Num_Filter_Input_Table);
		click(driver, BL_Num_Filter_Input_Table);
		sendKeys(driver, BL_Num_Filter_Input_Table, BL_Num);
		Step_End(21, ".system has shown the bl's in below grid for that service based", test, test1);
		Step_Start(22, "in below grid right side, cargotype column value has shown", test, test1);
		Step_End(22, "in below grid right side, cargotype column value has shown", test, test1);
		Step_Start(23, "In the import documentation, the cargo type for that BL should match the cargo type in the discharge reconciliation.", test, test1);

		waitForElement(driver, Cargo_type_Value);
		String cargo_value = getText(driver, Cargo_type_Value);
		if (Cargo_Type.equalsIgnoreCase(cargo_value)) {
			Extent_pass_New(driver, "Matched || Expected The Cargo Type Value From Import Documentaion Module : " + Cargo_Type + " || Actual Cargo Type value From Discharge Reconciliation Module : " + cargo_value, test, test1);
			System.out.println("Matched || Expected The Cargo Type Value From Import Documentaion Module : " + Cargo_Type + " || Actual Cargo Type value From Discharge Reconciliation Module : " + cargo_value);
		} else {
			System.out.println("Not Matched || Expected The Cargo Type Value From Import Documentaion Module : " + Cargo_Type + " || Actual Cargo Type value From Discharge Reconciliation Module : " + cargo_value);
			Extent_fail(driver, "Not Matched || Expected The Cargo Type Value From Import Documentaion Module : " + Cargo_Type + " || Actual Cargo Type value From Discharge Reconciliation Module : " + cargo_value, test,
					test1);
		}
		Step_End(23, "In the import documentation, the cargo type for that BL should match the cargo type in the discharge reconciliation.", test, test1);
