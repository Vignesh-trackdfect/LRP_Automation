...
public class TC_Feeder_Contract_SC49 extends Keywords {

	public void Feeder_Contract_SC49(WebDriver driver, ExtentTest test, ExtentTest test1, String selected_dataset,
			Map<String, String> Excel_data, String url) {

		String testcase_Name = "TC_Feeder_Contract_SC49";
		...
		List<String> surcharge_Value = splitAndExpand(DG_Class);
		List<String> DG_Classname_Values = splitAndExpand(DG_Classname_Value, "/");
		...
		for (String surcharge : surcharge_Value) {
			if (surcharge != null && !surcharge.trim().isEmpty()) {
				...
				waitForElement(driver, FC_DG_Plus_Button);
				click(driver, FC_DG_Plus_Button);

				// Step 2: Search & Select the Surcharge

				String dgclassname = DG_Classname_Values.get(i);

				twoColumnSearchWindowMultipleValue_multipleheader(driver, DG_Class_Header, Two_Column_Condition,
						surcharge, DGClassName_Header, dgclassname);

				// Step 3: Format locator and wait for display
				String added = formatLocatorClick(driver, FC_Surcharge_Gird_Value, surcharge);
				waitForDisplay(driver, added);

				// Step 4: Check if displayed and perform mouse over
				if (isdisplayed(driver, added)) {
					System.out.println("Expected result : The given DG code " + surcharge
							+ " should be added Charge code was added: Actual result : The DG code " + surcharge
							+ "was added");
					Extent_pass(driver,
							"Expected result : The given DG code " + surcharge
									+ " should be added Charge code was added: Actual result : The DG code " + surcharge
									+ "was added",
							test, test1);
				} else {
