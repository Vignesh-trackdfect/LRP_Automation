...
public class TC_Booking_SC054 extends Keywords {

	public void Booking_SC054 (WebDriver driver, ExtentTest test,ExtentTest test1,String selected_dataset,Map<String, String> Excel_data,String url) {

		String test_Case_Name = "TC_Booking_SC054";
		...
		String ETD_arrdate = Excel_data.get("ETD_arrdate");
		...
		Step_Start(8, "change the schedule details ", test, test1);
		...
		if (date_Picker.equalsIgnoreCase("Yes")) {
			waitForElement(driver, ETD_Arr_date);
			selectDatePickerDoubleClick(driver, ETD_Arr_date, ETD_arrdate);
		} else {
			waitForElement(driver, ETD_grid);
			click(driver, ETD_grid);
			waitForElement(driver, ETD_Arr_date);
			doubleClick(driver, ETD_Arr_date);
			clear(driver, ETD_Arr_date);
			sendKeys(driver, ETD_Arr_date, ETD_arrdate);
			enter(driver);
		...
		if(feeder_update.equals(feeder_update_msg)) {
			...
			Extent_pass_New(driver,"Matched||Excepted Pop-up Value is:"+ feeder_update_msg+" Actual Pop-up Value is:"+feeder_update, test, test1);
			System.out.println("Matched ||Excepted Pop-up Value is:"+ feeder_update_msg+" Actual Pop-up Value is:"+feeder_update);
		}
		else {
			System.out.println("Not matched ||Excepted Pop-up Value is:"+ feeder_update_msg+" Actual Pop-up Value is:"+feeder_update);
			Extent_fail(driver,"Not matched ||Excepted Pop-up Value is:"+ feeder_update_msg+" Actual Pop-up Value is:"+feeder_update, test, test1);
		}
		waitForElement(driver, Close_Current_tab);
		click(driver, Close_Current_tab);

		Step_End(8, "change the schedule details ", test, test1);

		Step_Start(9, "system update the latest details in leg tab", test, test1);
		...
		waitForElement(driver, refresh_leg);
		safeclick(driver, refresh_leg);

		waitForElement(driver, refreshleg_popup);
		safeclick(driver, refreshleg_popup);

		waitForElement(driver, refreshleg_popup_yes);
		safeclick(driver, refreshleg_popup_yes);

		waitForElement(driver, legtab_POL);
		String updated_leg_ETDdate=getText(driver, legtab_POL);
		if(updated_leg_ETDdate.equals(ETDdepdate)) {
			Extent_pass_New(driver,"Matched || Excepted ETD Date : "+ ETDdepdate+" Actual ETD Date : "+updated_leg_ETDdate, test,test1);
			System.out.println("Matched || Excepted ETD Date : "+ ETDdepdate+" Actual ETD Date : "+updated_leg_ETDdate);
		}else {
			System.out.println("Not matched || Excepted ETD Date : "+ ETDdepdate+" Actual ETD Date : "+updated_leg_ETDdate);
			Extent_fail(driver,"Not matched || Excepted ETD Date : "+ ETDdepdate+" Actual ETD Date : "+updated_leg_ETDdate, test,test1);
		}
		Step_End(9, "system update the latest details in leg tab", test, test1);
