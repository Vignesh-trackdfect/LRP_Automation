...
public class TC_Feeder_Contract_SC64 extends Keywords {

	public void Feeder_Contract_SC64(WebDriver driver, ExtentTest test, ExtentTest test1, String selected_dataset,
			Map<String, String> Excel_data, String url) {

		String testcase_Name = "TC_Feeder_Contract_SC64";
		...
		String Upload_File_Name = Excel_data.get("Upload_File_Name");
		String Uploaded_Popup = Excel_data.get("Uploaded_Popup");
		...
		Step_Start(10, "Click select file and choose the file .", test, test1);

		uploadfile_new(driver, FC_Document_Attachment_File_Upload, Upload_File_Name);

		waitForElement(driver, FC_Document_Attachment_Upload_Button);
		click(driver, FC_Document_Attachment_Upload_Button);

		waitForPopup(driver, popup_Message, Uploaded_Popup);
		String actual_Popup = getText(driver, popup_Message);

		if (actual_Popup.equals(Uploaded_Popup)) {

			System.out.println("Matched || Expected Popup : " + Uploaded_Popup + " || Actual Popup : " + actual_Popup);
			Extent_pass_New(driver,
					"Matched || Expected Popup : " + Uploaded_Popup + " || Actual Popup : " + actual_Popup, test,
					test1);
			click(driver, popup_Message_Ok_Button);
		} else {

			System.out.println(
					"Not Matched || Expected Popup : " + Uploaded_Popup + " || Actual Popup : " + actual_Popup);
			Extent_fail(driver,
					"Not Matched || Expected Popup : " + Uploaded_Popup + " || Actual Popup : " + actual_Popup, test,
					test1);

		}
		Step_End(10, "Click select file and choose the file .", test, test1);
