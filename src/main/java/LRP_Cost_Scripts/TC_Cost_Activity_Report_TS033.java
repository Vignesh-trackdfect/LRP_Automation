...
public class TC_Cost_Activity_Report_TS033 extends Keywords{
	...
	public void  Cost_Activity_Report_TS033(WebDriver driver, ExtentTest test, ExtentTest test1,String selected_dataset,Map<String, String> Excel_data,String url) throws Exception{
		...
		String Expensive_Headers_Names = Excel_data.get("Expensive_Headers_Names");
		...
		Step_Start(10, "Check whether the submitted/rejected/approved by username and date column is present.And ensure that this column is present in open expenses tab.  ", test, test1);

		waitForElement(driver, Open_Expensive_CAR);
		click(driver,Open_Expensive_CAR);
		
		waitForDisplay(driver, Open_Expensive_CAR);
		if(isdisplayed(driver, Open_Expense_Table_Rows)) {
			...
			List<String> UserNameValues=getValuesByHeader(Open_Expensive_TableData, "User Name");
			List<String> Date_Values=getValuesByHeader(Open_Expensive_TableData, "Date");
			List<String> Status_Values=getValuesByHeader(Open_Expensive_TableData, "Status");
		...
		Step_End(10, "Check whether the submitted/rejected/approved by username and date column is present.And ensure that this column is present in open expenses tab.  ", test, test1);

		Step_Start(11, "Check whether the submitted/rejected/approved by username and date column is present.And ensure that this column is present in approved expenses tab.  ", test, test1);
		...
		if(isdisplayed(driver, App_Expense_Table_Rows)) {
			...
			List<String> UserNameValues=getValuesByHeader(App_Expensive_TableData, "User Name");
			List<String> Date_Values=getValuesByHeader(App_Expensive_TableData, "Date");
			List<String> Status_Values=getValuesByHeader(App_Expensive_TableData, "Status");
		...
		Step_End(11, "Check whether the submitted/rejected/approved by username and date column is present.And ensure that this column is present in approved expenses tab.  ", test, test1);
