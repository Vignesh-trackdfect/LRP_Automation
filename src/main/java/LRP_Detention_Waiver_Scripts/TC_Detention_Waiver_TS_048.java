...
public class TC_Detention_Waiver_TS_048 extends Keywords {

	public void Detention_Waiver_TS_048(WebDriver driver, ExtentTest test, ExtentTest test1, String selected_dataset,
			Map<String, String> Excel_data, String url)
			throws EncryptedDocumentException, InvalidFormatException, IOException {

		String testcase_Name = "TC_Detention_Waiver_TS_048";
		...
		String Upload_Filename2 = Excel_data.get("Upload_Filename2");
		...
		Step_Start(4, "a) Click on upload button again upload the Second files", test, test1);

		uploadfile_new(driver, DW_Uploads, Upload_Filename2);
		...
		if (actualpopup2.equals(Upload_Popup)) {

			System.out.println(
					"File uploaded Successfully || Expected : " + Upload_Popup + " || Actual : " + actualpopup2);
			Extent_pass_New(driver,
					"File uploaded Successfully || Expected : " + Upload_Popup + " || Actual : " + actualpopup2, test,
					test1);

		} else {

			System.out.println("File not uploaded || Expected : " + Upload_Popup + " || Actual : " + actualpopup2);
			Extent_fail(driver, "File not uploaded || Expected : " + Upload_Popup + " || Actual : " + actualpopup2,
					test, test1);
		...
		Step_End(4, "a) Click on upload button again upload the Second files", test, test1);
		...
		String Filecheck2 = String.format(DW_uploadedfilename, Upload_Filename2);

		if (isdisplayed(driver, Filecheck2)) {

			System.out.println(
					"Expected result : Uploaded file should showin in the preview Tab || Actual result is : Uploaded Second file is showing in the preview Tab");
			Extent_pass_New(driver,
					"Expected result : Uploaded file should showin in the preview Tab || Actual result is : Uploaded Second file is showing in the preview Tab",
					test, test1);
		} else {
			...
			System.out.println(
					"Expected result : Uploaded file should showin in the preview Tab || Actual result is : Uploaded Second file is not showing in the preview Tab");
			Extent_fail(driver,
					"Expected result : Uploaded file should showin in the preview Tab || Actual result is : Uploaded Second file is not showing in the preview Tab",
					test, test1);
		...
		String User_name2 = String.format(DW_uploadedUser, Upload_Filename2);

		String AddedUsername2 = getText(driver, User_name2);

		if (AddedUsername2.equals(Username.toUpperCase())) {
			System.out.println("Uploaded file is added with correct username || Expected : " + Username
					+ " || Actual : " + AddedUsername2);
			Extent_pass_New(driver, "Uploaded file is added with correct username || Expected : " + Username
					+ " || Actual : " + AddedUsername2, test, test1);
		} else {
		...
		String Uploadeddate2 = String.format(DW_uploadedDate, Upload_Filename2);

		String AddedDate2 = getText(driver, Uploadeddate2);

		if (AddedDate2.contains(currentdate)) {
			System.out.println("Uploaded file is added with correct Date || Expected : " + currentdate + " || Actual : "
					+ AddedDate2);
			Extent_pass_New(driver, "Uploaded file is added with correct Date || Expected : " + currentdate
					+ " || Actual : " + AddedDate2, test, test1);
		} else {
