...
public class TC_Receivable_Receipt_RR24 extends Keywords{
	public void Receivable_Receipt_RR24(WebDriver driver, ExtentTest test, ExtentTest test1,String selected_dataset,Map<String, String> Excel_data,String url) throws EncryptedDocumentException, InvalidFormatException, IOException{

		String testcase_Name="TC_Receivable_Receipt_RR24";
		...
		String Receipt_Amount_Value = Excel_data.get("Receipt_Amount_Value");
		...
		String Bank_Charges_header= Excel_data.get("Bank_Charges_header");
		String Bank_Charges_header_value= Excel_data.get("Bank_Charges_header_value");
		String Bank_Charges_filter= Excel_data.get("Bank_Charges_filter");
     ...
     if(Date_Picker.equalsIgnoreCase("Yes")) {
		...
		clearAndType(driver, Receipt_amount_Edit,Receipt_Amount_Value);
		enter(driver);
		
		Step_End(7, "Enter the positive value in receipt amount column", test, test1);
		...
		clearAndType(driver, localAmount_Textfield_RR, Receipt_Amount_Value);
		waitForElement(driver, localAmount_Textfield_RR);
		 click(driver, cheque_TT_No_Textfield_RR);
		 waitForElement(driver, localAmount_Textfield_RR);

		Step_Start(12, "Select the ROE Difference", test, test1);
		...
		Step_End(12, "Select the ROE Difference", test, test1);
		Step_Start(13, "Select the Bank charges", test, test1);
		if(!Bank_Charges.equals("")) {
			...
			twoColumnMultipleSearchWindow(driver,Bank_Charges_header,Bank_Charges_filter,Bank_Charges_header_value);
			waitForElement(driver, acccountName_Filter_RR);
		}
		Step_End(13, "Select the Bank charges", test, test1);	
		...
		if(Saved_Popup.equals(Saved_Receipt_Popup)) {
			...
			System.out.println("Not Matched ||Expected : After click on save button Popup value should be display in the Receivable Receipt Screen : : " + Saved_Popup + " || Actual : After click on save button Popup value is display in the Receivable Receipt Screen : " + Saved_Receipt_Popup);
			Extent_fail(driver,"Not Matched ||Expected : After click on save button Popup value should be display in the Receivable Receipt Screen : : " + Saved_Popup + " || Actual : After click on save button Popup value is display in the Receivable Receipt Screen   : " + Saved_Receipt_Popup,test, test1);
		}

		waitForElement(driver, popup_Message_Ok_Button);
		click(driver, popup_Message_Ok_Button);

		Step_End(19, "Click save, should show the pop up message as 'Receivable Receipt Saved'", test, test1);
