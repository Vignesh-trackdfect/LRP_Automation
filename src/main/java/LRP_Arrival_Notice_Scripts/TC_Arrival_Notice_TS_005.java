...
public class TC_Arrival_Notice_TS_005 extends Keywords{
	public void Arrival_Notice_TS_005(WebDriver driver,ExtentTest test,ExtentTest test1,String selected_dataset,Map<String, String> Excel_data,String url) {

		//		Get data from test data

		String testcase_Name="TC_Arrival_Notice_TS_005";
		...
		String Discharge_Service_Input = Excel_data.get("Discharge_Service_Input");
		...
		click(driver, discharge_Service_TF_AN);
		sendKeys(driver, discharge_Service_TF_AN, Discharge_Service_Input);
		waitForElement(driver, discharge_Service_Select);
		doubleClick(driver, discharge_Service_Select);

		Step_End(4, "Enter the discharge service ", test, test1);
		waitForElement(driver, discharge_Service_TF_AN);
		if(Discharge_Service_TF_Check.equalsIgnoreCase("Y")) {
			if(isElementAccessible(driver, discharge_Service_TF_AN)) {
				Extent_pass(driver, "Matched || Expected Result is : Discharge Service TF Should be Enable || Actual Result is : Discharge Service TF is Enabled", test, test1);
				System.out.println("Matched || Expected Result is : Discharge Service TF Should be Enable || Actual Result is : Discharge Service TF is Enabled");
			}else {
				System.out.println("Not Matched || Expected Result is : Discharge Service TF Should be Enable || Actual Result is : Discharge Service TF is Not Enabled");
				Extent_fail(driver,"Not Matched || Expected Result is : Discharge Service TF Should be Enable || Actual Result is : Discharge Service TF is Not Enabled", test, test1);
			}
		}

		if(Show_Button_Check.equalsIgnoreCase("Y")) {
			if(isElementAccessible(driver, show_Btn_An)) {
