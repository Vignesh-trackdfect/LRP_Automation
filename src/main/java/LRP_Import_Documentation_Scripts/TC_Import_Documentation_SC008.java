...
public class TC_Import_Documentation_SC008 extends Keywords{

	public void Import_Documentation_SC008(WebDriver driver, ExtentTest test,ExtentTest test1,String selected_dataset,Map<String, String> Excel_data,String url) {
		String testCaseName="TC_Import_Documentation_SC008";
        ...
        String Get_Master_BL_SAVED = Excel_data.get("Get_Master_BL_SAVED");
		...
		if(Get_BL_Status.equalsIgnoreCase("NEW")) {
			...
			if(BL_Act_Popup.equals(SavedPopup)) {
				...
				Extent_pass_New(driver, "Matched || Expected Popup was : " + SavedPopup + " || Actual Popup was : " + BL_Act_Popup, test,
						test1);
		...
		Step_End(17, "save the import documentation", test, test1);
			...
			Step_Start(18, "import documentation status has to be shown as Master Bl saved.", test, test1);

			waitForElement(driver, BL_status_IMP_DOC);
			String Get_BL_New = getText(driver, BL_status_IMP_DOC);
			System.out.println("Get_BL_Status:"+ Get_BL_New);
			
			if(Get_BL_New.equals(Get_Master_BL_SAVED)) {
				System.out.println("Matched || Expected BL Status : "+Get_BL_New +" || Actual  BL Status : "+Get_Master_BL_SAVED);
				Extent_pass_New(driver, "Matched || Expected BL Status : "+Get_BL_New +" || Actual BL Status : "+Get_Master_BL_SAVED, test, test1);
			}else {
				System.out.println("Not Matched || Expected BL Status : "+Get_BL_New +" || Actual BL Status : "+Get_Master_BL_SAVED);
				Extent_fail(driver, "Not Matched || Expected BL Status : "+Get_BL_New +" || Actual BL Status : "+Get_Master_BL_SAVED, test, test1);

			}
			
			Step_End(18, "import documentation status has to be shown as Master Bl saved.", test, test1);
			
			Step_Start(19, "once saved the import documentation ,in tool bar delete icon is available", test, test1);
			Step_Start(20, "click the delete icon.", test, test1);
			Step_Start(21, "delete the import documentation.", test, test1);

			if(Delete_Perform.equalsIgnoreCase("YES")){
				deleteImportDocument(driver, Delete_Confirmation_Popup, Delete_Success_Popup);
			}
			
			Step_End(21, "delete the import documentation.", test, test1);
			Step_End(20, "click the delete icon.", test, test1);
			Step_End(19, "once saved the import documentation ,in tool bar delete icon is available", test, test1);
