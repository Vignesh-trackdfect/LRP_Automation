...
public class TC_Discharge_Reconciliation_MD30C extends Keywords {
	public void Discharge_Reconciliation_MD30C(WebDriver driver, ExtentTest test, ExtentTest test1, String selected_dataset,Map<String, String> Excel_data,String url) {

		String testcase_Name = "TC_Discharge_Reconciliation_MD30C";
		...
		//	String select_Option_3 = String.format(DropDown_Select, Plan_Input);
			...
			Step_Start(10, "Capture the BLs List and store it  WT", test, test1);
		...
		//	reArrangeAG_GridColumnsNew(driver, Cargo_Detials_Column_btn, Column_Header);
			...
			waitForElement(driver, Cargo_WT_Detials);
			...
			Step_End(10, "Capture the BLs List and store it  WT", test, test1);
			...
			Step_Start(16, "Verify that Make sure BLs matched these fields- WT", test, test1);
			

			List<WebElement> tableCells = driver.findElements(By.xpath("//div[@id='OPS-OPS_OpsListGrid_tbl']//div[@col-id='weight' and @role='gridcell']"));
			...
			if (Dis_WT_Code==totalSum) {
				System.out.println("Pass : The expected WT Detials from Discharge Reconciliation is : "+ Dis_WT_Code +" || The actual WT Detials from Operetion List View is : "+ totalSum );
				Extent_pass(driver, "Pass : The expected WT Detials from Discharge Reconciliation is : "+ Dis_WT_Code +" || The actual WT Detials from Operetion List View is : "+ totalSum , test, test1);
			} else {
				System.out.println(" Fail : The expected WT Detials from Discharge Reconciliation is : "+ Dis_WT_Code +" || The actual WT Detials from Operetion List View is : "+ totalSum );
				Extent_fail(driver, "Fail : The expected WT Detials from Discharge Reconciliation is : "+ Dis_WT_Code +" || The actual WT Detials from Operetion List View is : "+ totalSum , test, test1);
			
			}
			Step_End(16, "Verify that Make sure BLs matched these fields- WT", test, test1);
