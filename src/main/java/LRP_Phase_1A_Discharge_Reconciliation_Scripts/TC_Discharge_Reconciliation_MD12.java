...
public class TC_Discharge_Reconciliation_MD12 extends Keywords{
	
	public void Discharge_Reconciliation_MD12(WebDriver driver, ExtentTest test, ExtentTest test1,
			String selected_dataset, Map<String, String> Excel_data, String url) {

		
			String test_Case_Name = "TC_Discharge_Reconciliation_MD12";
		...
		String DC_Condition_Filter = Excel_data.get("DC_Condition_Filter");
		  ...
		  waitForElement(driver, popup_Message_Yes_Button);
			...
			Step_Start(8, "system shows message as Import Documentation has deleted", test, test1);
			  ...
			  waitForElement(driver, popup_Message_Ok_Button);
				...
				Step_Start(10,"Make sure that below configuration is marked as Y ISB-Import Documentation Allow to save import documentation for Work order service BL",test, test1);
			    ...
			    Step_End(10,"Make sure that below configuration is marked as Y ISB-Import Documentation Allow to save import documentation for Work order service BL",test, test1);
				...
				Step_Start(16, " Verify that service BL is loaded in Discharge recon module.", test, test1);
				reArrangeAG_GridColumnsNew(driver, Discharge_recon_cocloum_click,DC_Column_Header);
				AGGridValueFilterMultipleValue(driver, DC_Column_Header,DC_Condition_Filter,Bl_Number);
				String GivenBLlisted = getText(driver, Text_Get_Yes_No);
				if (Bl_Number.equals(GivenBLlisted)) {
				System.out.println("Pass || Expected result: Given Bl should listed as same in impdoc transfer module " +Bl_Number + "Actual result:Given Bl is listed as same in impdoc transfer module" + GivenBLlisted);
				Extent_pass(driver,
						"Pass || Expected result: Expected result: Given Bl should listed as same in impdoc transfer module " +Bl_Number + " Actual result:Given Bl is listed as same in impdoc transfer module" + GivenBLlisted,
						test, test1);

					
				} else {
					System.out.println("Fail || Expected result: Given Bl should listed as same in impdoc transfer module" +Bl_Number + "Actual result:Given Bl is not  listed as same in impdoc transfer module" + GivenBLlisted);
					Extent_fail(driver,
							"Fail || Expected result: Expected result: Given Bl should listed as same in impdoc transfer module" +Bl_Number + "Actual result:Given Bl is notlisted as same in impdoc transfer module" + GivenBLlisted,
							test, test1);
				...
				Step_End(16, "Verify that service BL is loaded in Discharge recon module. ", test, test1);
				
				Step_Start(17, " Open Import Documentation Load the BL and Verify that import documentation is saved.", test, test1);
				...
				twoColumnSearchWindow(driver,BL_twocoloumn_impdoc , Agency_condition,Bl_Number );
				...
				if (Saved_popup.equals(Actual_save_popup)) {
					...
					Extent_fail(driver, "Fail: The Excepted popup is : "+Saved_popup+"|| The Actual Popup is : "+Actual_save_popup, test, test1);
					
		 
				}
		 
				Step_End(17, "17. Open Import Documentation Load the BL and Verify that import documentation is saved.", test, test1);
