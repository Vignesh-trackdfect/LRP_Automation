...
public class TC_Feeder_Contract_SC36 extends Keywords {

	public void Feeder_Contract_SC36(WebDriver driver, ExtentTest test, ExtentTest test1, String selected_dataset,
			Map<String, String> Excel_data, String url) {

		String testcase_Name = "TC_Feeder_Contract_SC36";
		...
		waitForElement(driver, FC_Feeder_Contract_Input_Details);
		String FC_Input_Page = getText(driver, FC_Feeder_Contract_Input_Details);
		...
		Step_Start(5, "Click cancel ,Without enter the value in the mandatory fields", test, test1);

		waitForElement(driver, FC_Add_Cancel);
		click(driver, FC_Add_Cancel);

		waitForElement(driver, FC_GridPOL);
		String PoL_Value = getText(driver, FC_GridPOL);

		if (PoL_Value.isEmpty()) {
			System.out.println(
					"Expected :After clicking on Cancel button without enter the value in the mandatory fields Empty row should be added || Actual : After clicking on Cancel button without enter the value in the mandatory fields Empty row is added");
			Extent_pass_New(driver,
					"Expected :After clicking on Cancel button without enter the value in the mandatory fields Empty row should be added || Actual : After clicking on Cancel button without enter the value in the mandatory fields Empty row is added",
					test, test1);
		} else {
			System.out.println(
					"Expected :After clicking on Cancel button without enter the value in the mandatory fields Empty row should not be added || Actual : After clicking on Cancel button without enter the value in the mandatory fields Empty row is not added");
			Extent_pass_New(driver,
					"Expected :After clicking on Cancel button without enter the value in the mandatory fields Empty row should not be added || Actual : After clicking on Cancel button without enter the value in the mandatory fields Empty row is not added",
					test, test1);
		...
		Step_End(5, "Click cancel ,Without enter the value in the mandatory fields", test, test1);
