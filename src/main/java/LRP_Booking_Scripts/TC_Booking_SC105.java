...
public class TC_Booking_SC105 extends Keywords{
//	check whether in booking, based on BLF global configuration mentined input value voy_bnd optional then system allowed different Loadbound and discharge bound in booking and system set same load bound as discharge bound
	
	public void Booking_SC105(WebDriver driver, ExtentTest test,ExtentTest test1,String selected_dataset,Map<String, String> Excel_data,String url) {
		
		String testCaseName="TC_Booking_SC105";
		...
		//Get data from Excel sheet
				...
				String first_service_data2 = Excel_data.get("first_service_data2");
				String Voyagenumber2 = Excel_data.get("Voyagenumber2");
				String vesselCode2 = Excel_data.get("VesselCode2");
...
//	Global configuration
		...
		Step_Start(1, "Set BLF Global Configuration as VOY", test, test1);
		...
		Step_End(1, "Set BLF Global Configuration as VOY", test, test1);
...
//	Feeder schedule
		...
		globalValueSearchWindow(driver, search_by_condition, select_by_service_code, first_service_data2, select_by_vesselcode, vesselCode2, select_by_voyage, Voyagenumber2);

		waitForElement(driver, ETA_arr_date);
		List<WebElement> eta_Count = listOfElements(driver, eta_Dates);
		int total=eta_Count.size();
		
		String ETA_Arr_Date=String.format(Eta_Date_Final, total);
	...
	// Rates 	
			...
			if(isDisplayed(driver, load_Voyage)) {
				...
				if(loadVoyage.equals(dischargeVoyage) && pod_ETA_LegTab.equals(eta_ArrDate)) {
					...
					Extent_pass_New(driver, "Not Matched || Expected Load Voyage : "+loadVoyage + " || Actual Discharge Voyage : "+dischargeVoyage, test, test1);
					System.out.println("Not Matched || Expected Load Voyage : "+loadVoyage + " ||Actual  Discharge Voyage : "+dischargeVoyage);
					
				}else {
					System.out.println("Matched || Expected Load Voyage : "+loadVoyage + " || Actual Discharge Voyage : "+dischargeVoyage +"\r\n ETA Date in Feeder Schedule: "+eta_ArrDate+" || ETA Date in Booking Leg Tab: "+pod_ETA_LegTab);
					Extent_fail(driver, "Matched || Expected Load Voyage : "+loadVoyage + " ||Actual  Discharge Voyage : "+dischargeVoyage +"<br>"+"ETA Date in Feeder Schedule: "+eta_ArrDate+" || ETA Date in Booking Leg Tab: "+pod_ETA_LegTab, test, test1);
				}
			}else {
