...
public class TC_Import_Invoice_SC008D extends Keywords {

	public void Import_Invoice_SC008D(WebDriver driver, ExtentTest test, ExtentTest test1, String selected_dataset,Map<String, String> Excel_data,String url) {

		String testcase_Name = "TC_Import_Invoice_SC008D";
		...
		String Refresh_popupMessage = Excel_data.get("Refresh_popupMessage");
		...
		waitForElement(driver, Close_DTE);
		click(driver, Close_DTE);

		Step_End(11, "Close the Module", test, test1);

		Extent_cal(test, test1, "For Draft status");
		...
		Extent_cal(test, test1, "For Refresh with New Mode");
		...
		Step_Start(17, "Click on Edit Button", test, test1);

		waitForElement(driver, Edit_Button_toolBar);
		click(driver, Edit_Button_toolBar);

		Step_End(17, "Click on Edit Button", test, test1);

		Step_Start(18, "Click on Refresh button", test, test1);

		waitForElement(driver, RefreshButton);
		click(driver, RefreshButton);

		Step_End(18, "Click on Refresh button", test, test1);

		Step_Start(19, "Validation shows 'Do you want to refresh the BL Yes/No Click on Yes Button'", test, test1);

		waitForPopup(driver, popup_Message, Refresh_popupMessage);

		String popuptextMessage = getText(driver, popup_Message);

		if (popuptextMessage.equals(Refresh_popupMessage)) {
			System.out.println("Matched || Expected Popup : " + popuptextMessage + " || Actual Popup : "
					+ Refresh_popupMessage);
			Extent_pass_New(driver, "Matched || Expected Popup : " + popuptextMessage + " || Actual Popup : "
					+ Refresh_popupMessage, test, test1);
		} else {
			System.out.println("Not Matched || Expected Popup : " + popuptextMessage + " || Actual Popup : "
					+ Refresh_popupMessage);
			Extent_fail(driver, "Not Matched || Expected Popup : " + popuptextMessage + " || Actual Popup : "
					+ Refresh_popupMessage, test, test1);
		}
		...
		Step_End(19, "Validation shows 'Do you want to refresh the BL Yes/No Click on Yes Button'", test, test1);
		waitForElement(driver, SaveButton_ToolBar);
		click(driver, SaveButton_ToolBar);

		waitForPopup(driver, popup_Message, Invoice_saved_pop);

		String import_saved_actual = getText(driver, popup_Message);
		System.out.println(import_saved_actual);

		if (Invoice_saved_pop.equals(import_saved_actual)) {
			System.out.println(
					"Matched || Expected Popup : " + Invoice_saved_pop + " || Actual Popup : " + import_saved_actual);
			Extent_pass_New(driver,
					"Matched || Expected Popup : " + Invoice_saved_pop + " || Actual Popup : " + import_saved_actual,
					test, test1);
		} else {
			System.out.println("Not Matched || Expected Popup : " + Invoice_saved_pop + " || Actual Popup : "
					+ import_saved_actual);
			Extent_fail(driver, "Not Matched || Expected Popup : " + Invoice_saved_pop + " || Actual Popup : "
					+ import_saved_actual, test, test1);
		}

		waitForElement(driver, popup_Message_Ok_Button);
		click(driver, popup_Message_Ok_Button);

		Extent_pass_New(driver, "Expected : Extention details listed after the Import invoice refreshed. ||   Actual : Extention details listed after the Import invoice refreshed.", test, test1);
