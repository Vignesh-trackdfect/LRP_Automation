...
public class TC_Movement_Entry_TS_015 extends Keywords {
	...
	public void Movement_Entry_TS_015(WebDriver driver, ExtentTest test, ExtentTest test1, String selected_dataset,
		...
		String savedPopupText = Excel_data.get("SavedPopup_ME");
		String datePicker = Excel_data.get("DatePicker");
		String activityDateSingle = Excel_data.get("StartDate");
		...
		String Value_Taken_From_E2E = Excel_data.get("Value_Taken_From_E2E");
		String Test_DataSet_Column_For_Value = Excel_data.get("Test_DataSet_Column_For_Value");
		String TestCase_Key_Value = Excel_data.get("TestCase_Key_Value");
...
//		LRP_Login(driver, username, password);
		...
		Step_Start(1, "Enter the module name in the search box and click on the corresponding module from the list",
				test, test1);
...
//		Module Search
		...
		Step_End(1, "Enter the module name in the search box and click on the corresponding module from the list", test,
				test1);
		...
		Step_Start(5,
				"Click \"Show Input Fields\", and if a confirmation prompt appears for the check digit, click \"Yes\" to proceed.",
				test, test1);
		...
		Step_End(5,
				"Click \"Show Input Fields\", and if a confirmation prompt appears for the check digit, click \"Yes\" to proceed.",
				test, test1);
		Step_Start(6, "Based on the provided data sheet, ensure the script fills the fields in the correct sequence.",
				test, test1);
		//		Location
		if (!Location.equals("")) {
...
//			Remarks
		...
		Step_End(6, "Based on the provided data sheet, ensure the script fills the fields in the correct sequence.",
				test, test1);
		...
		Excel_data.put("New_Equipment_ID", ActualEqNo);
//		Utils.writeOrUpdateExcelValue("TestData_End_to_End", "End_To_End_SC001", "TC_End_To_End_SC001", "Dataset1", "New_Equipment_ID",ActualEqNo);
		...
		Utils.writeOrUpdateExcelValue(TC_File_Path, TC_Sheet_Name, testCaseName1, Testcases.dataSet,Value_Taken_From_E2E,ActualEqNo);
			scrollTop(driver);
		waitForElement(driver, SaveButton_ToolBar);
		click(driver, SaveButton_ToolBar);

		Step_End(7, "Finally, click the \"Save\" button from the toolbar.", test, test1);
		Step_Start(8, "A \"Single Entry Save\" success message should be displayed upon successful submission.", test,
				test1);
		...
		if (actText.equals(savedPopupText)) {
			System.out.println(
					"Matched || " + " Expected popup is : " + savedPopupText + " || Actual popup is : " + actText);
			Extent_pass_New(driver, "Matched || " + " Expected popup is : " + savedPopupText + " || Actualpopup is : "
			...
			System.out.println(
					"Not matched || " + " Expected popup is : " + savedPopupText + " || Actual popup  is : " + actText);
			System.out.println(savedPopupText + " popup is not displayed");
			Extent_fail(driver, "Not matched || " + " Expected popup is : " + savedPopupText + " || Actual popup is : "
					+ actText + "\r\n" + savedPopupText + " popup is not displayed", test, test1);
		}

		Extent_pass_New(driver, "Actual Equipment Number : " + ActualEqNo, test, test1);

		Step_End(8, "A \"Single Entry Save\" success message should be displayed upon successful submission.", test,
				test1);
