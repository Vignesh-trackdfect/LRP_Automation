...
public class TC_Contract_Management_System_SC051 extends Keywords {
	public void Contract_Management_System_SC51(WebDriver driver, ExtentTest test, ExtentTest test1,
			String selected_dataset, Map<String, String> Excel_data, String url) {

		String testCaseName = "TC_Contract_Management_System_SC051";
		...
		String ImportFilename1 = Excel_data.get("ImportFilename");
		...
		Step_Start(8, "Click route/S.ch and save and Retrieve the contract in the global search", test, test1);
		...
		Step_End(8, "Click route/S.ch and save and Retrieve the contract in the global search", test, test1);

		Step_Start(9, "click edit", test, test1);

		waitForElement(driver, Contract_Management_System_Edit);
		click(driver, Contract_Management_System_Edit);

		Step_End(9, "click edit", test, test1);
		...
		waitForDisplay(driver, ImportInput);
		uploadfile_new(driver, ImportInput, ImportFilename1);

		String ExcelrowCount1 = rowcountxls(driver, ImportFilename, MainSheetName);
		...
		waitForElement(driver, PortpairGridCount);
		String gridcount1 = getText(driver, PortpairGridCount).trim();

		if (gridcount1.equals(ExcelrowCount1)) {

			System.out.println("All the row values are imported in the portpair grid || Expected rows : "
					+ ExcelrowCount1 + " Actual rows : " + gridcount1);
			Extent_pass(driver, "All the row values are imported in the portpair grid || Expected rows : "
					+ ExcelrowCount1 + " Actual rows : " + gridcount1, test, test1);
		} else {
		...
		if (isdisplayed(driver, CMS_Verify_Duplicate_Table_Page)) {
			Extent_pass(driver, "After Clicking verify Button , Duplicate Records page should be displayed  "
					+ DuplicateReportTable, test, test1);
		} else {
			Extent_fail(driver,
					"After Clicking verify Button , Duplicate Records page not displayed  " + DuplicateReportTable,
					test, test1);
		}
