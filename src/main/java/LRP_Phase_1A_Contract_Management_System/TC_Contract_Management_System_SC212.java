...
public class TC_Contract_Management_System_SC212  extends Keywords{

	public void Contract_Management_System_SC212(WebDriver driver, ExtentTest test, ExtentTest test1,
			String selected_dataset,Map<String, String> Excel_data,String url) {

		String testCaseName = "TC_Contract_Management_System_SC212";
		...
		Step_Start(6, "Import the port pair lanes with DG YES and without the DG details in DG tab", test, test1);
		...
		if (isdisplayed(driver, CMS_Verify_Duplicate_Table_Page)) {
			...
			String DuplicateReportTable = TableDataForReport(driver, tableData, "DG missing details", bookingHeaders);
 
			String DG_missing_popup = getText(driver, CMS_DG_Missing_window_Row);
			System.out.println("DG_Missing_popup is " + DG_missing_popup);
 
			if (DG_Details_Missing_popup.equals(DG_missing_popup)) {
 
				Extent_pass(driver, "After Clicking ON upload Button , DG missing details should be displayed  "
						+ DuplicateReportTable, test, test1);
			} else {
				Extent_fail(driver, "After Clicking ON upload Button , DG missing details should be displayed  "
						+ DuplicateReportTable, test, test1);
			}
			...
			Extent_fail(driver,
					"After Clicking ON upload Button ,Import the port pair lanes with DG YES and without the DG details in DG tab DG missing details is be displayed, wew will  not get this Window ",
					test, test1);
		}
 
		Step_End(6, "Import the port pair lanes with DG YES and without the DG details in DG tab", test, test1);
