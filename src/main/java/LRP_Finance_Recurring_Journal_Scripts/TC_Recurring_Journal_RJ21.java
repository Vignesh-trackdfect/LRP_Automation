...
public class TC_Recurring_Journal_RJ21 extends Keywords{
	public void Recurring_Journal_RJ21(WebDriver driver, ExtentTest test, ExtentTest test1,String selected_dataset,Map<String, String> Excel_data,String url) throws EncryptedDocumentException, InvalidFormatException, IOException{
		String testcase_Name="TC_Recurring_Journal_RJ21";
		...
		String Closed_status = Excel_data.get("Closed_status");
		...
		waitForElement(driver, Search_Button_RJ);
		click(driver, Search_Button_RJ);
		Step_Start(2, "Select the transactions", test, test1);
		globalValueSearchWindow(driver, Condition, Select_search_recurring_journal, Recurring_number, "", "", "", "");
		Step_End(1, "Click Global search ,serach the recurring no", test, test1);
		...
		Step_Start(6, "Click \"Posting Date\"radio button and select the \"From date and To date\" from the date picker", test, test1);
		waitForElement(driver, Posting_status_RadioBtn_RJ);
		click(driver, Posting_status_RadioBtn_RJ);
		...
		Step_End(6, "Click \"Posting Date\"radio button and select the \"From date and To date\" from the date picker", test, test1);
		Step_Start(7, "Choose the Post type as OPEN/CLOSED/ALL", test, test1);
		...
		System.out.println("Type_Of_Status:"+Type_Of_Status);
		Step_End(7, "Choose the Post type as OPEN/CLOSED/ALL", test, test1);
		...
		Step_Start(9, "Recurring JV should be showed", test, test1);
		...
		for(int i=0;i<Batch_Status.size();i++) {
			if(Closed_status.equalsIgnoreCase(Batch_Status.get(i))) {
				System.out.println("Matched || Batch status Taken From Grid : " + Batch_Status.get(i) + " || Batch Status Selected : " + Closed_status);
				System.out.println("Posted records should be showed in Closed Status");
				Extent_pass_New(driver,"Matched || Batch status Taken From Grid : " + Batch_Status.get(i) + " || Batch Status Selected : " + Closed_status, test,test1);
			} else {
				System.out.println("Not Matched || atch status Taken From Grid : " + Batch_Status.get(i) + " || Batch Status Selected : " + Closed_status);
				Extent_fail(driver,"Not Matched || Batch status Taken From Grid  : " + Batch_Status.get(i) + " || Batch Status Selected  : " + Closed_status,test, test1);
			}
		}
		Step_End(9, "Recurring JV should be showed", test, test1);
