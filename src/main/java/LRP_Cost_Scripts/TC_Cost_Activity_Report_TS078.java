...
public class TC_Cost_Activity_Report_TS078 extends Keywords {
	...
	public void  Cost_Activity_Report_TS078(WebDriver driver, ExtentTest test, ExtentTest test1,String selected_dataset,Map<String, String> Excel_data,String url) throws Exception{
		...
		String currency_data_vc = Excel_data.get("currency_data_vc");
		String wantToApprove_Popup = Excel_data.get("WantToApprove_Popup");
		...
		String Contract_Code_search = Excel_data.get("Contract_Code_search");
		String Contract_Code_Search_Gam = Excel_data.get("Contract_Code_Search_Gam");
		...
		if(Activitity_Module_Perform.equalsIgnoreCase("Yes")) {
			...
			waitForDisplay(driver, new_button_Act_Mod); // This for Activity New button 
			if(isdisplayed(driver, new_button_Act_Mod)) {
				click(driver, new_button_Act_Mod);
			}
			
			newButton(driver);  // This is for toolbar New button click
	...
	Step_Start(10, "Select the required main activity, right-click on it, and select the 'Remove_Btn_CAR selected items' option. Click 'Yes'.", test, test1);
		...
		waitForPopup(driver, popup_Message,wantToApprove_Popup);
		...
		if(actual_Popup.equals(wantToApprove_Popup)) {
			...
			Extent_pass_New(driver, "Matched || Expected Popup : "+wantToApprove_Popup+" || Actual Popup : "+actual_Popup, test, test1);
			click(driver, popup_Message_Yes_Button);
		}
		else {
