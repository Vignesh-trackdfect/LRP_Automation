...
public class TC_Service_Creation_SC01 extends Keywords{	
	...
	public void Service_Creation_SC01(WebDriver driver, ExtentTest test,ExtentTest test1,String selected_dataset,Map<String, String> Excel_data,String url) {
		...
		String Service_Code_new = Excel_data.get("Service_Code");
		...
		//	Create new Service
			String service_Code = generateChecksumForService(Service_Code_new);
			...
			Utils.writeOrUpdateExcelValue(TC_File_Path,TC_Sheet_Name, testCaseName1, Testcases.dataSet, "Service_Code_new", service_Code);
			Excel_data.put("Service_Code_new", service_Code);
		...
		if(date_Picker.equalsIgnoreCase("Yes")) {
			selectDatePickerWithTime_CS(driver, Start_Date, startDate);
		}else {
		...
		if(date_Picker.equalsIgnoreCase("Yes")) {
			selectDatePickerWithTime_CS(driver, End_Date, endDate);
		}else {
			if(endDate.contains("-")) {
		...
		Utils.writeOrUpdateExcelValue("TestData_End_to_End", "End_To_End_SC001", "TC_End_To_End_SC001", "Dataset1", "Service_Code_new",service_Code);
