...
public class TC_DG_Creation_SC_061 extends Keywords {


	public void DG_Creation_SC_061(WebDriver driver, ExtentTest test, ExtentTest test1,
			String selected_dataset,Map<String, String> Excel_data,String url) {
		//		Get data from excel sheet

		String testCaseName="TC_DG_Creation_SC_061";
...
//		Module Search
		...
		Step_Start(3, "Edit ems value.  system not allow to edit ", test, test1);
		waitForDisplay(driver, emsTextField);
		scrollElementToView(driver, emsTextField);
		if(elementNotEditable(driver, emsTextField)) {
			System.out.println("Matched || " + " Expected : ems Name Text field should not be editabe || Actual : ems Name Text field is not editabe");            
			Extent_pass_New(driver, "Matched || " + " Expected : ems Name Text field should not be editabe || Actual : ems Name Text field is not editabe", test,test1);     
		}                                                                                                                                                                        
		else {                                                                                                                                                                   
			System.out.println("Not Matched || " + " Expected : ems Name Text field should not be editabe || Actual : ems Name Text field is editabe");        
			Extent_fail(driver, "Not Matched || " + " Expected : ems Name Text field should not be editabe || Actual : ems Name Text field is editabe", test,test1);
		}
		
		Step_End(3, "Edit ems value.  system not allow to edit ", test, test1);
