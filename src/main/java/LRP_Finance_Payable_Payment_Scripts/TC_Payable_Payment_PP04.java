...
public class TC_Payable_Payment_PP04 extends Keywords{
	
	public void Payable_Payment_PP04(WebDriver driver, ExtentTest test, ExtentTest test1,String selected_dataset,Map<String, String> Excel_data,String url) {
		
		String testCaseName="TC_Payable_Payment_PP04";
		...
		String AccountHead_Header_input =Excel_data.get("AccountHead_Header_input");
		...
		if (current_Date.equals(paymentDate)) {
			...
			Extent_pass_New(driver,"Matched || Payment Date Should be Automatically Select Based On Month || Expected Current Date : "+current_Date+" || The Actual Payment Date  : "+paymentDate+"Payment Date Automatically Selected Based On Month", test, test1);
		} else {
		...
		Step_Start(8, "Select the payment A/c", test, test1);
		waitForElement(driver, payment_AC_SearchButton_PPT);
		click(driver,payment_AC_SearchButton_PPT);
		
		 twoColumnSearchWindow(driver,Account_Name_Header,dropdownCondition, accountName);

	
		Step_End(8, "Select the payment A/c", test, test1);
   	...
   	if(!documentAmt.equals("")) {
		...
		Step_Start(9, "Enter the Bank charges and select the bank nominal", test, test1);
		...
		Step_End(9, "Enter the Bank charges and select the bank nominal", test, test1);
		Step_Start(10, "Enter the ROE Difference and select ROE/Diff value", test, test1);
		...
		twoColumnSearchWindow(driver, AccountHead_Header, dropdownCondition, AccountHead_Header_input);

		
		}
		
		Step_End(10, "Enter the ROE Difference and select ROE/Diff value", test, test1);
		Step_End(11, "Click on Add button without \"selecting the invoice\"", test, test1);

		waitForElement(driver, add_Button_Cash_Bank_Details_PPT);
		click(driver, add_Button_Cash_Bank_Details_PPT);
		
		Step_End(11, "Click on Add button without \"selecting the invoice\"", test, test1);
		Step_Start(12, "system should show pop up validation as \"Select the Invoice\".", test, test1);
		...
		Step_End(12, "system should show pop up validation as \"Select the Invoice\".", test, test1);
