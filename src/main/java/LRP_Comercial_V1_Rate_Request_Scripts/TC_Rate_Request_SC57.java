...
public class TC_Rate_Request_SC57  extends Keywords {
	...
	public void Rate_Request_SC57(WebDriver driver, ExtentTest test, ExtentTest test1, String selected_dataset,Map<String, String> Excel_data,String url)
			throws Exception {
...
newButton(driver);
		...
		Actionsendkeys(driver, Commodity_Input, commodity);
			...
			if(actualPopup.equals(wantToProceedPopup)) {
				Extent_pass_New(driver,"Matched || Expected : popup message should be shown as : "+wantToProceedPopup+" || Actual : popup message is shown as : "+ actualPopup, test,test1);
				System.out.println("Matched || Expected : popup message should be shown as : "+wantToProceedPopup+" Actual : popup message shown"+ actualPopup);
		...
		click(driver, Mail_Cancel_button);
			...
			String actual_To_Date = getFirstValueByHeader(SRR_Table_Value, "Valid To");
			if(isSecondDateBeforeFirst(actual_From_Date, actual_To_Date)) {
				System.out.println("Matched || After the Rate Request '2' is Approved the Rate Request 1's valid to "+actual_To_Date+" changed to one day before the valid from2 "+actual_From_Date);
				Extent_pass_New(driver, "Matched || After the Rate Request '2' is Approved the Rate Request 1's valid to "+actual_To_Date+" changed to one day before the valid from2 "+actual_From_Date, test, test1);
			}else {
				System.out.println("Not Matched || After the Rate Request '2' is Approved the Rate Request 1's valid to "+actual_To_Date+" Not changed to one day before the valid from2 "+actual_From_Date);
				Extent_fail(driver, "Not Matched || After the Rate Request '2' is Approved the Rate Request 1's valid to "+actual_To_Date+" Not changed to one day before the valid from2 "+actual_From_Date, test, test1);
