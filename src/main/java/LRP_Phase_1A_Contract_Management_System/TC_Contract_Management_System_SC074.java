...
public class TC_Contract_Management_System_SC074 extends Keywords{

	public void Contract_Management_System_SC74(WebDriver driver, ExtentTest test,ExtentTest test1, String selected_dataset,Map<String, String> Excel_data,String url)  {
		 
		String testCaseName = "TC_Contract_Management_System_SC074";
		...
		Step_Start(6, "Import the port pair lanes with no surcharge in master", test, test1);
		...
		Step_End(6, "Import the port pair lanes with no surcharge in master", test, test1);
		...
		if (BackRoundColor.equals(color1)) {
			System.out.println("Excepted ||  Imported port pair lanes with no surcharge lanes in the master data should be highlighted in: " + BackRoundColor + " || Actual Imported  Excel port pair lanes with no surcharge lanes is Highlighted in " + color1);
			Extent_pass(driver, "Excepted||  Imported port pair lanes with no surcharge lanes in the master data should be highlighted in: "  + BackRoundColor+ " || Actual  Imported Excel  port pair lanes with no surcharge lanes is Highlighted in " + color1, test, test1);
			temp = false;
			break;
		}
		}
		...
		if(temp) {
			System.out.println("UnExpected || Imported port pair lanes with no surcharge lanes in the master data should be highlighted in: " + BackRoundColor + " || Actual imported Excel file does not contain any lanes with surcharge lanes.");
			Extent_fail(driver,"UnExpected || Imported port pair lanes with no surcharge lanes in the master data should be highlighted in: " + BackRoundColor + " || Actual imported Excel file does not contain any lanes with surcharge lanes." , test, test1);
		}
