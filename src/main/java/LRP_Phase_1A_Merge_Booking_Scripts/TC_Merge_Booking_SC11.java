...
public class TC_Merge_Booking_SC11 extends Keywords{

public void Merge_Booking_SC11(WebDriver driver, ExtentTest test, ExtentTest test1, String selected_dataset,Map<String, String> Excel_data,String url) throws Exception {
		
		
	    String testCaseName="TC_Merge_Booking_SC11";
		...
		String cancelled_booking_draft_Color = Excel_data.get("cancelled_booking_draft_Color");
		...
		String Merge_Booking_popup = Excel_data.get("Merge_Booking_popup");
		String Split_Booking_Module_Name = Excel_data.get("Split_Booking_Module_Name");
		String Search_booking = Excel_data.get("Search_booking");
		String Search_Condition = Excel_data.get("Search_Condition");
		String Splip_Booking_popup = Excel_data.get("Splip_Booking_popup");
		...
		Step_Start(1, "Create 2 containerised booking in confirmed status and In plugin configuration enable SBO plugin as true", test, test1);	
...
//	Master tab		
		...
		Step_End(1, "Create 2 containerised booking in confirmed status and In plugin configuration enable SBO plugin as true", test, test1);
		
		
		Step_Start(2, " Open merge booking module", test, test1);	
		
		moduleNavigate(driver, Merge_Booking_Module_Name);
		
		Step_End(2, " Open merge booking module", test, test1);
		
		Step_Start(3, "Enter input values for service,vessel,voyage,bound,PLA,POL,POD,PLD and agreement party.", test, test1);
		...
		waitForElement(driver, POL_Search_Button);
		click(driver, POL_Search_Button);
		twoColumnSearchWindow(driver, POL_Header, Condition, Booking_POL);
		
		waitForElement(driver, POD_Search_Button);
		click(driver, POD_Search_Button);
		...
		waitForElement(driver, MB_Confirmed_Button);
		click(driver, MB_Confirmed_Button);
		...
		Step_End(3, "Enter input values for service,vessel,voyage,bound,PLA,POL,POD,PLD and agreement party.", test, test1);
		
		Step_Start(4, "Then below the PLA field draft and confirm button is there in that select confirmed option", test, test1);
		
		
		
		Step_End(4, "Then below the PLA field draft and confirm button is there in that select confirmed option", test, test1);
		
		Step_Start(5, "Then click on show button system will list the available booking based on the input criteria", test, test1);
		
		waitForElement(driver, MB_Show_btn);
		click(driver, MB_Show_btn);
		
		Step_End(5, "Then click on show button system will list the available booking based on the input criteria", test, test1);
		
		Step_Start(6, " first select one booking from available booking tab and move the book no to booking to merge tab by clicking on right side button in between the two tabs", test, test1);
		
		formatLocatorClick(driver, MB_BookNum_Select, booknum);
		waitForElement(driver, MB_Next_Button);
		click(driver, MB_Next_Button);
		
		Step_End(6, " first select one booking from available booking tab and move the book no to booking to merge tab by clicking on right side button in between the two tabs", test, test1);
		
		
		Step_Start(7, "again select another booking and same process to move right side", test, test1);
		
		formatLocatorClick(driver, MB_BookNum_Select, booknum1);
		waitForElement(driver, MB_Next_Button);
		click(driver, MB_Next_Button);
		
		Step_End(7, "again select another booking and same process to move right side", test, test1);
		
		
		Step_Start(8, " Then on the screen click on merge button .", test, test1);
		waitForElement(driver, MB_Merge_btn);
		click(driver, MB_Merge_btn);
		Step_End(8, " Then on the screen click on merge button .", test, test1);
		
		
        Step_Start(9, " then system ask for confirmation for merge booking.", test, test1);
        waitForElement(driver, popup_Message);
		click(driver, popup_Message_Yes_Button);
		Step_End(9, "then system ask for confirmation for merge booking.", test, test1);
		
		
		
        Step_Start(10, " then merged booking message will be shown ", test, test1);
		...
		Step_End(10, " then merged booking message will be shown ", test, test1);
		
		  Step_Start(11, "Then 2 draft booking get cancelled and new booking will be created with old booking details.", test, test1);
		...
		if(draft_bookingColor11.equals(cancelled_booking_draft_Color)) {                                                                                                                  
			System.out.println("Matched || [ Booking number input field Should be change to Red colour ] Expected Report Activity is : " + cancelled_booking_draft_Color + " || The Actual Report Activity is : " + draft_bookingColor11+" [Booking number input field is changed to Red colour] ");            
			Extent_pass_New(driver,"Matched || [ Booking number input field Should be change to Red colour ] Expected Report Activity is : " + cancelled_booking_draft_Color + " || The Actual Report Activity is : " + draft_bookingColor11+" [Booking number input field is changed to Red colour] ", test, test1);     
		}else {                                                                                                                                                                   
			System.out.println("Not Matched || [ Booking number input field Should be change to Red colour ] Expected Report Activity is : " + cancelled_booking_draft_Color + " || The Actual Report Activity is : " + draft_bookingColor11+" [Booking number input field is Not changed to Red colour] ");        
			Extent_fail(driver,"Not Matched || [ Booking number input field Should be change to Red colour ] Expected Report Activity is : " + cancelled_booking_draft_Color + " || The Actual Report Activity is : " + draft_bookingColor11+" [Booking number input field is Not changed to Red colour] ", test, test1); 
		}
		...
		waitForElement(driver, SearchButton_Toolbar);
		click(driver, SearchButton_Toolbar);
		globalValueSearchWindow(driver, dd1_condition, select_by_book_number, booknum1, "", "", "", "");
		...
		if(draft_bookingColor2.equals(cancelled_booking_draft_Color)) {                                                                                                                  
			System.out.println("Matched || [ Booking number input field Should be change to Red colour ] Expected Report Activity is : " + cancelled_booking_draft_Color + " || The Actual Report Activity is : " + draft_bookingColor2+" [Booking number input field is changed to Red colour] ");            
			Extent_pass_New(driver,"Matched || [ Booking number input field Should be change to Red colour ] Expected Report Activity is : " + cancelled_booking_draft_Color + " || The Actual Report Activity is : " + draft_bookingColor2+" [Booking number input field is changed to Red colour] ", test, test1);     
		}else {                                                                                                                                                                   
			System.out.println("Not Matched || [ Booking number input field Should be change to Red colour ] Expected Report Activity is : " + cancelled_booking_draft_Color + " || The Actual Report Activity is : " + draft_bookingColor2+" [Booking number input field is Not changed to Red colour] ");        
			Extent_fail(driver,"Not Matched || [ Booking number input field Should be change to Red colour ] Expected Report Activity is : " + cancelled_booking_draft_Color + " || The Actual Report Activity is : " + draft_bookingColor2+" [Booking number input field is Not changed to Red colour] ", test, test1); 
		}
		
		
		  Step_End(11, "Then 2 draft booking get cancelled and new booking will be created with old booking details.", test, test1);
		
        Step_Start(12, " if same no try do merge/split then system won't allow with a validation.", test, test1);
       
        waitForElement(driver,Merge_Booking);
		click(driver, Merge_Booking);
        
        waitForElement(driver, MB_Show_btn);
		click(driver, MB_Show_btn);
        
        
        formatLocatorClick(driver, MB_BookNum_Select, Merged_bookingNo);
		waitForElement(driver, MB_Next_Button);
		click(driver, MB_Next_Button);
        
        
        waitForPopup(driver, popup_Message, Merge_Booking_popup);
        String update_popup = getText(driver, popup_Message);
        
		if(update_popup.equals(Merge_Booking_popup)) {
		    System.out.println("Matched || Expected value is : After click on Merge Booking Number the popup  message should be : "+update_popup+" || Actual value is : After click on Merge Booking Number the popup  message is : "+ Merge_Booking_popup);
		    Extent_pass_New(driver, "Matched || Expected value is : After click on Merge Booking Number the popup  message should be : "+update_popup+" || Actual value is : After click on Merge Booking Number the popup  message is : "+ Merge_Booking_popup, test, test1);
		}else {
			System.out.println("Not Matched || Expected value is : After click on Merge Booking Number the popup  message should be : "+update_popup+" || Actual value is : After click on Merge Booking Number the popup  message is not display :"+ Merge_Booking_popup);
			Extent_fail(driver,"Not Matched || Expected value is : After click on Merge Booking Number the popup  message should be: "+update_popup+" || Actual value is : After click on Merge Booking Number the popup  message is not display : "+ Merge_Booking_popup, test,test1);
		...
		Step_End(12, " if same no try do merge/split then system won't allow with a validation.", test, test1);
		
		
        Step_Start(13, "After operation job complete then system will allow to split or merge the book no's.", test, test1);
        
        moduleNavigate(driver, Split_Booking_Module_Name);
        
    	waitForDisplay(driver, SB_booking_search_checkbox);
		click(driver, SB_booking_search_checkbox);
        
		
		waitForElement(driver, SB_bookingnio_search_btn);
		click(driver, SB_bookingnio_search_btn);
		globalValueSearchWindow(driver, Search_Condition, Search_booking, Merged_bookingNo, "", "", "", "");
		
		waitForDisplay(driver, SB_Show_btn);
		click(driver, SB_Show_btn);
		
		 waitForPopup(driver, popup_Message, Splip_Booking_popup);
	        String update_popup1 = getText(driver, popup_Message);
	        
			if(update_popup1.equals(Merge_Booking_popup)) {
			    System.out.println("Matched || Expected value is : After click on Merge Booking Number the popup  message should be : "+update_popup+" || Actual value is : After click on Merge Booking Number the popup  message is : "+ Splip_Booking_popup);
			    
			    Extent_pass(driver,"Matched || Expected value is : After click on Merge Booking Number the popup  message should be : "+update_popup+" || Actual value is : After click on Merge Booking Number the popup  message is : "+ Splip_Booking_popup, test,test1);
			}else {
				System.out.println("Not Matched || Expected value is : After click on Merge Booking Number the popup  message should be : "+update_popup+" || Actual value is : After click on Merge Booking Number the popup  message is not display :"+ Splip_Booking_popup);
				
				Extent_fail(driver,"Not Matched || Expected value is : After click on Merge Booking Number the popup  message should be: "+update_popup+" || Actual value is : After click on Merge Booking Number the popup  message is not display : "+ Splip_Booking_popup, test,test1);
			...
			if(Merge_Booking_popup.equals(Splip_Booking_popup)) {
				System.out.println("Matched || Expected popup message was : " + Merge_Booking_popup
						+ " || Actual popup message was : " + Splip_Booking_popup);
				Extent_pass_New(driver, "Matched || Expected popup message was : " + Merge_Booking_popup
						+ " || Actual popup message was : " + Splip_Booking_popup, test, test1);
			} else {

				System.out.println("Not Matched || Expected popup message was : " + Merge_Booking_popup
						+ " || Actual popup message was : " + Splip_Booking_popup);
				Extent_fail(driver, "Not Matched || Expected popup message was : " + Merge_Booking_popup
						+ " || Actual popup message was : " + Splip_Booking_popup, test, test1);
		...
		Step_End(13, "After operation job complete then system will allow to split or merge the book no's.", test, test1);
