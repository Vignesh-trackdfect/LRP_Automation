...
public class TC_Delivery_Order_TS_034 extends Keywords{
	public void Delivery_Order_TS_034(WebDriver driver, ExtentTest test, ExtentTest test1,String selected_dataset,Map<String, String> Excel_data,String url) throws EncryptedDocumentException, InvalidFormatException, IOException{

		String testcase_Name="TC_Delivery_Order_TS_034";
		...
		String DO_Saved_EXP = Excel_data.get("DO_Saved_EXP");
		...
		List<String> banknames = splitAndExpand(bankname, "/");
		...
		for(int i = 0;i<BL_Numbers.size();i++) {
			...
			for(int j = 0;j<doubleList.size();j++) {
				if (Double.compare(doubleList.get(j), 0.0) != 0.0) {
					Step_Start(15, "Click new and click customer search field ", test, test1);
					newButton(driver);
					List<String> d_Banks = splitAndExpand(deposit_banks.get(i), ",");
					List<String> Bank = splitAndExpand(banknames.get(i), ",");
					...
					if(isdisplayed(driver, retrivedGlobalValue)) {
						...
						twoColumnSearchWindow(driver, Deposite_Bank_Name_Column_Header, Condition, d_Banks.get(j));
		...
		for(int i = 0;i<BL_Numbers.size();i++) {
			act_HBL_Numbers = splitAndExpand(HBL_Numbers.get(i), ",");
			for( int j = 1;j<act_HBL_Numbers.size();j++) {
				if(act_HBL_Numbers.size()>1) {
					Step_Start(37, "Go to import documentation screen", test, test1);
					moduleNavigate(driver, ImportDocumentationModule);
				...
				for(int k = 0;k<doubleList.size();k++) {
					if (Double.compare(doubleList.get(k), 0.0) != 0.0) {
						newButton(driver);
						List<String> d_Banks = splitAndExpand(deposit_banks.get(j), ",");
						List<String> Bank = splitAndExpand(banknames.get(j), ",");
						waitForElement(driver, Invoice_currency_search_Button);
						click(driver, Invoice_currency_search_Button);
						...
						if(isdisplayed(driver, retrivedGlobalValue)) {
							...
							waitForElement(driver, bank_Search_Icon);
							click(driver, bank_Search_Icon);
							twoColumnSearchWindow(driver, Search_Bank_Name_Column_Header, Condition, Bank.get(k));
							//deposit bank
							waitForElement(driver, deposit_Bank_Search_Icon);
							click(driver, deposit_Bank_Search_Icon);
							twoColumnSearchWindow(driver, Deposite_Bank_Name_Column_Header, Condition, d_Banks.get(k));
							waitForElement(driver, add_Payment_Details);
							click(driver, add_Payment_Details);
							waitForElement(driver, InvoiceAndReceipt_Tab);
							click(driver, InvoiceAndReceipt_Tab);
		...
		act_HBL_Numbers = splitAndExpand(HBL_Numbers, ",");
		for(int i = 0;i<act_HBL_Numbers.size();i++) {
			Step_Start(4, "Click the \" ADD \" Button", test, test1);
			waitForElement(driver, DO_Add_Button);
			click(driver, DO_Add_Button);
			Step_End(4, "Click the \" ADD \" Button", test, test1);
			Step_Start(5, "Paste the  BL number, click search  and fetch the record", test, test1);
			globalValueSearchWindow(driver, dropdownCondition, globalSearchFilterdelivery, act_HBL_Numbers.get(i), "", "", "", "");
			Step_End(5, "Paste the  BL number, click search  and fetch the record", test, test1);
		}
		Step_Start(6, "Click the SHOW button", test, test1);
		waitForElement(driver, DO_show);
		doubleClick(driver, DO_show);
		Step_End(6, "Click the SHOW button", test, test1);
		Step_Start(7, "Select the container details in below grid", test, test1);
		waitForElement(driver, select_All_Checkbox_DO);
		click(driver, select_All_Checkbox_DO);
		Step_End(7, "Select the container details in below grid", test, test1);
		Step_Start(8, "Click the Save button", test, test1);
		scrollTop(driver);
		waitForElement(driver, SaveButton_ToolBar);
		jsClick(driver, SaveButton_ToolBar);
		Step_End(8, "Click the Save button", test, test1);
		Step_Start(9, "save Delivery Order and capture the validation as \" Delivery Order saved\"", test, test1);
		waitForPopup(driver,Popup_Message,DO_Saved_EXP);
		DO_POP_ACT = getText(driver, Popup_Message);
		if (DO_Saved_EXP.equals(DO_POP_ACT)) {
			System.out.println("Matched || Expected Pop-up value is : " + DO_Saved_EXP + " || Actual Pop-up value is : " + DO_POP_ACT);
			Extent_pass_New(driver,"Matched || Expected Pop-up value is : " + DO_Saved_EXP + " || Actual Pop-up value is : " + DO_POP_ACT, test,test1);
			Extent_call(test, test1, DO_POP_ACT);
		} else {
			System.out.println("NotMatched || Expected Pop-up value is : " + DO_Saved_EXP + " || Actual Pop-up value is : " + DO_POP_ACT);
			Extent_fail(driver,"NotMatched || Expected Pop-up value is : " + DO_Saved_EXP + " || Actual Pop-up value is : " + DO_POP_ACT,test, test1);
		}
		Step_End(9, "save Delivery Order and capture the validation as \" Delivery Order saved\"", test, test1);
		Step_Start(42, "Make sure Delivery order saved and Delivery order number generated", test, test1);
		String deliveryOrderNo = getAttribute(driver, Deliver_Order_No_TF, "value");
		if(!deliveryOrderNo.equals(null)) {
			System.out.println("Matched ||  Actual Delivery Order Number is : " + deliveryOrderNo);
			Extent_pass_New(driver, "Matched ||  Actual Delivery Order Number is : " + deliveryOrderNo, test, test1);
			System.out.println("The Select the BL/HBL numbers Are" + act_HBL_Numbers);
			Extent_pass_New(driver, "The Select the BL/HBL numbers Are" + act_HBL_Numbers, test, test1);
		} else {
			System.out.println("Not Matched || Delivery Order Number is Not Created ");
			Extent_fail(driver, "Not Matched || Delivery Order Number is Not Created ", test, test1);
		}
		Step_End(42, "Make sure Delivery order saved and Delivery order number generated", test, test1);
		if(delete_Option.equalsIgnoreCase("Yes")) {
			...
			for(int i = 0;i<Reciept_Numbers.size();i++) {
				click(driver, SearchButton_Toolbar);
				globalValueSearchWindow1(driver, dropdownCondition, globalSearchFilterOption_ChequeRealization, Reciept_Numbers.get(i), "", "", "", "");
				waitForDisplay(driver, retrivedGlobalValue);
				if(isdisplayed(driver, retrivedGlobalValue)) {
					waitForElement(driver, retrivedGlobalValue);
					click(driver, retrivedGlobalValue);
					...
					waitForElement(driver, Delete_button_toolBar);
					jsClick(driver, Delete_button_toolBar);
					waitForDisplay(driver, Cheque_Realizaation_Ok_Button);
					if(isdisplayed(driver, Cheque_Realizaation_Ok_Button)) {
						click(driver, Cheque_Realizaation_Ok_Button);
					}
					waitForDisplay(driver, popup_Message_Yes_Button);
					if(isdisplayed(driver, popup_Message_Yes_Button)) {
						click(driver, popup_Message_Yes_Button);
			...
			for(int i = 0;i<Reciept_Numbers.size();i++) {
				...
				globalValueSearchWindow1(driver, dropdownCondition, globalSearchFilterOption_Receipt2, Reciept_Numbers.get(i), "", "", "", "");
			...
			for(int i = 0;i<Invoice_Numbers.size();i++) {
				...
				globalValueSearchWindow1(driver, dropdownCondition, globalSearchFilterOption_InvoiceReversal, Invoice_Numbers.get(i), "", "", "", "");
				if(isdisplayed(driver, retrivedGlobalValue)) {
					waitForElement(driver, retrivedGlobalValue);
					click(driver, retrivedGlobalValue);
					waitForElement(driver, SelectButton);
