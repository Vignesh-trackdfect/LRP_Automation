...
public class TC_Discharge_Reconciliation_MD05 extends Keywords {

	public void Discharge_Reconciliation_MD05(WebDriver driver, ExtentTest test, ExtentTest test1,
			String selected_dataset, Map<String, String> Excel_data, String url)
			throws EncryptedDocumentException, InvalidFormatException {

		String test_Case_Name = "TC_Discharge_Reconciliation_MD05";
		...
		for (int i = 1; i <= HeaderCount; i++) {
			...
			if (!isDisplayed(driver, columnheaders)) {
				horizontalscroll(driver, GridScroll_DR, 1500);
			}

			if (isDisplayed(driver, columnheaders)) {
				String headername = getText(driver, columnheaders).trim();
				System.out.println("headername : " + headername);
				GridHeaders.add(headername);
			} else {
				System.out.println("Header " + i + " is still not visible after scroll.");
			}
		}

		System.out.println("GridHeaders : " + GridHeaders);
		Step_End(8, "Click on excel Button and download it.", test, test1);


		Step_End(9, "Open the Excel sheet ", test, test1);

		Step_Start(10, "Verify that count of Header.", test, test1);

		int actualHeaderCount = getColumnCount(downloadfile_path1);

		int ExpectedHeaderCount = GridHeaders.size();
		System.out.println("Expected header count: " + ExpectedHeaderCount);
		
		System.out.println("Actual header count: " + actualHeaderCount);

		if (actualHeaderCount == ExpectedHeaderCount) {
			
			   System.out.println("Pass || Expected result:  coloumn  count headers From Excel" + actualHeaderCount + "|| Actual:Coloumn count headers From Application" + ExpectedHeaderCount);
			    Extent_pass(driver, "Pass || Expected result: Coloumn  count headers From Excel"  + actualHeaderCount + " || Actual:Coloumn count headers From Application" + ExpectedHeaderCount, test, test1);
			} else {
			    System.out.println("Fail || Expected: Coloumn  count headers From Excel" + actualHeaderCount + "  || Actual: Coloumn count headers From Application"+ ExpectedHeaderCount );
			    Extent_fail(driver, "Fail || Expected: Coloumn  count headers From Excel" + actualHeaderCount + " || Actual: Coloumn count headers From Application"  + ExpectedHeaderCount, test, test1);

		
			

		}
		Step_End(10, "Verify that count of Header.", test, test1);
