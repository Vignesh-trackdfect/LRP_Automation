...
public class TC_Rate_Request_SC090 extends Keywords{
	
	public void Rate_Request_SC090(WebDriver driver, ExtentTest test, ExtentTest test1, String selected_dataset,
			Map<String, String> Excel_data, String url) {

	String testcase_Name = "TC_Rate_Request_SC090";
	...
	Step_Start(9, "click on the DG check box and enter the DG details.", test, test1);
	...
	Step_End(9, "click on the DG check box and enter the DG details.", test, test1);
	...
	safeclick(driver, Auto_Panel_First);
	System.out.println("Delivery Value:" + delivery);
	Step_End(19, "Enter the Delivery", test, test1);

	Step_Start(20, "Enter the Commodity.", test, test1);
	...
	Step_Start(21, "Enter the Equipment type.", test, test1);
	waitForElement(driver, Eqp_Type);
	click(driver, Eqp_Type);
	sendKeys(driver, Eqp_Type, eqpType);
	waitForElement(driver, Auto_Panel_First);
	safeclick(driver, Auto_Panel_First);
	System.out.println("Eqp type:"+eqpType);
	Step_End(21, "Enter the Equipment type.", test, test1);

	Step_Start(22, " Enter the Quantity", test, test1);

	waitForElement(driver, Eqp_Quantity);
	click(driver, Eqp_Quantity);
	Newclear(driver, Eqp_Quantity);
	Actionsendkeys(driver, Eqp_Quantity,Eqp_Quantity_Input );
	Step_End(22, " Enter the Quantity", test, test1);

	Step_Start(23, "Enter the Gross Weight", test, test1);

	waitForElement(driver, RR_Gross_Weight);
	Newclear(driver, RR_Gross_Weight);
	Actionsendkeys(driver, RR_Gross_Weight, grossWeight);
	Step_End(23, "Enter the Gross Weight", test, test1);
	
	Step_Start(24, "click on the DG check box and enter the DG details.", test, test1);

	checkBox(driver, DG_checkbox, DG_checkboxs);
	 
	
	waitForElement(driver, dgDetails_Frame);
	String actDGDetailsFrame1=getText(driver, dgDetails_Frame);
	if(actDGDetailsFrame1.equals(dGDetailsFrame)) {
		Extent_pass_New(driver,"Matched || Expected : "+dGDetailsFrame+" Actual : "+ actDGDetailsFrame1, test,test1);
		System.out.println("Matched || Expected : "+dGDetailsFrame+" Actual : "+ actDGDetailsFrame1);
		Extent_pass_New(driver, dGDetailsFrame+" is displayed", test,test1);
		System.out.println(dGDetailsFrame+" is displayed");
		waitForElement(driver, class_Search_Button);
		click(driver, class_Search_Button);
		...
		System.out.println("Not Matched || Expected : "+dGDetailsFrame+" Actual : "+ actDGDetailsFrame1);
		Extent_fail(driver,"Not Matched || Expected : "+dGDetailsFrame+" Actual : "+ actDGDetailsFrame1, test,test1);
	}
	...
	Step_End(24, "click on the DG check box and enter the DG details.", test, test1);

	Step_Start(25, "Click routing", test, test1);

	waitForElement(driver, Routing_Button);
	click(driver, Routing_Button);
	Step_End(25, "Click routing", test, test1);


	Step_Start(26, "Select the routing", test, test1);
	...
	Step_End(26, "Select the routing", test, test1);
	rateRequestRoutingDays(driver, Exp_Det_Days_Input, Imp_Det_Days_Input, Exp_Demmurage_Input, Imp_Demmurage_Input);
	rateRequestPaymentMode_Select(driver, payment_Mode,Select_type_location, condition, payLocation);
	Step_Start(27, "Enter the OFR", test, test1);
	waitForElement(driver, OFR_Txt_Field);
	safeclick(driver, OFR_Txt_Field);
	sendKeys(driver, OFR_Txt_Field, oFR);

	Step_End(27, "Enter the OFR", test, test1);

	Step_Start(28, "Click Add", test, test1);
	waitForElement(driver, Rate_AddBtn);
	click(driver, Rate_AddBtn);
	...
	if(actualPopup1.equals(wantToProceedPopup)) {
		Extent_pass_New(driver,"Matched || Expected : popup message should be shown as : "+wantToProceedPopup+" || Actual : popup message is shown as : "+ actualPopup1, test,test1);
		System.out.println("Matched || Expected : popup message should be shown as : "+wantToProceedPopup+" || Actual : popup message is shown as : "+ actualPopup1);
		...
		System.out.println("Not Matched || Expected : popup message should be shown as : "+wantToProceedPopup+" || Actual : popup message is shown as : "+ actualPopup1);

		Extent_fail(driver,"Not Matched || Expected : popup message should be shown as : "+wantToProceedPopup+" || Actual : popup message is shown as : "+ actualPopup1, test,test1);
	}
	Step_End(30, "Click submit", test, test1);
	
	
	Step_Start(31, "Click Ok", test, test1);
	...
	Step_Start(31, "click edit.", test, test1);
	
	waitForElement(driver, Edit_Button_toolBar);
	click(driver, Edit_Button_toolBar);
	
	Step_End(31, "click edit.", test, test1);

	
	Step_Start(32, "Click save", test, test1);

	
	waitForElement(driver, SaveButton_ToolBar);
	click(driver, SaveButton_ToolBar);
	
	Step_End(32, "Click save", test, test1);
	
	Step_Start(33, "Click Submit.", test, test1);
	...
	Step_End(33, "Click Submit.", test, test1);
