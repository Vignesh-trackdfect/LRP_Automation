...
public class TC_Discharge_Reconciliation_MD25 extends Keywords{
	
public void Discharge_Reconciliation_MD25(WebDriver driver,ExtentTest test,ExtentTest test1,String selected_dataset,Map<String, String> Excel_data,String url) {
		
		String testcase_Name="TC_Discharge_Reconciliation_MD25";
		...
		Step_Start(9, "Capture the BL NO count in Cargo section", test, test1);
		// show Non-container btn 
		  waitForElement(driver, Bl_No_Cargo_det);
		  
		  waitForElement(driver,DR_Cargo_count_of_num);
		  String DR_Cargo_count_of_no=getText(driver, DR_Cargo_count_of_num); 
...
//			click(driver, DR_Show_container_clbtn);	
		Step_End(9, "Capture the BL NO count in Cargo section", test, test1);
...
//		click(driver, OLV_Filter_btn);	
		...
		Step_Start(16, "Distinct the BL number List", test, test1);
		 int i=0;
	        ...
	        int BL_Num_Cout=newValue1.size();
		
		Step_End(16, "Distinct the BL number List", test, test1);
		
		Step_Start(17, "Verify that Make sure BLs count are Matched in Operation List view and Non-Container summary", test, test1);
		if(DR_Cargo_count_of_no.equals(String.valueOf(BL_Num_Cout))) {
			 System.out.println("Matched || Expected :  BLs count are Matched in Operation List view as "+BL_Num_Cout+ " || Actual  : BLs are listed in Operation List view count : Non-Container summary count : "+ DR_Cargo_count_of_no);
			 Extent_pass(driver,"Matched || Expected :  BLs count are Matched in Operation List view as "+BL_Num_Cout+ " || Actual  : BLs are listed in Operation List view count : Non-Container summary count : "+ DR_Cargo_count_of_no,test, test1);
		 }else {
			 System.out.println("Not Matched || Expected :  BLs count are Matched in Operation List view as "+BL_Num_Cout+ " || Actual  : BLs are listed in Operation List view count : "+ DR_Cargo_count_of_no);
			 Extent_fail(driver,"Not Matched || Expected :  BLs count are Matched in Operation List view as "+BL_Num_Cout+ " || Actual  : BLs are listed in Operation List view count : Non-Container summary count : "+ DR_Cargo_count_of_no,test, test1);
		 }
