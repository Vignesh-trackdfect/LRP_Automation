...
public class TC_Rate_Request_SC38 extends Keywords {
	...
	public void Rate_Request_SC38(WebDriver driver, ExtentTest test, ExtentTest test1, String selected_dataset,Map<String, String> Excel_data,String url)
			throws Exception {
...
//		String password = Excel_data.get("Password");
		...
		String grossWeight = Excel_data.get("GrossWeight_RR");
		...
		String Routing_Service = Excel_data.get("Service_Code_new");
		String Eqp_Quantity_Input = Excel_data.get("Eqp_Quantity_Input");
		String Cust_code = Excel_data.get("Cust_code");
		String Condition_Value = Excel_data.get("dropdown_Condition");
		String Customer_code_Value = Excel_data.get("New_Customer_Code");
		String To_Date_Input = Excel_data.get("Rate_EndDate");
		String date_Picker_RR_38 = Excel_data.get("DatePicker");
		String From_Date_Input = Excel_data.get("Rate_StartDate");
...
//		LRP_Login(driver, username, password);
		...
		rateRequestDateSelect(driver, date_Perform, date_Picker_RR_38, From_Date_Input, To_Date_Input);
		...
		List<String> origin_Values = splitAndExpand(origin, ",");
		List<String> delivery_Values = splitAndExpand(delivery, ",");
		List<String> eqpType_Values = splitAndExpand(eqpType, ",");
		List<String> OFR_Values = splitAndExpand(OFR, ",");
		for(int i=0;i<eqpType_Values.size();i++) {

			Step_Start(3, "Enter the origin", test, test1);
			waitForElement(driver, Orgin_Input);
			Newclear(driver, Orgin_Input);
			sendKeys(driver, Orgin_Input, origin_Values.get(i));
			waitForElement(driver, Auto_Panel_First);
			safeclick(driver, Auto_Panel_First);
			...
			Newclear(driver, RR_Delivery_Input);
			sendKeys(driver, RR_Delivery_Input, delivery_Values.get(i));
			waitForElement(driver, Auto_Panel_First);
			safeclick(driver, Auto_Panel_First);
			...
			Newclear(driver, Eqp_Type);
			sendKeys(driver, Eqp_Type, eqpType_Values.get(i));
			waitForElement(driver, Auto_Panel_First);
			safeclick(driver, Auto_Panel_First);
			...
			if(isDisplayed(driver, routingSearch_Frame)) {
				...
				sendKeys(driver, Routing_OriginTF, origin_Values.get(i));
			...
			sendKeys(driver, OFR_Txt_Field, OFR_Values.get(i));
		...
		if(!actualReqNo.equals(null)) {
			Extent_pass_New(driver,"Matched || After clik ok button Request Number  is number should be generated : "+ actualReqNo + "  is  generated", test,test1);
			System.out.println("Matched || After clik ok button Request Number  is number should be generated : "+actualReqNo+ "  is  generated");

		}else {
			System.out.println("Not Matched || After clik ok button Request Number  is number should be generated : "+ actualReqNo+ " Request Number  is not generated");

			Extent_fail(driver,"Not Matched || After clik ok button Request Number  is number should be generated : "+ actualReqNo + " Request Number  is not generated", test,test1);
		}
	...
	if (actGridRequestStatus.equals(gridRequestStatus)) {
		...
		if(Filed.equals(FILED)) {
			Extent_pass_New(driver,"Matched || Expected Result is : After click on save button The Status should be : "+Filed+" || Actual Result is : : click on save button The Status is : "+ FILED, test,test1);
			System.out.println("Matched || Expected Result is : After click on save button The Status should be : "+Filed+" || Actual Result is : : click on save button The Status is : "+ FILED);
		}else {
			System.out.println("Not Matched || Expected Result is : After click on save button The Status should be : "+Filed+" || Actual Result is : : click on save button The Status is : "+ FILED);
			Extent_fail(driver,"Not Matched || Expected Result is : After click on save button The Status should be : "+Filed+" || Actual Result is : : click on save button The Status is : "+ FILED, test,test1);
		}
