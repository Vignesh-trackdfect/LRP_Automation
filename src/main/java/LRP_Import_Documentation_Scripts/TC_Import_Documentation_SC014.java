...
public class TC_Import_Documentation_SC014 extends Keywords {

	public void Import_Documentation_SC014(WebDriver driver, ExtentTest test, ExtentTest test1, String selected_dataset,Map<String, String> Excel_data,String url) {
		String testCaseName="TC_Import_Documentation_SC014";
		...
		String BL_Saved_Status_Check = Excel_data.get("BL_Saved_Status_Check");	
		...
		Step_Start(15, "saved BL's are shown with master bl saved status for the given service.", test, test1);
		...
		List<String> BL_Saved_Status_Values=splitAndExpand(BL_Saved_Status_Check, ",");
		boolean Bl_Status_Check=true;
		...
		for(int i=1;i<Endvalue;i++) {
			...
			if(!BL_Saved_Status_Values.contains(BL_Status_Act)) {
				UnMatched_BL_Numbers.add(BL_NumberValue);
				Bl_Status_Check=false;
			}
		}
		if(Bl_Status_Check) {
			System.out.println("Matched || Expected : All the BL Number's Status should be equalt to "+BL_Saved_Status_Check+" When selected 'Saved' in the dropdown || Actual : All the BL Number's Status equal to "+BL_Saved_Status_Check);
			Extent_pass_New(driver, "Matched || Expected : All the BL Number's Status should be equalt to "+BL_Saved_Status_Check+" When selected 'Saved' in the dropdown || Actual : All the BL Number's Status equal to "+BL_Saved_Status_Check, test, test1);
		}else {
			System.out.println("Not Matched || Expected : All the BL Number's Status should be equalt to "+BL_Saved_Status_Check+" When selected 'Saved' in the dropdown || Actual : Some BL Number's Status is not equal to "+BL_Saved_Status_Check+" || BL Status Mismatched BL Numbers : "+UnMatched_BL_Numbers);
			Extent_fail(driver, "Not Matched || Expected : All the BL Number's Status should be equalt to "+BL_Saved_Status_Check+" When selected 'Saved' in the dropdown || Actual : Some BL Number's Status is not equal to "+BL_Saved_Status_Check+" || BL Status Mismatched BL Numbers : "+UnMatched_BL_Numbers, test, test1);
		}
		
		Step_End(15, "saved BL's are shown with master bl saved status for the given service.", test, test1);
