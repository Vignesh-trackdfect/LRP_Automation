...
public class TC_Detention_Waiver_TS_028 extends Keywords {

	public void Detention_Waiver_TS_028(WebDriver driver, ExtentTest test, ExtentTest test1, String selected_dataset,Map<String, String> Excel_data,String url)
			throws Throwable {

		String testcase_Name = "TC_Detention_Waiver_TS_028";
		...
		Step_Start(5, "Verify that % Radio button and % Text Field  are available to enter Values", test, test1);
		scrollUsingElement(driver, DW_Total_records);
		if (formatLocatorIsDisplayed(driver, DW_Radio_Button, Radio_Button_Option)) {
			System.out.println(
					"Expected result : Expected Weiver % radio button should  displayed\"  || Actual result :  Weiver % radio button   displayed\"");
			Extent_pass_New(driver,
					"Expected result : Expected Weiver % radio button should  displayed\" || Actual result :Weiver % radio button   displayed\\\"",
					test, test1);
		
			
			Extent_call(test, test1, "Verify that % Radio button and % Text Field  are available to enter Values");
			
			formatLocatorClick(driver, DW_Radio_Button, Radio_Button_Option);
			...
			System.out.println(
					"Expected result : Expected Weiver % radio button should  displayed\"|| Actual result : Weiver % radio button not  displayed\\\"\"");
			Extent_fail(driver,
					"Expected result :Expected Weiver % radio button should  displayed\"|| Actual result : Weiver % radio button   not displayed\\\"",
					test, test1);
		}
		Step_End(5, "Verify that % Radio button and % Text Field  are available to enter Values", test, test1);
