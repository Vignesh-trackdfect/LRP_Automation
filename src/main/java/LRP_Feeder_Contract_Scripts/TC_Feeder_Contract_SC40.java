...
public class TC_Feeder_Contract_SC40 extends Keywords {
	public void Feeder_Contract_SC40(WebDriver driver, ExtentTest test, ExtentTest test1, String selected_dataset,
			Map<String, String> Excel_data, String url) {
		String testcase_Name = "TC_Feeder_Contract_SC40";
		...
		waitForDisplay(driver, Excel_Gird_First_Cell);
		
		if (isdisplayed(driver, Excel_Gird_First_Cell)) {
			Extent_pass_New(driver,
					"Expected result : Imported data's should be uploaded in the grid || Actual result : Imported data's are reflected in the grid",
					test, test1);
			System.out.println(
					"Expected result : Imported data's should be uploaded in the grid || Actual result : Imported data's are reflected in the grid");
		} else {
			Extent_FailNew(driver,
					"Expected result : Imported data's should be uploaded in the grid || Actual result : Imported data's are not reflected in the grid",
					test, test1);
			System.out.println(
					"Expected result : Imported data's should be uploaded in the grid || Actual result : Imported data's are not reflected in the grid");
		}
