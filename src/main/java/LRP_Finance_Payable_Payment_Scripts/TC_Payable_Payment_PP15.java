...
public class TC_Payable_Payment_PP15 extends Keywords{
	
public void Payable_Payment_PP15(WebDriver driver, ExtentTest test, ExtentTest test1,String selected_dataset,Map<String, String> Excel_data,String url) {
		
		
		String testcase_Name="TC_Payable_Payment_PP15";
		...
		String actual_Payment_Date =null;
		if(payment_Date.equals("")) {
			...
			waitForElement(driver, paymentDate_PPT);
			 actual_Payment_Date = getAttribute(driver, paymentDate_PPT, "value");
		...
		Step_Start(8, "In Cash/Bank Detail, date should be the same as payment date", test, test1);
	
		waitForPopup(driver, date_Cash_Bank_Details_PPT,actual_Payment_Date);
		
		String date_Of_Cash_Bank_Tab = getAttribute(driver, date_Cash_Bank_Details_PPT, "value");
		
		if(actual_Payment_Date.equals(date_Of_Cash_Bank_Tab)) {
					
			System.out.println("BOTH ACTUAL PAYMENT DATE AND CASH_BANK DATES ARE EQUAL");
			System.out.println("MATCHED || Expeted : After click on In Cash/Bank Detail ,Date should be the same as payment date : "+actual_Payment_Date+" Actual The payment date  : "+date_Of_Cash_Bank_Tab+"Click on In Cash/Bank Detail ,Date is the same as payment date");
			Extent_pass_New(driver, "MATCHED || Expeted : After click on In Cash/Bank Detail ,Date should be the same as payment date "+actual_Payment_Date+"  Actual The payment date  : "+date_Of_Cash_Bank_Tab+"Click on In Cash/Bank Detail ,Date is the same as payment date", test, test1);
		}else {
			
			System.out.println("BOTH ACTUAL PAYMENT DATE AND CASH_BANK DATES ARE NOT EQUAL");
			System.out.println("Not MATCHED || Expeted : After click on In Cash/Bank Detail ,Date should be the same as payment date : "+actual_Payment_Date+" Actual The payment date : "+date_Of_Cash_Bank_Tab+"Click on In Cash/Bank Detail ,Date is the Not same as payment date");
			Extent_fail(driver, "Not MATCHED || Expeted : After click on In Cash/Bank Detail ,Date should be the same as payment dateS : "+actual_Payment_Date+" Actual The payment date  : "+date_Of_Cash_Bank_Tab+"Click on In Cash/Bank Detail ,Date is the Not same as payment date", test, test1);
		
			
		}
		
		
		Step_End(8, "In Cash/Bank Detail, date should be the same as payment date", test, test1);
