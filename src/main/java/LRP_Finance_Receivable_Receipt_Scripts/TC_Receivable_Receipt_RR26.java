...
public class TC_Receivable_Receipt_RR26 extends Keywords{
	public void Receivable_Receipt_RR26(WebDriver driver, ExtentTest test, ExtentTest test1,String selected_dataset,Map<String, String> Excel_data,String url) throws EncryptedDocumentException, InvalidFormatException, IOException{

		String testcase_Name="TC_Receivable_Receipt_RR26";
		...
		String Receipt_Popup = Excel_data.get("Receipt_Popup");
		...
		Step_Start(7, "Enter the negative value in receipt amount column.", test, test1);
		...
		String amount_Value = getValueByFirstColumnAndHeader(Invoice_Table_Data,Trans_Number_Header, Transaction_Number, Get_Amout_Value_From_Header);
		...
		if(!Roe_Difference_Amount.trim().equals("")) {
			...
			Newclear(driver, roe_Differencfe_Textfield_RR);
			sendKeys(driver, roe_Differencfe_Textfield_RR, Roe_Difference_Amount);
			waitForElement(driver, roe_Differencfe_Textfield_RR);
		...
		Step_Start(14, "System should show the pop up message as Check the receipt amount", test, test1);	
		waitForPopup(driver, popup_Message,Receipt_Popup);
		
		String Popup=getText(driver, popup_Message);
		if(Receipt_Popup.equals(Popup)) {
			System.out.println("Matched || Expected : After click on save button Popup value should be display in the Receivable Receipt Screen : " + Receipt_Popup + " || Actual : After click on save button Popup value is display in the Receivable Receipt Screen : " + Popup);
			Extent_pass_New(driver,"Matched || Expected : After click on save button Popup value should be display in the Receivable Receipt Screen : " + Receipt_Popup + " || Actual : After click on save button Popup value is display in the Receivable Receipt Screen : " + Popup, test,test1);

		} else {
			System.out.println("Not Matched || Expected : After click on save button Popup value should be display in the Receivable Receipt Screen : " + Receipt_Popup + " ||Actual : After click on save button Popup value is display in the Receivable Receipt Screen : " + Popup);
			Extent_fail(driver,"Not Matched || Expected : After click on save button Popup value should be display in the Receivable Receipt Screen : " + Receipt_Popup + " || Actual : After click on save button Popup value is display in the Receivable Receipt Screen :" + Popup,test, test1);
		}
		waitForElement(driver, popup_Message_Ok_Button);
		click(driver, popup_Message_Ok_Button);

		
		Step_End(14, "System should show the pop up message as Check the receipt amount", test, test1);
