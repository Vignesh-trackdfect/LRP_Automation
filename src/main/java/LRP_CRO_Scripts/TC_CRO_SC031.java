...
public class TC_CRO_SC031 extends Keywords {

	public void CRO_SC031(WebDriver driver, ExtentTest test, ExtentTest test1, String selected_dataset,Map<String, String> Excel_data,String url) throws Exception {

		String testCaseName="TC_CRO_SC031";
		...
		String Band_Name2 = Excel_data.get("Band_Name2");
		...
		String Band_Name_2_Select = Excel_data.get("Band_Name_2_Select");
		String Vessel_Module_Name = Excel_data.get("Vessel_Module_Name");
		...
		Step_Start(4, "Click VslOprBand check box.", test, test1);
		...
		Step_End(4, "Click VslOprBand check box.", test, test1);
		Step_Start(5, "Click ContOprBand check box", test, test1);

		String selectBand2=String.format(band_Select, Band_Name2);
		
		scrollElementToView(driver, selectBand2);
		waitForElement(driver, selectBand2);
		if(Band_Name_2_Select.equalsIgnoreCase("Yes")){
		click(driver, selectBand2);
		}
		Step_End(5, "Click ContOprBand check box", test, test1);
		Step_Start(6, "Click save btton.", test, test1);

		waitForElement(driver, SaveButton_ToolBar);
		click(driver, SaveButton_ToolBar);
		
		Step_End(6, "Click save btton.", test, test1);
		Step_Start(7, "Printout Configuration updated message shown.", test, test1);
		...
		Step_End(7, "Printout Configuration updated message shown.", test, test1);

		Step_Start(8, "Click ok button.", test, test1);

		click(driver, popup_Message_Ok_Button);

		Step_End(8, "Click ok button.", test, test1);
		Step_Start(9, "Open vessel module.", test, test1);
		
		moduleNavigate(driver, Vessel_Module_Name);
		
		Step_End(9, "Open vessel module.", test, test1);
		Step_Start(10, "Search booking vessel in vessel code column", test, test1);

		waitForElement(driver, Vessel_Code_Filter_Input_VSL);
		sendKeys(driver, Vessel_Code_Filter_Input_VSL, vessel_Code);
	
		Step_End(10, "Search booking vessel in vessel code column", test, test1);
		Step_Start(11, "Find vessel operator.", test, test1);

		waitForElement(driver, Vessel_Operator_Gridcell);
		String actualVessel_Operator=getText(driver, Vessel_Operator_Gridcell);
		System.out.println("actualVessel_Operator : "+actualVessel_Operator);
		
		Step_End(11, "Find vessel operator.", test, test1);
...
//  CRO 
		Step_Start(12, "Go to Container release order module.", test, test1);

		moduleNavigate(driver, CRO_Module_Name);
		

		Step_End(12, "Go to Container release order module.", test, test1);
		...
		Step_Start(14, "Check the Container operator emprty and Vessel Operator from vessel module.", test, test1);

		
		Step_End(14, "Check the Container operator emprty and Vessel Operator from vessel module.", test, test1);
