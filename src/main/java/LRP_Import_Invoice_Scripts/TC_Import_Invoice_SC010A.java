...
public class TC_Import_Invoice_SC010A extends Keywords {

	public void Import_Invoice_SC010A(WebDriver driver, ExtentTest test, ExtentTest test1, String selected_dataset,Map<String, String> Excel_data,String url)
			throws EncryptedDocumentException, InvalidFormatException, IOException {

		String testcase_Name = "TC_Import_Invoice_SC010A";
		...
		String Table_Header_print_Application = Excel_data.get("Application_Header");
		...
		if (BL_Retrive_from_Import_invoice.equals("By_bl_Number")) {
			...
			if(isdisplayed(driver, popup_Message)) {
				...
				if(Part_Lot_BL_Popup.equals(act_Popup)) {
					...
					Extent_pass(driver, "Matched ||   Expected Popup " + Part_Lot_BL_Popup + "||  Actual Popup :" + act_Popup, test, test1);
					...
					if(Charges_Popup.equals(act_Popup)) {
						...
						Extent_pass(driver, "Matched ||   Expected Popup " + Charges_Popup + "||  Actual Popup :" + act_Popup, test, test1);
						waitForElement(driver, popup_Message_Ok_Button);
						jsClick(driver, popup_Message_Ok_Button);
		...
		Extent_call(test, test1, "In Excel Define as new Mode Follow this Steps");
		Step_Start(6, "Go to additional charges tab", test, test1);
		scrollBottom(driver);
		waitForElement(driver, Additional_charges_tab);
		click(driver, Additional_charges_tab);
		Step_End(6, "Go to additional charges tab", test, test1);
		...
		Step_Start(7, "Click on upload Button and upload it, save the IMformation", test, test1);

		waitForDisplay(driver, upload_addtional_select);
		...
		Extent_pass(driver, report, test, test1);
		...
		Step_End(7, "Click on upload Button and upload it, save the IMformation", test, test1);
		...
		Step_Start(9, "system has validate import invoice Saved.click ok", test, test1);
		...
		waitForElement(driver, popup_Message_Ok_Button);
		click(driver, popup_Message_Ok_Button);
		Step_End(9, "system has validate import invoice Saved.click ok", test, test1);
		Step_Start(10, "Verify the Charges still available in Additional charges tab", test, test1);
		waitForElement(driver, addtional_tab_header);
		List<Map<String, String>> print_table = getTableData(driver, addtional_tab_header,
				addtional_tab_header_row);
		List<String> header1 = splitAndExpand(Table_Header_print_Application);
		...
		Step_End(10, "Verify the Charges still available in Additional charges tab", test, test1);
