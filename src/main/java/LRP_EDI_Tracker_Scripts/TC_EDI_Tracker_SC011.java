...
public class TC_EDI_Tracker_SC011 extends Keywords {
	public void EDI_Tracker_SC011(WebDriver driver, ExtentTest test, ExtentTest test1, String selected_Excel_dataset,
			Map<String, String> Excel_data, String url) {

		String testCaseName = "TC_EDI_Tracker_SC011";
		...
		String EDI_Tracker_ReInitate_Popup = Excel_data.get("EDI_Tracker_ReInitate_Popup");
		...
		Step_Start(3, "select the status (Requested)", test, test1);
		...
		Step_End(3, "select the status (Requested)", test, test1);
		...
		Step_Start(5, "click the re-initiate button one by one for all the records", test, test1);
		Step_Start(6, "print the request id and EDI name and partner id and agency for all the re-initiate data", test,
				test1);
		...
		for (int i = 0; i < totalSize; i++) {
			...
			waitForElement(driver, ReInitate_Btn_ET);
			click(driver, ReInitate_Btn_ET);

			waitForPopup(driver, Popup_Message, EDI_Tracker_ReInitate_Popup);
			String Act_Popup = getText(driver, Popup_Message);
			System.out.println(Act_Popup);

			if (EDI_Tracker_ReInitate_Popup.equals(Act_Popup)) {

				System.out.println("The row which has Reference Id : " + refIDText + ", EDI Id : " + ediIDText
						+ ", Partner ID : " + partnerIDText + " and Agenct : " + agencyText + " has been Re-initiated");
				Extent_cal(test, test1, "The row which has Reference Id : " + refIDText + ", EDI Id : " + ediIDText
						+ ", Partner ID : " + partnerIDText + " and Agenct : " + agencyText + " has been Re-initiated");

				System.out.println("Matched || Expected EDI details updated popup : " + EDI_Tracker_ReInitate_Popup
						+ " || Actual : " + Act_Popup);
				Extent_pass_New(driver, "Matched || Expected EDI details updated popup: " + EDI_Tracker_ReInitate_Popup
						+ " || Actual: " + Act_Popup, test, test1);
			} else {
				System.out.println("Not Matched || Expected EDI details updated popup : " + EDI_Tracker_ReInitate_Popup
						+ " || Actual : " + Act_Popup);
				Extent_fail(driver, "The row which has Reference Id : " + refIDText + ", EDI Id : " + ediIDText
						+ ", Partner ID : " + partnerIDText + " and Agenct : " + agencyText + " has not Re-initiated",
						test, test1);

				Extent_fail(driver, "Not Matched || Expected EDI details updated popup : " + EDI_Tracker_ReInitate_Popup
						+ " || Actual: " + Act_Popup, test, test1);
			}
			...
			if (!isdisplayed(driver, Verify_Reference_ID_Value_ET)) {
				System.out.println(
						"Expected result : After Re-initiate, reference ID should not present in the Data grid || Actual result : After Re-initiate, reference ID is not present in the Data grid");
				Extent_pass_New(driver,
						"Expected result : After Re-initiate, reference ID should not present in the Data grid || Actual result : After Re-initiate, reference ID is not present in the Data grid",
						test, test1);
			} else {
				System.out.println(
						"Expected result : After Re-initiate, reference ID should not present in the Data grid || Actual result : After Re-initiate, reference ID is still present in the Data grid");
				Extent_fail(driver,
						"Expected result : After Re-initiate, reference ID should not present in the Data grid || Actual result : After Re-initiate, reference ID is still present in the Data grid",
						test, test1);
			}
		...
		Step_End(6, "print the request id and EDI name and partner id and agency for all the re-initiate data", test,
				test1);
		Step_End(5, "click the re-initiate button one by one for all the records", test, test1);
