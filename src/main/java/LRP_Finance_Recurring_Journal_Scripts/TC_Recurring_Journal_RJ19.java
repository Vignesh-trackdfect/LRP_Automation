...
public class TC_Recurring_Journal_RJ19 extends Keywords{
	public void Recurring_Journal_RJ19(WebDriver driver, ExtentTest test, ExtentTest test1,String selected_dataset,Map<String, String> Excel_data,String url) throws EncryptedDocumentException, InvalidFormatException, IOException{
		String testcase_Name="TC_Recurring_Journal_RJ19";
		...
		String Open_Status = Excel_data.get("Open_Status");
		...
		Step_Start(7, "Choose the Post type as ALL", test, test1);	
		...
		Step_End(7, "Choose the Post type as ALL", test, test1);
		...
		Step_Start(9, "Recurring JV should be showed which is in ALL status like Open/Closed", test, test1);
		...
		for(int i=0;i<Batch_Status.size();i++) {
			if(Open_Status.equalsIgnoreCase(Batch_Status.get(i))) {
				System.out.println("Matched || Batch status Taken From Grid : " + Batch_Status.get(i) + " || Batch Status Selected : " + Open_Status);
				Extent_pass_New(driver,"Matched || Batch status Taken From Grid : " + Batch_Status.get(i) + " || Batch Status Selected : " + Open_Status, test,test1);
			} else if(Closed_status.equalsIgnoreCase(Batch_Status.get(i))) {
				...
				System.out.println("Not Matched ||Batch status Taken From Grid : " + Batch_Status.get(i) + " || Batch Status Selected : " + Closed_status);
		...
		Step_End(9, "Recurring JV should be showed which is in ALL status like Open/Closed", test, test1);
