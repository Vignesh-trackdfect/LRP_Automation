...
public class TC_Recurring_Journal_RJ20 extends Keywords{
	public void Recurring_Journal_RJ20(WebDriver driver, ExtentTest test, ExtentTest test1,String selected_dataset,Map<String, String> Excel_data,String url) throws EncryptedDocumentException, InvalidFormatException, IOException{
		String testcase_Name="TC_Recurring_Journal_RJ20";
		...
		String Row_Message = Excel_data.get("Row_Message");
		...
		Step_Start(10, "Click CLEAR Button", test, test1);
		waitForElement(driver, Clear_button_RJ);
		click(driver, Clear_button_RJ);
		Step_End(10, "Click CLEAR Button", test, test1);
		Step_Start(11, " All records should be cleared", test, test1);
		waitForElement(driver, posting_Details_Grid_Rows);
		String row_ValuesString=getAttribute(driver, posting_Details_Grid_Rows, "class");
		if(row_ValuesString.contains(Row_Message)) {
			System.out.println("Matched || Batch status Taken From Grid : " + Row_Message + " || Batch Status Selected : " + row_ValuesString);
			System.out.println("After Clicking Records should be cleared");
			Extent_pass_New(driver,"Matched || Batch status Taken From Grid : " + Row_Message + " || Batch Status Selected : " + row_ValuesString, test,test1);
		} else {
			System.out.println("Not Matched || Batch status Taken From Grid : " + Row_Message + " || Batch Status Selected : " + row_ValuesString);
			Extent_fail(driver,"Not Matched || Batch status Taken From Grid  : " + Row_Message + " || Batch Status Selected  : " + row_ValuesString,test, test1);
		}
		Step_End(11, " All records should be cleared", test, test1);
