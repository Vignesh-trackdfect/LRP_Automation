...
public class TC_Vendor_Invoice_Registration_SC32 extends Keywords {

	public void Vendor_Invoice_Registration_SC32(WebDriver driver, ExtentTest test, ExtentTest test1,
			String selected_Excel_dataset,Map<String, String> Excel_data,String url) {

		String testCaseName = "TC_Vendor_Invoice_Registration_SC32";
		...
		String Voyage_Name = Excel_data.get("Voyage_Name");
		...
		String DischargePort_Header = Excel_data.get("DischargePort_Header");
		String Discharge_Port = Excel_data.get("Discharge_Port");
		String DischargeTerminal_Header = Excel_data.get("DischargeTerminal_Header");
		...
		if (!Payment_method.trim().isEmpty()) {
		...
		Step_Start(14, "Select the Route", test, test1);

		waitForElement(driver, FDR_Route);
		click(driver, FDR_Route);

		Step_End(14, "Select the Route", test, test1);

		Step_Start(15, "Click the Service search  and select the Required Service", test, test1);

		waitForElement(driver, VIR_ServiceSearch);
		click(driver, VIR_ServiceSearch);

		twoColumnSearchWindow(driver, Servicecode_Header, Search_condition, Service_Code);

		Step_End(15, "Click the Service search  and select the Required Service", test, test1);

		Step_Start(16, "Click the Vessel search  and select the Required Vessel", test, test1);

		waitForElement(driver, VIR_VesselSearch);
		click(driver, VIR_VesselSearch);

		twoColumnSearchWindow(driver, Vesselcode_Header, Search_condition, Vessel_Code);

		Step_End(16, "Click the Vessel search  and select the Required Vessel", test, test1);

		Step_Start(17, "Click the Voyage search  and select the Required Voyage", test, test1);

		waitForElement(driver, VIR_VoyageSearch);
		click(driver, VIR_VoyageSearch);

		twoColumnSearchWindow(driver, Voyage_Header, Search_condition, Voyage_Name);

		Step_End(17, "Click the Voyage search  and select the Required Voyage", test, test1);

		Step_Start(18, "Click the Load Port search  and select the Required Load Port", test, test1);

		waitForElement(driver, VIR_LoadPort_Search);
		click(driver, VIR_LoadPort_Search);

		twoColumnSearchWindow(driver, PortCode_Header, Search_condition, Port_Code);

		Step_End(18, "Click the Load Port search  and select the Required Load Port", test, test1);

		Step_Start(19, "Click the Load Terminal search  and select the Required Load Terminal", test, test1);

		if (!Terminal_Code.trim().isEmpty()) {

			waitForElement(driver, VIR_LoadTerminal_Search);
			click(driver, VIR_LoadTerminal_Search);

			twoColumnSearchWindow(driver, Terminal_Header, Search_condition, Terminal_Code);

		}

		Step_End(19, "Click the Load Terminal search  and select the Required Load Terminal", test, test1);

		Step_Start(20, "Click the Discharge Port search  and select the Required Discharge Port", test, test1);

		waitForElement(driver, VIR_DischargePort_Search);
		click(driver, VIR_DischargePort_Search);

		twoColumnSearchWindow(driver, DischargePort_Header, Search_condition, Discharge_Port);

		Step_End(20, "Click the Discharge Port search  and select the Required Discharge Port", test, test1);

		Step_Start(21, "Click the Discharge Terminal search  and select the Required Discharge Terminal", test, test1);

		if (!Discharge_Terminal.trim().isEmpty()) {

			waitForElement(driver, VIR_DischargeTerminalSearch);
			click(driver, VIR_DischargeTerminalSearch);

			twoColumnSearchWindow(driver, DischargeTerminal_Header, Search_condition, Discharge_Terminal);

		}

		Step_End(21, "Click the Discharge Terminal search  and select the Required Discharge Terminal", test, test1);

		Step_Start(23, "Click the 'ADD' button, Details added in the Grid", test, test1);

		waitForElement(driver, Add_Button_All_VIR);
		click(driver, Add_Button_All_VIR);

		Step_End(23, "Click the 'ADD' button, Details added in the Grid", test, test1);

		Step_Start(24, "Click the save", test, test1);
		...
		waitForDisplay(driver, VIR_Grid_Table_row1);
		if (isdisplayed(driver, VIR_Grid_Table_row1)) {
		...
		Step_End(24, "Click the save", test, test1);

		Step_Start(25, "It auto generate the Invoice Reg No", test, test1);
		...
		Step_End(25, "It auto generate the Invoice Reg No", test, test1);

		moduleNavigate(driver, Haulage_Expense_Module);

		waitForElement(driver, new_Button_VM);
		jsClick(driver, new_Button_VM);
		...
		String Reference_number_vendor = parts[0];
		System.out.println(Reference_number_vendor);

		globalValueSearchWindow(driver, Feeder_search_Conditon, Feeder_search_Header, Reference_number_vendor, "", "",
				"", "");
		...
		if (isdisplayed(driver, FER_firstrow)) {
			...
			System.out.println(
					"Expected result is : Saved ref no details should show in the Haulage Expense report || Actual result is : Saved ref no details are showing in the Haulage Expense report");
			Extent_pass(driver,
					"Expected result is : Saved ref no details should show in the Haulage Expense report || Actual result is : Saved ref no details are showing in the Haulage Expense report",
					test, test1);
		} else {
			...
			System.out.println(
					"Expected result is : Saved ref no details should show in the Haulage Expense report || Actual result is : Saved ref no details are not showing in the Haulage Expense report");
			Extent_fail(driver,
					"Expected result is : Saved ref no details should show in the Haulage Expense report || Actual result is : Saved ref no details are not showing in the Haulage Expense report",
					test, test1);
