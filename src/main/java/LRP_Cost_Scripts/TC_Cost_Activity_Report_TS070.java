...
public void  Cost_Activity_Report_TS070(WebDriver driver, ExtentTest test, ExtentTest test1,String selected_dataset,Map<String, String> Excel_data,String url) throws Exception{
		...
		String columns_to_filter1 = Excel_data.get("columns_to_filter1");
		String CAR_Retrieve_Type2 = Excel_data.get("CAR_Retrieve_Type2");
		String CAR_Number_Retrieve2 = Excel_data.get("CAR_Number_Retrieve2");
		String CAR_Retrieve_Type3 = Excel_data.get("CAR_Retrieve_Type3");
		String CAR_Number_Retrieve3 = Excel_data.get("CAR_Number_Retrieve3");
		...
		globalValueSearchWindow(driver, CAR_Retrieve_Condition, CAR_Retrieve_Type, CAR_Number_Retrieve, CAR_Retrieve_Type2, CAR_Number_Retrieve2, CAR_Retrieve_Type3, CAR_Number_Retrieve3);
		...
		if(isdisplayed(driver, load_discharge_window)) {
			System.out.println("Expected : Load discharge window Should opened || Actaul : load discharge window is opened");
			Extent_pass_New(driver, "Expected : Load discharge window Should opened || Actaul : load discharge window is opened", test, test1);
		}else {
			System.out.println("Expected : Load discharge window Should opened || Actaul : load discharge window is not opened");
			Extent_fail(driver, "Expected : Load discharge window Should opened || Actaul : load discharge window is not opened", test, test1);
		}
		...
		Map<String,Double> Gross_Weight_Values_Load_CAR=new HashMap<String,Double>(); 
		Map<String,Double> Gross_Weight_Values_Discharge_CAR=new HashMap<String,Double>(); 
		...
		if(LoadVal) {
			...
			while(isdisplayed(driver, container)) {
				...
				String weight=String.format(Gross_Weight_Load_CAR, k);
				String containerNum=getText(driver, container);
				String Gross_Weight=getText(driver, weight);
				double grossWeightNum=getNumberValue(Gross_Weight);
				Gross_Weight_Values_Load_CAR.put(containerNum, grossWeightNum);
		...
		if(DischargeVal) {
			...
			while(isdisplayed(driver, container)) {
				...
				String Gross_Weight=getText(driver, weight);
				double grossWeightNum=getNumberValue(Gross_Weight);
				Gross_Weight_Values_Discharge_CAR.put(containerNum, grossWeightNum);
		...
		if(LoadVal) {
			...
			Map<String,Double> Gross_Weight_Values_Load_OL=new HashMap<String,Double>(); 
			...
			while(isdisplayed(driver, container)) {
				mouseOverToElement(driver, container);
				String weight=String.format(Gross_Weight_OperationList, k);
				String containerNum=getText(driver, container);
				String Gross_Weight=getText(driver, weight);
				double grossWeightNum=getNumberValue(Gross_Weight);
				Gross_Weight_Values_Load_OL.put(containerNum, grossWeightNum);
				k++;
				container=String.format(Container_Number_OperationList, k);
			}
			
			if(compareMapDoubleValues(Gross_Weight_Values_Load_OL,Gross_Weight_Values_Load_CAR)) {
				System.out.println("Load Gross Weight Matched || Expected in Cost Activity "+Gross_Weight_Values_Load_CAR+"      ||       Actual in Operation List View  : "+Gross_Weight_Values_Load_OL);
				Extent_pass_New(driver, "Load Container Number Matched || Expected in Cost Activity "+Gross_Weight_Values_Load_CAR+"      ||       Actual in Operation List View  : "+Gross_Weight_Values_Load_OL, test, test1);
			}else {
				System.out.println("Load Gross Weight Not Matched || Expected in Cost Activity "+Gross_Weight_Values_Load_CAR+"      ||       Actual in Operation List View  : "+Gross_Weight_Values_Load_OL);
				Extent_fail(driver, "Load Gross Weight Not Matched || Expected in Cost Activity "+Gross_Weight_Values_Load_CAR+"      ||       Actual in Operation List View  : "+Gross_Weight_Values_Load_OL, test, test1);
			}
		...
		if(DischargeVal) {
			...
			Map<String,Double> Gross_Weight_Values_Discharge_OL=new HashMap<String,Double>(); 
			...
			while(isdisplayed(driver, container)) {
				...
				String Gross_Weight=getText(driver, weight);
				double grossWeightNum=getNumberValue(Gross_Weight);
				Gross_Weight_Values_Discharge_OL.put(containerNum, grossWeightNum);
				k++;
				container=String.format(Container_Number_OperationList, k);
			}
			
			if(compareMapDoubleValues(Gross_Weight_Values_Discharge_OL,Gross_Weight_Values_Discharge_CAR)) {
				System.out.println("Discharge Gross Weight Matched || Expected in Cost Activity "+Gross_Weight_Values_Discharge_CAR+"      ||       Actual in Operation List View  : "+Gross_Weight_Values_Discharge_OL);
				Extent_pass_New(driver, "Discharge Container Number Matched || Expected in Cost Activity "+Gross_Weight_Values_Discharge_CAR+"      ||       Actual in Operation List View  : "+Gross_Weight_Values_Discharge_OL, test, test1);
			}else {
				System.out.println("Discharge Gross Weight Not Matched || Expected in Cost Activity "+Gross_Weight_Values_Discharge_CAR+"      ||       Actual in Operation List View  : "+Gross_Weight_Values_Discharge_OL);
				Extent_fail(driver, "Discharge Gross Weight Not Matched || Expected in Cost Activity "+Gross_Weight_Values_Discharge_CAR+"      ||       Actual in Operation List View  : "+Gross_Weight_Values_Discharge_OL, test, test1);
			}
		}
