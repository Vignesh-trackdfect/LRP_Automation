...
public class TC_Import_Documentation_SC036 extends Keywords {

	public void Import_Documentation_SC036(WebDriver driver, ExtentTest test, ExtentTest test1, String selected_dataset,Map<String, String> Excel_data,String url) {
	
		String testcase_Name="TC_Import_Documentation_SC036";
		...
		String Discharge_Status_Header=Excel_data.get("Discharge_Status_Header");
		...
		if(!Discharge_status_Value.trim().equals("")) {
			Step_Start(3, "click the Discharge  status search field in import documentation", test, test1);
			waitForElement(driver, Discharge_status_search_button);
			click(driver, Discharge_status_search_button);
			Step_End(3, "click the Discharge  status search field in import documentation", test, test1);
			Step_Start(4, "list of the Discharge status", test, test1);
			//sendKeys(driver, Discharge_status_Input_TF, Discharge_status_Value);
			Step_Start(5, "select one the Discharge Status", test, test1);

			twoColumnMultipleSearchWindow(driver, Discharge_Status_Header, Condition, Discharge_status_Value);

			
			waitForElement(driver, Discharge_status_TF);
			String Discharge_status = getAttribute(driver, Discharge_status_TF, "value");
			System.out.println("Discharge status:" + Discharge_status);
			if (Discharge_status_Value.equals(Discharge_status)) {
				System.out.println("Matched || Expected status was : " + Discharge_status_Value + " || Actual status was : "	+ Discharge_status);
				Extent_pass_New(driver, "Matched || Expected status was : " + Discharge_status_Value + " || Actual status was : "	+ Discharge_status, test, test1);
			} else {
				System.out.println("NotMatched || Expected status was : " + Discharge_status_Value	+ " || Actual status was : " + Discharge_status);
				Extent_fail(driver, "NotMatched || Expected status was : " + Discharge_status_Value+ " || Actual status was : " + Discharge_status, test, test1);
			}
			Step_End(4, "list of the Discharge status", test, test1);
			Step_End(5, "select one the Discharge Status", test, test1);
		}
