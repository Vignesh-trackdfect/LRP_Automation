...
//Check whether system allow to Add multiple customers in Customer tab
public class TC_Booking_SC038 extends Keywords {

	public void Booking_SC038(WebDriver driver, ExtentTest test, ExtentTest test1, String selected_dataset,Map<String, String> Excel_data,String url) {

		String test_Case_Name = "TC_Booking_SC038";
		...
		String Customer_Type_Value = Excel_data.get("Customer_Type_Value");
		String Customer_Type_Verification = Excel_data.get("Customer_Type_Verification");
		String Customer = Excel_data.get("Customer");
		String CustomerName = Excel_data.get("CustomerName");
		...
		Step_Start(7, "Right safeclick copy option system will add the selected customer details Start", test, test1);

		waitForElement(driver, Customer_Tab);
		safeclick(driver, Customer_Tab);

		waitForElement(driver, Customer_Grid_EditBtn);
		RightClick(driver, Customer_Grid_EditBtn);

		waitForElement(driver, Customer_Copy1);
		safeclick(driver, Customer_Copy1);
		waitForDisplay(driver, Paste_PopUp);
		if (isDisplayed(driver, Paste_PopUp)) {
			System.out.println("Matched || Expected Result is : ' Customer type Popup Should be displayed ' || The Actual Result is : ' Customer type Popup is displayed '");
			Extent_pass_New(driver,"Matched || Expected Result is : ' Customer type Popup Should be displayed ' || The Actual Result is : ' Customer type Popup is displayed '", test, test1);
		} else {
			System.out.println("Not Matched || Expected Result is : ' Customer type Popup Should be displayed ' || The Actual Result is : ' Customer type Popup is Not displayed '");
			Extent_fail(driver,"Not Matched || Expected Result is : ' Customer type Popup Should be displayed ' || The Actual Result is : ' Customer type Popup is Not displayed '", test, test1);
		}
		Actions action = new Actions(driver);
		action.keyDown(Keys.CONTROL).build().perform();
		List<String> options = splitAndExpand(Customer_Type_Value);
		for(String option : options) {
			String select_Paste=String.format(Booking_Locators.Customer_Typ,option);
			waitForElement(driver, select_Paste);
			click(driver, select_Paste);
		}
		action.keyUp(Keys.CONTROL).build().perform();
		waitForElement(driver, Paste_button);
		safeclick(driver, Paste_button);
		String Customer_Name = null;
		List<String> Customer_Names = new ArrayList<String>();
		waitForElement(driver, Customer_types);
		List<WebElement> Customer_Types = listOfElements(driver, Customer_types);

		for (WebElement Customer_Type : Customer_Types) {
			Customer_Name = getText1(driver, Customer_Type);
			System.out.println("Cutomer Names:" + Customer_Name);
			Customer_Names.add(Customer_Name);
		}
		List<String> Customer_Type_Verifications = splitAndExpand(Customer_Type_Verification);
		for(int i = 0; i < Customer_Type_Verifications.size();i++) {
			boolean found = true;
			for(int j = 0; j < Customer_Names.size();j++) {
				if (Customer_Names.get(j).equals(Customer_Type_Verifications.get(i))) {
					System.out.println("Matched || Expected value was : " + Customer_Type_Verifications.get(i) + " || Actual value was : "
							+ Customer_Names.get(j));
					System.out.println("Able to add new customer");
					Extent_pass_New(driver, "Matched || Expected value was : " + Customer_Type_Verifications.get(i) + " || Actual value was : "
							+ Customer_Names.get(j), test, test1);
					found=false;
				}
			}
			if(found) {
				System.out.println("NotMatched || Expected value was : " + Customer_Type_Verifications + " || Actual value was : "
						+ Customer_Names);
				System.out.println("Unable to add new customer");
				Extent_fail(driver, "NotMatched || Expected value was : " + Customer_Type_Verifications + " || Actual value was : "
						+ Customer_Names, test, test1);
			}
		}
		...
		if (!Customer.equals("")) {
			...
			safeclick(driver, Customer_type_dropdown);
			safeclick(driver, BKG_Customer_Select);
		}
		if (!CustomerCode.equals("")) {
			waitForElement(driver, Customer_Code_SearchBtn);
			safeclick(driver, Customer_Code_SearchBtn);

			waitForElement(driver, Customer_Global_Search_TF);
			sendKeys(driver, Customer_Global_Search_TF, CustomerName);
			waitForElement(driver, Customer_code_TF);
			sendKeys(driver, Customer_code_TF, CustomerCode);
			waitForElement(driver, Customer_Global_SytemCode);
			sendKeys(driver, Customer_Global_SytemCode, SystemCode);
			waitForElement(driver, Customer_Detail_SearchBtn);
			safeclick(driver, Customer_Detail_SearchBtn);
			safeclick(driver, Customer_Details_Select);
			formatLocatorClick(driver, Customer_select, Customer);
			waitForElement(driver, Customer_addBtn);
			safeclick(driver, Customer_addBtn);
