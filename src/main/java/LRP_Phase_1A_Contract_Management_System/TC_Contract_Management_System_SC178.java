...
public class TC_Contract_Management_System_SC178 extends Keywords {

	public void Contract_Management_System_SC178(WebDriver driver, ExtentTest test, ExtentTest test1,
			String selected_dataset, Map<String, String> Excel_data, String url) throws InterruptedException {

		String testCaseName = "TC_Contract_Management_System_SC178";
		...
		String CMS_Email_Field = Excel_data.get("CMS_Email_Field");
		String CMS_Email_Sub_Field = Excel_data.get("CMS_Email_Sub_Field");
		...
		String CMS_Mail_Send_Popup = Excel_data.get("CMS_Mail_Send_Popup");
		...
		Step_Start(8, " click on PDF output button", test, test1);
		
		waitForElement(driver, print_Common_btn);
		click(driver, print_Common_btn);
		
		waitForElement(driver, CMS_Print_Mail_Btn);
		click(driver, CMS_Print_Mail_Btn);
		
		waitForElement(driver, CMS_Print_Mail_field);
		sendKeys(driver, CMS_Print_Mail_field, CMS_Email_Field);
		
	waitForElement(driver, CMS_mail_Subject);
		sendKeys(driver, CMS_mail_Subject, CMS_Email_Sub_Field);
		
		waitForElement(driver, CMS_Print_Mail_Send_Btn);
		click(driver, CMS_Print_Mail_Send_Btn);
		
		waitForPopup(driver, popup_Message, CMS_Mail_Send_Popup);
		String mail_popup =getText(driver, popup_Message);
		System.out.println("mail_popup :"+mail_popup);
		
		if(mail_popup.equals(CMS_Mail_Send_Popup)) {
			Extent_pass(driver,"Matched || Expected popup value was : "+CMS_Mail_Send_Popup+" || Actual popup value was is display : "+ mail_popup, test,test1);
			System.out.println("Matched || Expected popup value was : "+CMS_Mail_Send_Popup+" || Actual popup value was is display: "+ mail_popup);
		}else {
			System.out.println("Not Matched || Expected popup value was : "+CMS_Mail_Send_Popup+" || Actual popup value was not display: "+ mail_popup);
			Extent_fail(driver,"Not Matched || Expected popup value was : "+CMS_Mail_Send_Popup+" || Actual popup value was not display: "+ mail_popup, test,test1);
		...
		mailinator_Verification(driver, mail_SignIn_Page_Url, mail_Id, CMS_Email_Sub_Field);
		
		
		Step_End(8, " click on PDF output button", test, test1);
