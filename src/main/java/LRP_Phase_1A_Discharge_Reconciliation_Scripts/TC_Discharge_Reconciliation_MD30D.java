...
public class TC_Discharge_Reconciliation_MD30D  extends Keywords {
	public void Discharge_Reconciliation_MD30D(WebDriver driver, ExtentTest test, ExtentTest test1, String selected_dataset,Map<String, String> Excel_data,String url) {

		String testcase_Name = "TC_Discharge_Reconciliation_MD30D";
		...
		//	String select_Option_3 = String.format(DropDown_Select, Plan_Input);
			...
			Step_Start(10, "Capture the BLs List and store it  Quantity", test, test1);
		...
		//	reArrangeAG_GridColumnsNew(driver, Cargo_Detials_Column_btn, Column_Header);
			...
			waitForElement(driver, Cargo_QTY_Detials);
			...
			Step_End(10, "Capture the BLs List and store it  Quantity", test, test1);
			...
			Step_Start(16, "Verify that Make sure BLs matched these fields- Quantity", test, test1);
			

			List<WebElement> tableCells = driver.findElements(By.xpath("//div[@id='OPS-OPS_OpsListGrid_tbl']//div[@col-id='qty' and @role='gridcell']"));
	        ...
	        for (WebElement cell : tableCells) {
	            ...
	            try {
	                double cellValue =Double.parseDouble(cellText); // Convert to integer
	                totalSum += cellValue;
	                System.out.println(totalSum);  // Add to the sum
	            } catch (NumberFormatException e) {
	                // Handle non-numeric cell values (optional)
			...
			if (Dis_QT_Code==totalSum) {
				System.out.println("Pass : The expected Quantity Detials from Discharge Reconciliation is : "+ Dis_QT_Code +" || The actual Quantity Detials from Operetion List View is : "+ totalSum );
				Extent_pass(driver, "Pass : The expected Quantity Detials from Discharge Reconciliation is : "+ Dis_QT_Code +" || The actual Quantity Detials from Operetion List View is : "+ totalSum , test, test1);
			} else {
				System.out.println(" Fail : The expected Quantity Detials from Discharge Reconciliation is : "+ Dis_QT_Code +" || The actual Quantity Detials from Operetion List View is : "+ totalSum );
				Extent_fail(driver, "Fail : The expected Quantity Detials from Discharge Reconciliation is : "+ Dis_QT_Code +" || The actual Quantity Detials from Operetion List View is : "+ totalSum , test, test1);
			
			}
			Step_End(16, "Verify that Make sure BLs matched these fields- Quantity", test, test1);
