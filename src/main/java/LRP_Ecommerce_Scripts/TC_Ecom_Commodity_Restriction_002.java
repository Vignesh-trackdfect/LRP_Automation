...
public class TC_Ecom_Commodity_Restriction_002 extends Keywords {
	public void Ecom_Commodity_Restriction_002(WebDriver driver, ExtentTest test, ExtentTest test1,
			String selected_dataset,Map<String, String> Excel_data,String url) {

		String testcase_Name = "TC_Ecom_Commodity_Restriction_002";
		...
		Step_Start(1, "Click on the Commodity Code Search option and then select Add Commodity", test, test1);
		...
		Step_End(1, "Click on the Commodity Code Search option and then select Add Commodity", test, test1);

		Step_Start(2, "Enter valid commodity details, including: *Commodity Name,*Commodity Code", test, test1);
		Step_Start(3, "Select the appropriate options (if required)", test, test1);

		twoColumnMultipleSearchWindow(driver, Commudity_Header, Filter_Condition, Commudity_Header_value);

		Step_End(3, "Select the appropriate options (if required)", test, test1);
		Step_End(2, "Enter valid commodity details, including: *Commodity Name,*Commodity Code", test, test1);
		...
		Step_Start(5, " The system should successfully save the valid commodity", test, test1);
		...
		Step_End(5, " The system should successfully save the valid commodity", test, test1);

		Step_Start(6, "The saved commodity should be displayed in the commodity lis", test, test1);
		...
		if (Act_Commodity_code.equals(Commodity_Code)) {
			...
			Extent_pass_New(driver, "Matched || Expected commodity code was: " + Commodity_Code
		...
		if (Act_Commodity_Name.equals(Commodity_Name)) {
			...
			Extent_pass_New(driver, "Matched || Expected commodity Name was: " + Commodity_Name + " || Actual Value was : "
					+ Act_Commodity_Name, test, test1);
		} else {
		...
		Step_End(6, "The saved commodity should be displayed in the commodity lis", test, test1);
