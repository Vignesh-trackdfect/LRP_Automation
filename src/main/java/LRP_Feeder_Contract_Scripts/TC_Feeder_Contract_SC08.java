...
public class TC_Feeder_Contract_SC08 extends Keywords {
	public void Feeder_Contract_SC08(WebDriver driver, ExtentTest test, ExtentTest test1, String selected_dataset,
			Map<String, String> Excel_data, String url) {
		String testcase_Name = "TC_Feeder_Contract_SC08";
		...
		if (POD_Input_Value.equals(Pickup_Location_Value_Act)) {
			System.out.println("Expected Result Taken from test data :" + POD_Input_Value
					+ " Actual Result taken from application after two coloum search : " + Pickup_Location_Value_Act);
			Extent_pass_New(driver, "Expected Result Taken from test data :" + POD_Input_Value
					+ " Actual Result taken from application after two coloum search : " + Pickup_Location_Value_Act,
					test, test1);
		} else {
			System.out.println("Expected Result Taken from test data :" + POD_Input_Value
					+ " Actual Result taken from application after two coloum search : " + Pickup_Location_Value_Act);
			Extent_fail(driver, "Expected Result Taken from test data :" + POD_Input_Value
					+ " Actual Result taken from application after two coloum search : " + Pickup_Location_Value_Act,
					test, test1);
		}
