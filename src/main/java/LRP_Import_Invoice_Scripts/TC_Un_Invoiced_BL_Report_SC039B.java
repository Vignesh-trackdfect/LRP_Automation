...
public class TC_Un_Invoiced_BL_Report_SC039B extends Keywords {

	public void Un_Invoiced_BL_Report_SC039B(WebDriver driver, ExtentTest test, ExtentTest test1, String selected_dataset,
			Map<String, String> Excel_data, String url) throws Exception {

		String testcase_Name = "TC_Un_Invoiced_BL_Report_SC039B";
		...
		Step_End(8,"Select(+)Button, keep multiple selection as well, (For ALL option No need to Pass any code//Region/Corporate Agency/Agency then pass the code that has to be refer)",
				test, test1);
		Step_Start(9,"Keep below is optional,(But need in Test Data)\r\n" + "Service,Vessel,Voyage,Bound,PLA,,POL,POD,PLD",
				test, test1);
		...
		waitForElement(driver, lineRevenue_Others_GridCell_UIB);
		String actual_LineRevenue_UIB = getText(driver, lineRevenue_Others_GridCell_UIB);
		System.out.println("actual_LineRevenue : " + actual_LineRevenue_UIB);
		String actual_Line_Revenue_STR = actual_LineRevenue_UIB.replace(",", "");
		double actual_Line_Revenue = Double.parseDouble(actual_Line_Revenue_STR);
		...
		if (!actual_BL_MCN_Number.contains("<-->")) { // AE00216200<-->AE00216200CV3
			...
			Extent_call(test, test1, "Compare Values For Line Revenue–Others");
			...
			Step_Start(12,"Shipment type has Export Invoice Find the Payment Mode Values as Prepaid and Charge code has OFR GO to Amount in USD.\r\n"
			...
			if (blType.equals("Export") || actualInvoiceType.equals("Export Invoice")) {
				...
				amount_BOL_LRO = getValuesByFirstTwoColumnAndHeader(billOfLading_Charges, "Charge Code", "OFR",
						"Payment Mode", "Prepaid", "Charge Type", "M", "Amount In USD");

				if (amount_BOL_LRO == 0) {
					amount_BOL_LRO = 0.00;
				}

				System.out.println("Line Revenue - Others Amount : " + amount_BOL_LRO);
			}
			...
			Step_Start(16,
					"Shipment type has Import Invoice Find the Payment Mode Values as Collect and and charge Mode as N  and Ignore OFR  AMT GO to Amount in USD .\r\n"
							+ "and compare both Uninvoiced  report and BL. If multiple charges is avaiable  system has to sum and compare Line Revenue–Others values in uninvoiced BL.",
					test, test1);
			...
			if (blType.equals("Import") || actualInvoiceType.equals("Import Invoice")) {
				...
				amount_BOL_LRO = getValuesByFirstTwoColumnAndHeader(billOfLading_Charges, "Charge Code", "OFR",
						"Payment Mode", "Collect", "Charge Type", "M", "Amount In USD");
			...
			Step_End(16,
					...
					test, test1);
			Step_Start(17,
					"Shipment type has Cross Invoice Find the Payment Mode Values as Payat and and charge Mode as N  and Ignore OFR  AMT GO to Amount in USD .\r\n"
							+ "and compare both Uninvoiced  report and BL. If multiple charges is avaiable  system has to sum and compare Line Revenue–Others values in uninvoiced BL.",
					test, test1);
			...
			if (actualInvoiceType.equals("Cross Invoice")) {
				...
				double amount_BOL_LRO1 = getValuesByFirstTwoColumnAndHeader(billOfLading_Charges, "Charge Code", "OFR",
						"Payment Mode", "PayAt-Prepaid", "Charge Type", "M", "Amount In USD");

				double amount_BOL_LRO2 = getValuesByFirstTwoColumnAndHeader(billOfLading_Charges, "Charge Code", "OFR",
						"Payment Mode", "PayAt-Collect", "Charge Type", "M", "Amount In USD");
				
				double amount_BOL_LRO3 = getValuesByFirstTwoColumnAndHeader(billOfLading_Charges, "Charge Code", "OFR",
						"Payment Mode", "PayAt", "Charge Type", "M", "Amount In USD");
				
				amount_BOL_LRO=amount_BOL_LRO1+amount_BOL_LRO2+amount_BOL_LRO3;
			...
			if (actual_Line_Revenue == amount_BOL_LRO) {
				System.out.println("Matched || Bill of lading Line Revevenue Others : " + amount_BOL_LRO
						+ " || Line Revevenue Others in Uninvoiced BL Report : " + actual_Line_Revenue);
				Extent_pass_New(driver, "Matched || Bill of lading Line Revevenue Others : " + amount_BOL_LRO
						+ " || Line Revevenue Others in Uninvoiced BL Report : " + actual_Line_Revenue, test, test1);
			} else {
				System.out.println("Not Matched || Bill of lading Line Revevenue Others : " + amount_BOL_LRO
						+ " || Line Revevenue Others in Uninvoiced BL Report : " + actual_Line_Revenue);
				Extent_fail(driver, "Not Matched || Bill of lading Line Revevenue Others : " + amount_BOL_LRO
						+ " || Line Revevenue Others in Uninvoiced BL Report : " + actual_Line_Revenue, test, test1);
			}

			Step_End(17,"Shipment type has Cross Invoice Find the Payment Mode Values as Payat and and charge Mode as N  and Ignore OFR  AMT GO to Amount in USD .\r\n"
							+ "and compare both Uninvoiced  report and BL. If multiple charges is avaiable  system has to sum and compare Line Revenue–Others values in uninvoiced BL.",
					test, test1);
...
//		Charges tab
			...
			List<Map<String, String>> MCN_Charges = getTableData(driver, MCN_Charges_Table_Headers,
			...
			Step_Start(13,"Shipment type has Export Invoice Find the Payment Mode Values as Prepaid and and charge Mode as N and Ignore OFR  AMT GO to Amount in USD .\r\n"
							+ "and compare both Uninvoiced  report and BL. If multiple charges is avaiable system has to sum and compare Line Revenue–Others values in uninvoiced BL.",
					test, test1);

			Extent_call(test, test1, "Compare Values For Line Revenue–Others.");
			double amount_MCN_LRO = 0;

			if (blType.equals("Export") || actualInvoiceType.equals("Export Invoice")) {

				Extent_pass_New(driver, "Shipment type is Export Invoice", test, test1);

				amount_MCN_LRO = getValuesByFirstTwoColumnAndHeader(MCN_Charges, "Charge Code", "OFR", "Payment Mode",
						"Prepaid", "Charge Type", "M", "Amount in USD");

				if (amount_MCN_LRO == 0) {
					amount_MCN_LRO = 0.00;
				}

				System.out.println("Line Revenue - Others Amount : " + amount_MCN_LRO);
			}

			Step_End(13,"Shipment type has Export Invoice Find the Payment Mode Values as Prepaid and and charge Mode as N and Ignore OFR  AMT GO to Amount in USD .\r\n"
							+ "and compare both Uninvoiced  report and BL. If multiple charges is avaiable system has to sum and compare Line Revenue–Others values in uninvoiced BL.",
					test, test1);

			Step_Start(14,"Shipment type has Import Invoice Find the Payment Mode Values as Collect and and charge Mode as N  and Ignore OFR  AMT GO to Amount in USD .\r\n"
							+ "and compare both Uninvoiced  report and BL. If multiple charges is avaiable  system has to sum and compare Line Revenue–Others values in uninvoiced BL.",
					test, test1);
			...
			if (blType.equals("Import") || actualInvoiceType.equals("Import Invoice")) {
				...
				amount_MCN_LRO = getValuesByFirstTwoColumnAndHeader(MCN_Charges, "Charge Code", "OFR", "Payment Mode",
						"Collect", "Charge Type", "M", "Amount in USD");
			...
			Step_End(14,"Shipment type has Import Invoice Find the Payment Mode Values as Collect and and charge Mode as N  and Ignore OFR  AMT GO to Amount in USD .\r\n"
							+ "and compare both Uninvoiced  report and BL. If multiple charges is avaiable  system has to sum and compare Line Revenue–Others values in uninvoiced BL.",
					test, test1);
			Step_Start(15,"Shipment type has Cross Invoice Find the Payment Mode Values as Payat and and charge Mode as N  and Ignore OFR  AMT GO to Amount in USD .\r\n"
							+ "and compare both Uninvoiced  report and BL. If multiple charges is avaiable  system has to sum and compare Line Revenue–Others values in uninvoiced BL.",
					test, test1);
			...
			if (actualInvoiceType.equals("Cross Invoice")) {
				...
				double amount_MCN_LRO1 = getValuesByFirstTwoColumnAndHeader(MCN_Charges, "Charge Code", "OFR", "Payment Mode",
						"PayAt-Prepaid", "Charge Type", "M", "Amount in USD");

				double amount_MCN_LRO2 = getValuesByFirstTwoColumnAndHeader(MCN_Charges, "Charge Code", "OFR", "Payment Mode",
						"PayAt-Collect", "Charge Type", "M", "Amount in USD");
				
				double amount_MCN_LRO3 = getValuesByFirstTwoColumnAndHeader(MCN_Charges, "Charge Code", "OFR", "Payment Mode",
						"PayAt", "Charge Type", "M", "Amount in USD");
				
				amount_MCN_LRO=amount_MCN_LRO1+amount_MCN_LRO2+amount_MCN_LRO3;
				if (amount_MCN_LRO == 0) {
					amount_MCN_LRO = 0.00;
				}
				System.out.println("Line Revenue - Others Amount : " + amount_MCN_LRO);
			...
			Extent_pass_New(driver,"Un Invoiced BL Report Table"+UnInvoiced_Table_Data,test,test1);
			if (actual_Line_Revenue == amount_MCN_LRO) {
				System.out.println("Matched || MCN Line Revevenue Others : " + amount_MCN_LRO+ " || Line Revevenue Others in Uninvoiced BL Report : " + actual_Line_Revenue);
				Extent_pass_New(driver, "Matched || MCN Line Revevenue Others : " + amount_MCN_LRO+ " || Line Revevenue Others in Uninvoiced BL Report : " + actual_Line_Revenue, test, test1);
			} else {
				System.out.println("Not Matched || MCN Line Revevenue Others : " + amount_MCN_LRO+ " || Line Revevenue Others in Uninvoiced BL Report : " + actual_Line_Revenue);
				Extent_fail(driver, "Not Matched || MCN Line Revevenue Others : " + amount_MCN_LRO+ " || Line Revevenue Others in Uninvoiced BL Report : " + actual_Line_Revenue, test, test1);
			}

			Step_End(15,"Shipment type has Cross Invoice Find the Payment Mode Values as Payat and and charge Mode as N  and Ignore OFR  AMT GO to Amount in USD .\r\n"
							+ "and compare both Uninvoiced  report and BL. If multiple charges is avaiable  system has to sum and compare Line Revenue–Others values in uninvoiced BL.",
					test, test1);
		}
