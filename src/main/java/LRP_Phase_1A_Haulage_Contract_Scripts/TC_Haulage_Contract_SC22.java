...
public class TC_Haulage_Contract_SC22 extends Keywords{
	public void Haulage_Contract_SC22(WebDriver driver, ExtentTest test, ExtentTest test1, String selected_dataset,Map<String, String> Excel_data,String url) {
		String testcase_Name="TC_Haulage_Contract_SC22";
		...
		twoColumnMultipleSearchWindow(driver, CurrencyCode_Header, condition, CurrencyCode);
		 ...
		 Step_Start(5, "Click the Amount type drop down and select the value.", test, test1);
		...
		waitForElement(driver, currency_Textfield_HC);
         String actualAmountType = getText(driver, amount_Type_Dropdown_HC);	
         
         if(AmountType.equals(actualAmountType)) {
        	 System.out.println("Expected Amount Type :"+AmountType + "||  Actual Amount Type : "+actualAmountType);
        	 Extent_pass_New(driver, "Expected Amount Type :"+AmountType + " || Actual Amount Type : "+actualAmountType, test, test1);
         }else {
        	 System.out.println("Expected Amount Type :"+AmountType + " || Actual Amount Type : "+actualAmountType);
             Extent_fail(driver, "Expected Amount Type :"+AmountType + "|| Actual Amount Type : "+actualAmountType, test, test1);
         }
		
         Step_End(5, "Click the Amount type drop down and select the value.", test, test1);
