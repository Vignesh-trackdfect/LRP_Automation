...
public class TC_Import_Documentation_SC097  extends Keywords{

	public void Import_Documentation_SC097(WebDriver driver, ExtentTest test, ExtentTest test1, String selected_dataset,Map<String, String> Excel_data,String url) {
		String testcase_Name="TC_Import_Documentation_SC097";
		...
		Map<String, String> newlyadded = setAccountingConfiguration(driver, AccountingConfiguration, Agency_Code_Header,attribute_Header,Condition_Filter,AgencyUser,Attribute_value,"","","",Value_checkbox_perform);
	
		Step_Start(5, "open the import documentation module", test, test1);
		moduleNavigate(driver, ImportDocumentationModule);
		Step_End(5, "open the import documentation module", test, test1);
		Step_Start(6, " Import Documentation Retrieve the  part lot bill using tool bar search.", test, test1);
		...
		Step_Start(7, " after retrieve the part lot bl,go to the collect charge tab..", test, test1);
		...
		Step_End(7, " after retrieve the part lot bl,go to the collect charge tab..", test, test1);
		Step_Start(8, " All agency local charges has shown for part lot bl..", test, test1);
		...
		for(String type_value :type_values) {
			if (Type_Verify.equals(type_value)||Type_Verify1.equals(type_value)) {
				System.out.println("Expected : All agency local charges should be present || Actual : All agency local charges are present || Expected : "+Type_Verify+" || Actual : "+type_value);
				Extent_pass_New(driver, "Expected : All agency local charges should be present || Actual : All agency local charges are present || Expected : "+Type_Verify+" || Actual : "+type_value, test, test1);
				Extent_pass_New(driver," Matched || Actual : n"+ "<pre>" +HBL_Table + "</pre>", test, test1);	
			}else {
				System.out.println("Expected : All agency local charges should be present || Actual : All agency local charges are not present || Expected : "+Type_Verify+" || Actual : "+type_value);
				Extent_fail(driver, "Expected : All agency local charges should be present || Actual : All agency local charges are not present || Expected : "+Type_Verify+" || Actual : "+type_value, test, test1);
			}
		}
		...
		reSetAccountingConfiguration(driver, accounting_Configuration_Reset, newlyadded, AccountingConfiguration, Agency_Code_Header, Condition_Filter, AgencyUser, Attribute_value, "");

		Step_End(8, " All agency local charges has shown for part lot bl..", test, test1);
