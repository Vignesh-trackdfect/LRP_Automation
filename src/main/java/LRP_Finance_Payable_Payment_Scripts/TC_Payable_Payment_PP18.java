...
public class TC_Payable_Payment_PP18 extends Keywords{
	
	public void Payable_Payment_PP18(WebDriver driver, ExtentTest test, ExtentTest test1,String selected_dataset,Map<String, String> Excel_data,String url) throws EncryptedDocumentException, InvalidFormatException, IOException{

		String testcase_Name="TC_Payable_Payment_PP18";
		...
		String accountName = Excel_data.get("accountName_val");
		...
		String Vendor_Code_1 = Excel_data.get("Vendor_Code_1");
		String Vendor_name=Excel_data.get("Account_Name_Header");
		...
		Step_Start(2, "Select the payment date in future from the date picker.", test, test1);
		...
		Step_End(2, "Select the payment date in future from the date picker.", test, test1);
		
		Step_Start(3, "Open period will get automatically populated based on payment date.", test, test1);
		...
		Step_End(3, "Open period will get automatically populated based on payment date.", test, test1);
		...
		twoColumnSearchWindow(driver, Vendor_Code_Header, dropdownCondition, Vendor_Code_1);
      ...
      if(exp_Cheque_Size>=total_Cheque_Size) {
		...
		twoColumnSearchWindow(driver,Vendor_name,dropdownCondition,accountName);


		Step_End(9, " Select Payment A/c", test, test1);
		
		Step_Start(10, "Select the ROE Difference if different currency", test, test1);
		
		if(!currencyCode.equals("")) {
			click(driver, currency_SearcButton_PPT);
			waitForElement(driver, currencyCode_Filter_PPT);
			
			sendKeys(driver, currencyCode_Filter_PPT, currencyCode);
			twoColumnSearchWindow(driver,Vendor_name,dropdownCondition,accountName);
			...
			if(!accountName_BankCharges.equals("") || !accountHead_BankCharges.equals("")) {
				...
				twoColumnSearchWindow(driver,accountName,dropdownCondition,accountHead_BankCharges);
				...
				if(!roeDifference.equals("")) {
					...
					twoColumnSearchWindow(driver,accountName,dropdownCondition,accountName_ROE);

				
					
					}
		Step_End(10, "Select the ROE Difference if different currency", test, test1);
		...
		Step_Start(15, "Click save, system should show the pop up message as The Payment Date must be earlier than or equal to the current date ", test, test1);
       ...
       waitForPopup(driver, popup_Message,Payable_Payment_Popup);
		...
		String PPT_Popup=getText(driver, popup_Message);
         if(Payable_Payment_Popup.equals(PPT_Popup)) {
        		System.out.println("Matched || Expected popup: After click save, system should be show the pop up message as The Payment Date must be earlier than or equal to the current date  : " + Payable_Payment_Popup + " || Actual Saved Popup :  After click save, system is show the pop up message as The Payment Date must be earlier than or equal to the current date" + PPT_Popup);
        		Extent_pass_New(driver," Expected popup: After click save, system should be show the pop up message as The Payment Date must be earlier than or equal to the current date " + Payable_Payment_Popup + " || Actual Saved Popup :  After click save, system is show the pop up message as The Payment Date must be earlier than or equal to the current date : " + PPT_Popup, test,test1);
    		    Extent_pass_New(driver, " system allows to create a receivable receipt by past date ", test, test1);
           
         } else {
    			System.out.println("Matched || Expected popup: After click save, system should be show the pop up message as The Payment Date must be earlier than or equal to the current date  : " + Payable_Payment_Popup + " || Actual Saved Popup :  After click save, system is not show the pop up message as The Payment Date must be earlier than or equal to the current date " + PPT_Popup);
    			Extent_fail(driver,"Matched || Expected popup: After click save, system should be show the pop up message as The Payment Date must be earlier than or equal to the current date  : " + Payable_Payment_Popup + " || Actual Saved Popup :  After click save, system is not show the pop up message as The Payment Date must be earlier than or equal to the current date: " + PPT_Popup,test, test1);
    		}

         waitForElement(driver, popup_Message_Ok_Button);
         click(driver, popup_Message_Ok_Button);
         
         Step_End(15, "Click save, system should show the pop up message as The Payment Date must be earlier than or equal to the current date ", test, test1);
