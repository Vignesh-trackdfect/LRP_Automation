...
public class TC_Vendor_Invoice_Registration_SC39  extends Keywords{
	
	
	public void Vendor_Invoice_Registration_SC39(WebDriver driver, ExtentTest test, ExtentTest test1,String selected_dataset,Map<String, String> Excel_data,String url) {

	String testcase_Name="TC_Vendor_Invoice_Registration_SC39";
	...
	Step_Start(1, "select and switch to the required agency", test, test1);

	SwitchProfile(driver, Agency);
	Step_End(1, "select and switch to the required agency", test, test1);
	//
	...
	Step_Start(13, "In the Activity details, Click Activity type dropdown and select the activity type as a \"\"BNK", test, test1);
	...
	Step_End(13, "In the Activity details, Click Activity type dropdown and select the activity type as a \"\"BNK", test, test1);
	...
	Step_Start(14, "Default system select the BRQ sum no.", test, test1);
	waitForElement(driver, BBQ_Grid_No);
	String FDR_Status = getAttribute(driver, BBQ_Grid_No, "class");

	if (extractstatus(FDR_Status)) {

		System.out.println(
				"Expected result is : By default system should select the BRQ No Tab || Actual result : BRQ No tab is selected as default ");
		Extent_pass_New(driver,
				"Expected result is : By default system should select the BRQ No Tab || Actual result : BRQ tab is selected as default",
				test, test1);

	} else {

		System.out.println(
				"Expected result is : By default system should select the BRQ No Tab || Actual result : BRQ  No tab is not selected as default ");
		Extent_fail(driver,
				"Expected result is : By default system should select the BRQ No Tab || Actual result : BRQ No tab is not selected as default",
				test, test1);

	}
	Step_End(14, "Default system select the BRQ sum no.", test, test1);
	
	
	
	
	
	Step_Start(15, "Default system select the BRQ sum no..", test, test1);
	waitForElement(driver, Manual_activity_button);
	click(driver, Manual_activity_button);
	Step_End(15, "Default system select the BRQ sum no..", test, test1);

	Step_Start(16, "Click the Main activity search and select the Activity.", test, test1);
	...
	Step_End(16, "Click the Main activity search and select the Activity.", test, test1);
	Step_Start(17, "Click the Sub activity \"\"+\"\" and select the require activity..", test, test1);
	...
	Step_End(17, "Click the Sub activity \"\"+\"\" and select the require activity..", test, test1);
	Step_Start(18, "Click eqp type search and select the Equipment type.", test, test1);
	...
	Step_End(18, "Click eqp type search and select the Equipment type.", test, test1);
	Step_Start(19, "Click the Port search and select the required Port", test, test1);
	...
	Step_End(19, "Click the Port search and select the required Port", test, test1);
