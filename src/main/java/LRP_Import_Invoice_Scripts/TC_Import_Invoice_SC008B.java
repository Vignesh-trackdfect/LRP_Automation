...
public class TC_Import_Invoice_SC008B extends Keywords {

	public void Import_Invoice_SC008B(WebDriver driver, ExtentTest test, ExtentTest test1, String selected_dataset,Map<String, String> Excel_data,String url) {

		String testcase_Name = "TC_Import_Invoice_SC008B";
		...
		String invoiceNUmberValue=getFirstValueByHeader(TabledataFull,"Inv Number");
		...
		List<Map<String, String>> ChargesTabValues_refresh;
		...
		if (Select_Extension.contains("Storage Extension")) {
			...
			ChargesTabValues_refresh = getTableData(driver, portStorageCharges_Table_Columns_IN,
			...
			ChargesTabValues_refresh = getTableData(driver, detentionCharges_Table_Columns_IN,
					detentionCharges_Table_Rows_IN);
		}

		List<String> refreshcolumnheaders = new ArrayList<String>();
		if (Select_Extension.equals("Storage Extension(S)")) {
			refreshcolumnheaders = splitAndExpand(Storage_Amount_Headers_Invoice);
		} else {
			refreshcolumnheaders = splitAndExpand(Detention_Amount_Headers_Invoice);
		}

		Map<String, List<String>> RefreshAmountValuesExp = getValuesByID(ChargesTabValues_refresh,
				Charges_Based_Header, containersList, refreshcolumnheaders);

		int j = 0;

			for (String container : containersList) {

				List<String> values = RefreshAmountValuesExp.get(container);

				String Amount = values.get(0);
				String Expected_amount = Container_costs.get(j);
				...
				if (Amount.equals(Expected_amount)) {
					...
					System.out.println("Containers are not added with expected Amount|| Expected Net amount : "
							+ Expected_amount + " || Actual Net amount : " + Amount);
					Extent_fail(driver, "Containers are not added with expected Amount || Expected Net amount : "
							+ Expected_amount + " || Actual Net amount : " + Amount, test, test1);
				}
		...
		Step_Start(16, "open the invoice reversal module ", test, test1);
		if(reverse_Option.equalsIgnoreCase("Yes")) {
			
			Invoice_Reversal(driver, InvoiceReversal_Module, BL_Condition, search_Type_Invoice_Reversal, invoiceNUmberValue, invoiceReversalRemarks, invoiceReversedPopup);
