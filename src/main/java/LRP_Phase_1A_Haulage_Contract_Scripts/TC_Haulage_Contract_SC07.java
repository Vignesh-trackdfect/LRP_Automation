...
public class TC_Haulage_Contract_SC07 extends Keywords{
	public void Haulage_Contract_SC07(WebDriver driver, ExtentTest test, ExtentTest test1, String selected_dataset,Map<String, String> Excel_data,String url) {
		String testcase_Name="TC_Haulage_Contract_SC07";
		...
		Step_Start(5, ".Click the Drop location search and select the location.", test, test1);
		
		waitForElement(driver, HC_Drop_Location_Search_Button);
		click(driver, HC_Drop_Location_Search_Button);
		
		twoColumnMultipleSearchWindow(driver, Port_Code_Header, condition, Drop_Location_Value);
		
		waitForElement(driver, HC_Drop_Location_Field);
         String Drop_Location_Value_Act = getAttribute(driver, HC_Drop_Location_Field, "value");	
         
         if(Drop_Location_Value.equals(Drop_Location_Value_Act)) {
        	 System.out.println("Expected Result is :"+Drop_Location_Value + " Actual Result is : "+Drop_Location_Value_Act);
        	 Extent_pass_New(driver, "Expected Result is :"+Drop_Location_Value + " Actual Result is : "+Drop_Location_Value_Act, test, test1);
         }else {
        	 System.out.println("Expected Result is :"+Drop_Location_Value + " Actual Result is : "+Drop_Location_Value_Act);
             Extent_fail(driver, "Expected Result is :"+Drop_Location_Value + " Actual Result is : "+Drop_Location_Value_Act, test, test1);
         }
		
         Step_End(5, ".Click the Drop location search and select the location.", test, test1);
