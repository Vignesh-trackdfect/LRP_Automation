...
public class TC_Import_Documentation_SC098 extends Keywords{

	public void Import_Documentation_SC098(WebDriver driver, ExtentTest test, ExtentTest test1, String selected_dataset,Map<String, String> Excel_data,String url) {
		String testcase_Name="TC_Import_Documentation_SC098";
		...
		if (SavedPopup.equals(ImportDocumentationSaved)) {
			...
			System.out.println("NotMatched || Expected value was : " + SavedPopup + " || Actual value was : " + ImportDocumentationSaved);
			Extent_fail(driver, "NotMatched || Expected value was : " + SavedPopup + " || Actual value was : " + ImportDocumentationSaved,
					test, test1);
		}
		waitForElement(driver, popup_Message_Ok_Button);
		safeclick(driver, popup_Message_Ok_Button);
		Step_Start(12, "select the hosuebl no in  HBL no drop down field and then go to the collect charge tab agency local charges has  not shown", test, test1);
		for(int i=0;i<HBL_BL_NumbersList.size();i++) {
			waitForElement(driver, HBL_No_DropDown);
			click(driver, HBL_No_DropDown);
			String HouseBL_No_Select = String.format(DropDown_Select,HBL_BL_NumbersList.get(i));
			click(driver, HouseBL_No_Select);
			...
			for(String type_value :type_values) {
				if (type_value.equals(Type_Verify)) {
					...
					System.out.println("Expected : '"+Type_Verify+"' should not be present in the Collect Charges table data for the House BL : "+HBL_BL_NumbersList.get(i)+" || Actual : '"+Type_Verify+"' is present in the Collect Charges table data ");
					Extent_fail(driver, "Expected : '"+Type_Verify+"' should not be present in the Collect Charges table data for the House BL : "+HBL_BL_NumbersList.get(i)+" || Actual : '"+Type_Verify+"' is present in the Collect Charges table data ", test, test1);
				}
			}
			
			if(agencyPresent!=false) {
				Extent_pass_New(driver," Collect Charges table data : "+ "<pre>" +HBL_Table + "</pre>", test, test1);	
				System.out.println("Expected : '"+Type_Verify+"' should not be present in the Collect Charges table data for the House BL : "+HBL_BL_NumbersList.get(i)+" || Actual : '"+Type_Verify+"' is not present in the Collect Charges table data ");
				Extent_fail(driver, "Expected : '"+Type_Verify+"' should not be present in the Collect Charges table data for the House BL : "+HBL_BL_NumbersList.get(i)+" || Actual : '"+Type_Verify+"' is not present in the Collect Charges table data ", test, test1);
			}
		}
		Step_Start(12, "select the hosuebl no in  HBL no drop down field and then go to the collect charge tab agency local charges has  not shown", test, test1);		
