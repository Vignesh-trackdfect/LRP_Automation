...
public class TC_Feeder_Contract_SC17 extends Keywords {
	public void Feeder_Contract_SC17(WebDriver driver, ExtentTest test, ExtentTest test1, String selected_Excel_dataset,
			Map<String, String> Excel_data, String url) {

		String testcase_Name = "TC_Feeder_Contract_SC17";
		...
		Step_Start(5, "Select the Load term using the Drop down", test, test1);
		...
		waitForElement(driver, FC_Load_Term_Field);
		String Actual_Load_Value = getAttribute(driver, FC_Load_Term_Field, "value");

		if (LoadTerm_Value.equals(Actual_Load_Value)) {
			System.out.println(" Matched || The Expected Load Term value should be : " + LoadTerm_Value
					+ " || The Actual Load Term value is : " + Actual_Load_Value);
			Extent_pass_New(driver, " Matched || The Expected Load Term value should be : " + LoadTerm_Value
					+ " || The Actual Load Term valueis : " + Actual_Load_Value, test, test1);
		} else {
			System.out.println(" Not Matched || The Expected Load Term value should be : " + LoadTerm_Value
					+ " || The Actual Load Term value is : " + Actual_Load_Value);
			Extent_fail(driver, " Not Matched || The Expected Load Term value should be : " + LoadTerm_Value
					+ " || The Actual Load Term value is : " + Actual_Load_Value, test, test1);
		}

		Step_End(5, "Select the Load term using the Drop down", test, test1);
