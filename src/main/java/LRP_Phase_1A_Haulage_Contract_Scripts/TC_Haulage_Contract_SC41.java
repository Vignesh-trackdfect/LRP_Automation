...
public class TC_Haulage_Contract_SC41 extends Keywords{
	public void Haulage_Contract_SC41(WebDriver driver, ExtentTest test, ExtentTest test1, String selected_dataset,Map<String, String> Excel_data,String url) {
		String testcase_Name="TC_Haulage_Contract_SC41";
		...
		String EQP_Equipment = Excel_data.get("EQP_Equipment");
		String HC_Amount_Grid_Value = Excel_data.get("HC_Amount_Grid_Value");
		String validation_Pop = Excel_data.get("validation_Pop");
								    		...
								    		for (String surcharge : surcharge_Value) {
								    		    if (surcharge != null && !surcharge.trim().isEmpty()) {
								    				...
								    				enter(driver);
								    				  ...
								    				  Step_Start(22, "Click the EQP behaviour and select the value.", test, test1);
								    		        
								    		        
								    		        waitForElement(driver, HC_Eqp_Behavier_click);
								    		        doubleClick(driver, HC_Eqp_Behavier_click);
								    		        mouseOverToElement(driver, HC_Eqp_Behavier_click);
								    		     formatLocatorClick(driver, DropDown_Select, EQP_Equipment);
								    		     
								    		 String eqp = getText(driver, HC_Eqp_Behavier_click);
								    		 System.out.println(eqp);
								    		     
								    		 if(EQP_Equipment.equals(eqp)) {
								            	 System.out.println("Matched||Expected Value taken From test data  :"+EQP_Equipment + " || Actual Value taken EQP Behaviour Droup Down: "+eqp);
								            	 Extent_pass_New(driver, "Matched||Expected Value taken From test data:"+EQP_Equipment + " ||  Actual Value taken EQP Behaviour Droup Down : "+eqp, test, test1);
								             }else {
								            	 System.out.println("NotMatched ||Expected Value taken From test data:"+EQP_Equipment + "|| Actual Value taken EQP Behaviour Droup Down  : "+eqp);
								            	 Extent_fail(driver, "NotMatched ||Expected Value taken From test data :"+EQP_Equipment + " || Actual Value taken EQP Behaviour Droup Down: "+eqp, test, test1);
								             }
								    		  waitForElement(driver, HC_Amount_Grid);
							    		        doubleClick(driver, HC_Amount_Grid);
							    		       // clear(driver, HC_Amount_Grid);
							    		        Actionsendkeys(driver, HC_Amount_Grid, HC_Amount_Grid_Value);
							    		        Step_End(22, "Click the EQP behaviour and select the value.", test, test1);
								    		 
											           
								    		    }
								    		}	
								    		  Step_Start(23, "Select the Location pair details and click apply surcharge.", test, test1);
								    			waitForElement(driver, HC_Apply_Charges_Button);
								    		click(driver, HC_Apply_Charges_Button);
								    		
								    		
								    		waitForElement(driver, popup_Message);	 
								    		 String validation = getText(driver, popup_Message);
								    		 System.out.println(validation);
								    		 if(validation.equals(validation_Pop)) {
								            	 System.out.println("Matched||Expected Value Popup is  :"+validation_Pop + " ||Actual Value Popup is: "+validation);
								            	 Extent_pass_New(driver, "Matched||Expected Value Popup is:"+validation_Pop + " ||Actual Value Popup is : "+validation, test, test1);
								             }else {
								            	 System.out.println("Matched||Expected Value Popup is:"+validation_Pop + "||Actual Value Popup is  : "+validation);
								            	 Extent_fail(driver, "Matched||Expected Value Popup is :"+validation_Pop + "|| Actual Value Popup is: "+validation, test, test1);
								             }
								    			waitForElement(driver, popup_Message_Ok_Button);
									    		click(driver, popup_Message_Ok_Button);
									    		
								    		 
								    		 Step_End(23, "Select the Location pair details and click apply surcharge.", test, test1);
